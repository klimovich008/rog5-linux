...
| GPU | rejected: KGSL second-open fault | GPUCC-only v12 proves the new GPUCC orphan completes, then localizes the global scan stall to `__clk_init_parent()` for a DISPCC pixel-clock orphan; watchdog rollback and cleanup pass; A660 DRM/MSM remains untested | reproduce a v13 inner parent-lookup trace, then power-domain/clock/regulator/IOMMU/GMU/firmware and full Tier 5 validation |
