...
| Wi-Fi/hotspot | passing | ath11k modules plus namespace packet tests for VPN-only forwarding, IPv4/IPv6 leak blocking, unsolicited isolation, VPN loss, and cleanup pass offline; the [real-WireGuard gate](../test-results/2026-07-27-vpn-hotspot-wireguard-offline.md) also accepts a kernel handshake and encrypted hotspot packet through the production kill-switch with disposable keys and exact cleanup | board PCIe/power/calibration/firmware, on-phone/provider WireGuard handshake, AP/DHCP/DNS, and device packet/thermal tests |
