...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; the sole v21 cycle now live-accepts one idle `arm-smmu` bind at runtime suspend with zero firmware/render/storage activity; the full A660 registration package passes offline but retains its pre-acceptance lock | pin v21 into and rebuild/reseal the registration-only tier, fully reverify it offline, then separate registration, first-open, and rendering gates; acceleration remains pending |
