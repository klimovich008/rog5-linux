...
| GPU | rejected: KGSL second-open fault | GPUCC-only v12 proves the new GPUCC orphan completes, then localizes the global scan stall to `__clk_init_parent()` for a DISPCC pixel-clock orphan; v13 inner callback/cache tracing passes duplicate offline gates but remains unbooted; A660 DRM/MSM remains untested | run one attended v13 probe with watchdog rollback, then power-domain/clock/regulator/IOMMU/GMU/firmware and full Tier 5 validation |
