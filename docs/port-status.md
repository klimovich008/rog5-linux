...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 offline-accepts its SMMU candidate; the full A660 graph, modular registration kernel, and exact four-node DT reproduce offline | run the v18 SMMU gate, finish the guarded runtime bundle, test registration, then test first-open separately; acceleration remains pending |
