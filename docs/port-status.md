...
| GPU | exact firmware requests, ucode source boundary, and rollback-safe diagnostic patch accepted; power/open pending | v3 accepts exact-reprobe GPU/GMU registration; the sole v4 cycle requests SQE/GMU once and rejects with `EUCLEAN` before ucode/power/HFI/ZAP; the next audit proves three SQE/shadow/reglist SMMU mappings, and its default-off one-shot patch passes eight mutations with balanced rollback | reproduce two isolated builds before considering a fresh gate, then test GMU resume/ZAP, successful open, submission, and rendering separately; acceleration remains pending |
