...
| GPU | rejected: KGSL second-open fault | GPUCC-only v13 proves the new GPUCC orphan completes, records the existing DISPCC pixel-clock provider as runtime-suspended, and enters its `get_parent()` callback without returning; exact regmap access and A660 DRM/MSM remain untested | source-test and reproduce a default-off v14 trace around the one existing display-RCG regmap read without changing PM behavior, then continue power-domain/clock/regulator/IOMMU/GMU/firmware and full Tier 5 validation |
