...
| GPU | exact firmware requests and rollback-safe ucode-allocation build pass; power/open pending | v3 accepts registration and v4 accepts exact SQE/GMU requests; the [v5 GO](../test-results/2026-07-26-a660-ucode-allocation-v5-prelive-go.md) produced one [safely rejected live cycle](../test-results/2026-07-26-a660-ucode-allocation-v5-live-rejected.md): kernel rollback balanced, but an incorrect wrapper-count oracle stopped before GEM snapshot; relocation evidence proves wrapper `get=1, put=2` and logical `4/4`; the [v6 offline root/gate](../test-results/2026-07-26-a660-ucode-allocation-v6-offline.md) pins direct helper traces and equal snapshots, its [one-shot host runner](../test-results/2026-07-26-a660-ucode-allocation-v6-prelive-hold.md) passes offline, and the [GO review](../test-results/2026-07-26-a660-ucode-allocation-v6-prelive-go.md) authorizes one RAM-only cycle | run the sole v6 cycle with no retry, then test GMU resume/ZAP, successful open, submission, and rendering; acceleration remains pending |
