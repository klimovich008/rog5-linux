...
| GPU | exact firmware requests and ucode-allocation source boundary accepted; power/open pending | v3 accepts exact-reprobe GPU/GMU registration; the sole v4 cycle requests SQE/GMU once and rejects with `EUCLEAN` before ucode/power/HFI/ZAP; the next offline audit proves exact SQE/shadow/reglist allocation uses three SMMU mappings and requires explicit rollback | implement and reproduce the rollback-safe one-shot diagnostic before considering a fresh gate, then test GMU resume/ZAP, successful open, submission, and rendering separately; acceleration remains pending |
