...
| GPU | exact firmware requests accepted; power/open pending | v3 accepts exact-reprobe GPU/GMU registration; the sole v4 cycle requests SQE/GMU once and rejects with `EUCLEAN` before ucode/power/HFI/ZAP while retaining zero DRM descriptors/storage/faults and exact fallback | isolate ucode allocation before power, then gate GMU resume/ZAP, successful open, submission, and rendering separately; acceleration remains pending |
