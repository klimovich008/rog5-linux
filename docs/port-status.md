...
| GPU | exact firmware requests, rollback-safe ucode allocation build, and trace-backed ucode-allocation v5 pre-live GO accepted; power/open pending | v3 accepts exact-reprobe GPU/GMU registration; the sole v4 cycle requests SQE/GMU once and rejects before ucode/power/HFI/ZAP; v5 has two byte-identical builds, a root-owned trace-backed gate, an exact one-invocation host runner, and a [verifier-first explicit live window](../test-results/2026-07-26-a660-ucode-allocation-v5-prelive-go.md) | run v5 at most once, then test GMU resume/ZAP, successful open, submission, and rendering separately; acceleration remains pending |
