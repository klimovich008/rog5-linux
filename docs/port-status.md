...
| GPU | exact firmware requests and rollback-safe ucode allocation build accepted; power/open pending | v3 accepts exact-reprobe GPU/GMU registration; the sole v4 cycle requests SQE/GMU once and rejects before ucode/power/HFI/ZAP; the next patch balances three SQE/shadow/reglist mappings and now has two byte-identical isolated builds with an unchanged Image/config/ABI and exact MSM-only delta | prepare and independently verify a fresh offline gate, then test GMU resume/ZAP, successful open, submission, and rendering separately; acceleration remains pending |
