...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 offline-accepts the consumer-disabled Adreno SMMU candidate; the full A660 graph is source-audited with probe-time IOMMU/RSCC/PDC separated from first-open firmware/power/SCM | run the v18 SMMU gate, build a manually loaded DRM/MSM registration tier with checked GMU power-level errors, then test first-open separately; acceleration remains pending |
