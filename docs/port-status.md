...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 stopped before probe on a verifier false positive; v19 passed baseline/GPUCC but safely rejected because the SMMU remained unbound, then watchdog rollback and cleanup passed; the full A660 graph and package reproduce offline | source-test a v20 exact-device deferred/supplier diagnostic and narrow platform reprobe, then require SMMU acceptance before registration and first-open gates; acceleration remains pending |
