...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 was safely consumed by a verifier false positive before SMMU bind; corrected v19 control scripts and firmware-free export pass offline around the unchanged binary; the full A660 graph, modular kernel, four-node DT, seven-module export, nested wrapper, and temporary-boot package reproduce offline | run the one-shot v19 SMMU gate, rebuild with its acceptance marker, test registration, then test first-open separately; acceleration remains pending |
