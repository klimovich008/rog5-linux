...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; the sole v21 cycle live-accepts one idle `arm-smmu` bind at runtime suspend; registration v2 pins that result into a new verified seven-module/zero-firmware export and re-passes the unchanged package offline | fail-first test the atomic one-shot registration runner, then keep registration, first-open, and rendering as separate gates; acceleration remains pending |
