...
| GPU | registration accepted; first open pending | v17 accepts isolated GPUCC; v21 accepts idle `arm-smmu`; the sole v3 cycle accepts exact-reprobe seven-module GPU/GMU registration, two IOMMU attachments, one unopened render node, zero firmware/faults, exact fallback, and complete cleanup | provision exact firmware under a no-open gate, then keep first-open and rendering separate; acceleration remains pending |
