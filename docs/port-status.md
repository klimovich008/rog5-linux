...
| GPU | exact firmware requests and rollback-safe ucode allocation pass; v8 GMU entry reached but userspace oracle rejected safely | v3 accepts registration and v4 accepts exact SQE/GMU requests; v5/v6 diagnose userspace oracles and are consumed; the [sole v7 live cycle](../test-results/2026-07-26-a660-ucode-allocation-v7-live-accepted.md) accepts raw-size-pinned allocation, exact rollback, logical `4/4`, and equal settled GEM state with zero later hardware activity; v7 is consumed; v8 passed its [pre-live HOLD](../test-results/2026-07-26-a660-gmu-resume-entry-v8-prelive-hold.md) and [one-cycle GO](../test-results/2026-07-26-a660-gmu-resume-entry-v8-prelive-go.md) controls; the [sole v8 cycle](../test-results/2026-07-26-a660-gmu-resume-entry-v8-live-rejected.md) reached exact entry/rollback and deliberate `EUCLEAN`, then rejected zero-extended signed returns and an unscoped runtime-PM count while direct inner resource probes stayed zero; exact fallback/cleanup passed and v8 is consumed | build a signed-`int`, GPU-device-scoped v9 userspace oracle around the unchanged v8 module; require settle/equal snapshot and new HOLD/GO before any cycle; only then isolate GMU power, HFI, and ZAP/SCM in separate tiers before successful open, submission, or rendering |
