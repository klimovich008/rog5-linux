...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 offline-accepts the consumer-disabled Adreno SMMU candidate; the full A660 graph and reproducible modular registration kernel pass offline with checked GMU power-level errors | run the v18 SMMU gate, build the four-node guarded registration bundle, test registration, then test first-open separately; acceleration remains pending |
