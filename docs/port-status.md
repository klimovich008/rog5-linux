...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v21 live-accepts idle `arm-smmu`; registration v3 carries its exact reprobe into a verified seven-module/zero-firmware export and passes nested-watchdog one-shot host/target gates offline | run at most one registration-only cycle, then keep first-open and rendering as separate gates; acceleration remains pending |
