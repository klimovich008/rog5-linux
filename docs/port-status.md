...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 stopped before probe on a detector false positive; v19 passed baseline/GPUCC but safely rejected because the SMMU remained unbound; v20 passed offline but its live gate stopped before action on the unset `(null)` override representation; the full A660 graph/package passes offline | source-test and offline-accept v21 with exact null-representation handling and a new root, then require SMMU acceptance before registration and first-open gates; acceleration remains pending |
