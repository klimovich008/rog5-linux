...
| GPU | rejected: KGSL second-open fault | GPUCC-only v14 reaches the existing DISPCC RCG `regmap_read()` while that provider is runtime-suspended and does not return; independent rollback, exact fallback, and cleanup pass; the mechanism and A660 DRM/MSM remain untested | model CCF/provider/regmap lock and PM ordering, test a general behavioral correction offline, then continue power-domain/clock/regulator/IOMMU/GMU/firmware and full Tier 5 validation |
