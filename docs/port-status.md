...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 stopped before probe on a verifier false positive; v19 passed baseline/GPUCC but safely rejected because the SMMU remained unbound; v20 exact-device reprobe and the full A660 graph/package now pass offline | run the single v20 SMMU bind/runtime-suspend gate, then require acceptance before registration and first-open gates; acceleration remains pending |
