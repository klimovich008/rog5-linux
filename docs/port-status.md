...
| GPU | rejected: KGSL second-open fault | v17 live-accepts isolated GPUCC; v18 stopped on a detector false positive; v19 safely rejected no-bind; v20 stopped before action on the unset `(null)` override text; v21 source-locks that text and passes offline with a new root; the full A660 graph/package also passes offline | run the single v21 SMMU bind/runtime-suspend gate, then require acceptance before registration and first-open gates; acceleration remains pending |
