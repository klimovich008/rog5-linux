...
| GPU | exact firmware requests and rollback-safe ucode-allocation build pass; power/open pending | v3 accepts registration and v4 accepts exact SQE/GMU requests; v5 exposed an inlining-blind wrapper oracle; the [sole v6 cycle](../test-results/2026-07-26-a660-ucode-allocation-v6-live-rejected.md) rejected raw sizes `43288/4/4096` against page-rounded expectations before the mandatory snapshot; watchdog fallback/cleanup passed and v5/v6 are consumed; [v7 passes offline](../test-results/2026-07-26-a660-ucode-allocation-v7-offline.md) with separate raw/object layers, logical `4/4`, equal snapshots, and a protected root; its [one-shot runner passes offline](../test-results/2026-07-26-a660-ucode-allocation-v7-prelive-hold.md), and the separate [pre-live GO review](../test-results/2026-07-26-a660-ucode-allocation-v7-prelive-go.md) authorizes at most one attended RAM-only cycle | run the sole v7 gate, consume it regardless of result, and only afterward test GMU resume/ZAP, successful open, submission, and rendering; acceleration remains pending |
