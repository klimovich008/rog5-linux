...
| Early capture / H01 | FAIL: pre-target sysfs ENODEV, exact exception phase not logged; retained replay is not a fix |
...
Last retained V7 health (not a fresh poll): uptime 12460.40 s, 29.9°C,
8.595 V, zero battery current, Good/USB online, P24 RO, no failed units.
...
0038/0039 in two files. Kernel/config/ABI inputs are frozen. Sequential clean
twins are running under private
...
The existing full local CI passed in 589.229 s for the builder correction;
subsequent workflow/observer changes use focused and active checks. This is
...
No new candidate is issued, signed, admitted or executed.
...
Last published code `8253a95a205e0b1014a54f2a9576af7778e67659` passed all four
jobs, run 34036160719. Working HEAD/status and current CI logs take precedence
...
Correct the capture defect only from discriminating evidence before another
phone cycle. No new boot is needed to finish these offline prerequisites.
