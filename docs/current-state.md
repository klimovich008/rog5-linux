...
Generation 199 is consumed with no p24 write. Mainline UFS, NCM, runtime and
key-only SSH passed in 7.86 seconds. The watchdog helper had survived its
200 ms init check but was no longer alive when the clone command began, so the
target failed before source verification or any write window. Exact slot-A
fastboot and durable `FALLBACK_RETURNED` resolution passed. The next cycle must
...
Generation 201 is consumed with no storage writes. The exact-ABI module removed
the load rejection, but upstream KPSS watchdog arming produced no reporter
frame, NCM TX timeout near 11 seconds, target USB loss near 13 seconds and stock
slot-A recovery return. Durable intent is `FALLBACK_RETURNED`. The upstream
`qcom,kpss-wdt` architecture is stopped; matching register offsets did not
...
Generation 206 is consumed: read-only mmap succeeded, but the first APSS
register access raised SIGBUS. Exact fastboot and `FALLBACK_RETURNED` passed.
...
Generation 212 is consumed and permanently non-retryable. It copied and proved
extents 1–17 (443,023,360 bytes), began extent 18, then reached the 840-second
softdog bound without an extent-18 PASS. Exact slot-A fastboot, cleanup, and
...
emitted exact PASS, synced, relocked, disarmed softdog, resolved
`TARGET_ACCEPTED`, and returned exact slot-A fastboot in 415.724 seconds.
...
Generation 218 passed and is consumed. Segment 2B copied 27,204 blocks /
111,427,584 bytes at offset 1,436,877, synced, relocked, disarmed softdog,
...
persistent slot-B boot installation. Generation 233 repeated the full result
in 249.912 seconds with first key SSH in 0.178 seconds, zero failed units,
...
exact-head publication CI passed. Generation 163 then repeated the complete
local Arch/systemd/key-only SSH path in 338.141 seconds and returned through
exitrd to exact slot-A fastboot. The RAM-only local-root MVP is reproducible.
