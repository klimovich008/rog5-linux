...
Current structured status: **Native mobile ABI passes; pinned GN reaches missing ARM64 sysroot**. Mobile physical tests: **NOT RUN**. Headless S06: **FAIL**; R01: **FAIL**.

Verified archival reclamation recovered 8.79 GB net while preserving final kernels, sources and evidence. The pinned GN executable is now available. After bounded interpreter-cache provisioning, a real network-disabled one-worker graph check reaches the explicit missing ARM64 sysroot assertion. Earlier environment/deadline FAIL results are retained. Next verify and provision the pinned ARM64 sysroot within measured disk capacity, then retry GN. Compilation, matching engine/ICU/AOT, native session and all physical qualification remain open. Source implementation and accepted/signed/installed/fallback artifacts are unchanged; no phone operation is authorized. S06/R01 remain FAIL. See [current repair evidence](../test-results/2026-09-12-storage-reclamation-gn.md) and [artifact pointer](../manifests/current-artifact.json). Historical checkpoints below remain unchanged.
