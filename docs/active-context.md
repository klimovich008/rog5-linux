...
The active v12 power/USB identity is defined only by
...
watchdog preconditions before hardware. V12 masks the two volatile units,
attests that rollback remains armed, captures the probe through pinned SSH,
then requests orderly reboot; it has no boot authority. See
