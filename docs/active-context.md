...
- **Generation 38 is consumed and localizes target loss to allocation or one
  of the three fixed-rate symbol-clock registrations.** Target NCM disappeared
  11.276 seconds after enumeration, essentially matching Generation 37, then
  exact Alpine fallback returned. Because this branch returned before OF
  clock-provider publication, that later boundary is cleared from the current
  failure set. The lifecycle separately hit a bounded host-cleanup race after
...
- **Generation 42 is consumed and clears the first fixed-rate clock with the
  generic CCF runtime-PM correction.** It reached stable target NCM in 61.333
  seconds, preserved it for 12.246 seconds, and returned to exact Alpine. No
...
- **Generation 55 is consumed and proves the retained-loader local-image
  path.** Exact UFS, both `ro,noload` ext4 mounts, tmpfs OverlayFS,
  `switch_root`, systemd, retained-loader attestation, NCM, and strict key-only
  SSH passed. Strict SSH accepted 344.676 seconds after lifecycle start, then
  normal systemd reboot, exact Alpine fallback, and host cleanup passed. No
  phone-storage write occurred. See the
...
- **Generations 61–66 are consumed; Generation 66 restored local-image Arch.**
  ...
  must never be retried. Generation 63 used a separate local-write kernel
  ...
  hardware-read-only gates. Its sole cycle passed the exact two-node write
  window, outer userdata RW mount, and writable loop attachment. The inner
  ...
  reused the hardware-accepted read-only v36 kernel and unchanged DTB, but its
  sealed initramfs accidentally omitted all four deferred UFS modules. Its
