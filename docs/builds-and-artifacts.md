...
| GPUCC/CCF network-root diagnostic | trace the SM8350 GPU clock-controller with every consumer disabled | v13 live completes the GPUCC orphan, records the DISPCC orphan runtime-suspended, and enters its `get_parent()` callback without returning; rollback and cleanup pass, exact regmap access remains unproved, normal coldplug remains rejected, and this is never a flash target |
