...
| minimal SSH-only Arch rootfs | active headless userspace without desktop/browser/GPU/radio/agent stacks | the 535,093,875-byte source archive from commit `eb61a459` becomes a byte-reproducible 535,110,731-byte sealed `network-root-v1` package; 37,669-entry full-tree seal, explicit hash-bound `workload=none`, exact 7.1.4 modules, 150 packages, strict key-only SSH, volatile machine/host identity, multi-user target, sleep inhibitor, no generic firmware; the dedicated 5,978,369-byte initramfs embeds the exact static AArch64 verifier; ephemeral-signed v2 bundle and actual consumed-P2 prepare/serve/verify/execute composition pass offline; unbooted, `authority=none`, no production key or phone authority |
