...
| locked Arch Plasma rootfs | headless-first target with Plasma/KRDP and browser/network tools | current 2,007,186,653-byte network-root archive has exact modules/firmware, a recoverable host key, and passes clean round-trip verification; privileged NFS gate passed, phone live gate pending |
...
| UFS-disabled network-root bundle | boot an ordinary distro from read-only NFS plus a volatile OverlayFS upper | fourteen-file v1 bundle, exact Arch rootfs, and privileged exact-peer host mount pass; attended phone boot pending; never flash |
...
and rollback cycles pass. Both cycles and the separate attended Linux 7.1
kexec/target/rollback gate now pass.
