...
| deployment SSH-key admission gate | bind one caller key to one non-fixture v3 root/package/candidate/runtime manifest before any privilege or phone discovery | fixed-descriptor `/usr/bin/ssh-keygen` derivation, canonical key/record metadata, exact corrected Image/DTB/generic-initramfs tuple, package-to-candidate-to-manifest root equality, explicit fixture-identity rejection, public-only canonical output, 14 hostile verifier tests, and 17 lifecycle-ordering tests pass with disposable keys only; integrated as local `key-preflight`, but no deployment key or non-fixture artifact chain has been used or built; `headless-ssh-deployment-v3` stable-recovery and read-only NFS profiles remain pending; see the [offline result](../test-results/2026-07-31-headless-ssh-v2-key-admission-offline.md) |
