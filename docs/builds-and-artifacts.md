...
| unissued NFS-return diagnostic successor | separate a returned NFS mount command from a blocked call and correlate it with host transport and current-cycle postmortem activity | host-only source adds monotonic stage 75, one boot-ID lineage line, private evidence-v2 same-port `cdc_ncm` plus kernel NFS-RPC snapshots, and a bounded read-only signed fallback pstore summary before strict health; the sealed 67,288-byte reporter is `dc53932d…a10` and qualified twin diagnostic initramfses are 6,011,337 bytes at `83240834…31d`; malformed counters, ambiguity, wrong ports, overflow, stage regression, pstore symlink/type/inode/mount/location races, evidence mutations, and cross-probe boot-ID changes fail closed across 63 fallback and 80 lifecycle tests; [complete local CI and independent review pass](../test-results/2026-08-05-stage75-postmortem-host-integration-offline.md), while publication and exact-head GitHub CI remain HOLD; no signed bundle, wrapper, candidate, policy row, or phone execution exists |
