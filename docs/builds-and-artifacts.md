...
| GPUCC/CCF network-root diagnostic | trace the SM8350 GPU clock-controller with every consumer disabled | v11 passes duplicate offline builds/packages and one attended live probe; it localizes the index-0 stall to `clk_core_reparent_orphans_nolock()` after orphan insertion, then rolls back cleanly; rejected for normal coldplug and never a flash target |
