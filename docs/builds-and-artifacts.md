...
| A660 GMU resume-entry v9 oracle tier | correct only the two v8 userspace trace assumptions before any deeper hardware step | [offline runtime acceptance](../test-results/2026-07-26-a660-gmu-resume-entry-v9-runtime-offline.md) reuses the exact v8 kernel/module, normalizes signed/zero-extended 32-bit `EUCLEAN`, scopes generic runtime PM by matching GPU device, accepts the evidence-derived 21-call fixture, reproduces byte-identical controls, and rejects twelve mutations; the [protected-root acceptance](../test-results/2026-07-26-a660-gmu-resume-entry-v9-root-offline.md) adds a consumed-v8-derived exact-delta root, signed/device oracle, compound target gate, and whole-tree/credential checks; the [pre-live HOLD](../test-results/2026-07-26-a660-gmu-resume-entry-v9-prelive-hold.md) adds a strict one-invocation runner, mock transport, credential agreement, private evidence, real unarmed refusal, root recheck, and inactive NFS/RPC; the [GO review HOLD](../test-results/2026-07-26-a660-gmu-resume-entry-v9-prelive-go-hold.md) adds one verifier-first explicit NFS case and passes all local package/server/host gates but stops because no physical fallback device exists; no NFS window, boot, or live authority; HOLD |
...
fallback remains required before any phone decision or GMU power-preparation
work. The
