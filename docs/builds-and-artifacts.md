...
| early-target diagnostic successor | distinguish post-NCM failure boundaries without adding a target command channel | the exact 67,288-byte write-only AArch64 reporter and 6,010,870-byte diagnostic initramfs reproduce under the qualified rootless ARM64 builder; `headless-netroot-early-diag-v1` binds them to the accepted Linux 7.1.4 Image, corrected DTB, and 37,735-entry sealed Arch lower. Production attempts exposed PolicyKit timing and stale-root pre-transfer gaps, then reached manifest transfer and deterministically rejected as fetch exit 50 before `PREPARE`/intent/NFS/`COMMIT_EXEC`: only the recovery fetcher incorrectly treated the diagnostic profile like persistent-root and required zero/`none` trust fields, while the documented contract, packager, host server, native verifier, and target cmdline all require the signed Arch tuple. Native, root-isolated, and AArch64/QEMU regressions now pass. A complete disposable-key deployment replay at clean checkpoint `2653e61` produced byte-identical 100,663,296-byte test-AVB wrappers `2a44a908…62c53`, raw wrappers `79ea621c…9ded`, ASUS Images `c6a7a4a6…ca87`, initramfses `3d4ac0cb…2b5ae`, and corrected fetcher `f410ca87…b5d13d`; native artifact preflight passed with `authority=none`, no production key, and no phone contact. A guarded production twin from checkpoint `84a9cc8` then produced exact AVB wrapper `f710bbcd…97b0ef`, raw wrapper `2f460aa0…628a01`, ASUS Image `7fac4dda…728ed`, initramfs `fec72c4d…1c57a`, and unchanged trust root `f10ca076…c57b`; source seals match, the private snapshot was destroyed, and the real artifact gate passes. Exact same-port fallback from the rejected live attempt passed at 43.5 C. The target remains unexecuted; the old `9c060a27…204ef` wrapper is consumed and denied, while the new wrapper is admitted for one RAM-only boot after bounded fallback-profile cleanup. See the [manifest rejection](../test-results/2026-08-01-early-target-diagnostic-fetch-manifest-rejected.md), [corrected disposable build](../test-results/2026-08-01-corrected-diagnostic-recovery-disposable-build.md), and [corrected production build](../test-results/2026-08-01-corrected-diagnostic-recovery-production-build.md). |
| diagnostic AVB generation issuer | avoid recompiling a byte-identical stable recovery after a consumed host-only lifecycle | generation zero exactly reproduces consumed AVB `f710bbcd…97b0ef`; generation one changes only the hash descriptor salt/digest and yields twin wrapper `332889a8…b51830` over unchanged raw recovery `2f460aa0…628a01`; atomic publication, exact output inventory, normalized full-descriptor equivalence, prefix/geometry/digest checks, distinct-generation tests, and complete diagnostic artifact preflight pass; AVB remains algorithm `NONE`, so trust is the pinned host identity plus embedded production Ed25519 root, not the generation nonce; no private key, phone, or boot action; see the [offline result](../test-results/2026-08-02-listener-successor-avb-generation-offline.md) |
