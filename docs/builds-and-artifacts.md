...
| persistent Arch P2 read-only boot | boot the sealed `userdata:/rog5/roots/arch-a` as an OverlayFS lower without authorizing a physical write | the [offline acceptance](../test-results/2026-07-28-persistent-root-p2-offline.md) adds an exact whole-tree AArch64 verifier, 116-node read-only lock, `ro,noload` userdata mount, volatile upper, strict SSH, screen-off attestation, fixed ACM controls, and an independent 600-second reset; verifier, target, Linux 7.1.4, nested stage, ASUS wrapper, and boot image reproduce twice; live remains pending because the phone has not reappeared on USB after the guarded reboot; temporary boot only, never flash |
