...
| persistent Arch P2 read-only boot | boot the sealed `userdata:/rog5/roots/arch-a` as an OverlayFS lower without authorizing a physical write | the [offline acceptance](../test-results/2026-07-28-persistent-root-p2-offline.md) adds an exact whole-tree AArch64 verifier, 116-node read-only lock, `ro,noload` userdata mount, volatile upper, strict SSH, screen-off attestation, fixed ACM controls, and an independent 600-second reset; the [first live attempt](../test-results/2026-07-28-persistent-root-p2-live-rejected.md) safely returned to Alpine but rejected the target, exposed an echoed-marker false positive and a missing wrapper UFS-discovery flag, and therefore keeps P2/P3 on HOLD; both controls now have fail-first regressions and the corrected wrapper reproduces twice; temporary boot only, never flash |
