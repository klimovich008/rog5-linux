...
| A660 ucode-allocation tier | isolate SQE/shadow/reglist creation before GPU/GMU runtime power or register access | source audit passes with exactly three new SMMU mappings; the default-off exact-device one-shot patch now passes strict verification and eight mutations with balanced object/IOVA/vmap/firmware rollback; duplicate builds, fresh root, and any live gate remain pending |
