...
| persistent Arch P2 read-only boot | boot the sealed `userdata:/rog5/roots/arch-a` as an OverlayFS lower without authorizing a physical write | the [offline acceptance](../test-results/2026-07-28-persistent-root-p2-offline.md) adds an exact whole-tree AArch64 verifier, 116-node read-only lock, `ro,noload` userdata mount, volatile upper, strict SSH, screen-off attestation, fixed ACM controls, and an independent 600-second reset; the [first live attempt](../test-results/2026-07-28-persistent-root-p2-live-rejected.md) safely returned to Alpine after rejecting the target and exposed an echoed-marker false positive plus volatile-key issue; the first correction then [safely rejected before staging](../test-results/2026-07-28-persistent-root-p2-wrapper-contract-live-rejected.md) because it enabled target-only UFS discovery on the ASUS wrapper; the next run [passed staging and selected the target's broad runtime-config branch](../test-results/2026-07-28-persistent-root-p2-config-timing-live-rejected.md) by exact fallback timing; the one-pass successor [also returned safely after 37 seconds](../test-results/2026-07-28-persistent-root-p2-config-identity-live-rejected.md) and is consumed; the latest fail-first package combines recovery-hashed Image identity, exact running release, and offline byte-exact embedded-config attestation with no live proc-config dependency; two latest repacks reproduce; P2/P3 remain HOLD; temporary boot only, never flash |
