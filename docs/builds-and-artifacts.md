...
| minimal SSH-only Arch rootfs | active headless userspace without desktop/browser/GPU/radio/agent stacks | 535,093,875-byte archive from commit `eb61a459`; exact 7.1.4 module tree, 150 packages, strict key-only SSH, volatile machine/host identity, multi-user target, sleep inhibitor, no generic firmware; build and clean re-extraction verification pass; rolling package snapshot is not yet bit-reproducible; unsigned/unbooted, no phone authority |
