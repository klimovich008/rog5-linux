...
| read-only UFS discovery bundle | enumerate the UFS topology without mounts or host-originated writes | v1 safely enumerated all UFS nodes read-only but was rejected on runtime-PM/BKOPS and shutdown; three-patch replacement source verifier passes; rebuild pending; never flash |
