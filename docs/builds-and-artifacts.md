...
| diagnostic AVB generation issuer | avoid recompiling a byte-identical stable recovery after a consumed host-only lifecycle | generation zero exactly reproduces consumed AVB `f710bbcd…97b0ef`; generation one `332889a8…b51830` reached signed-bundle `PREPARED`, exposed the host NFS-policy omission, returned to exact fallback, and is consumed; after the fail-closed host fix passed review/local/GitHub CI, generation two changed only descriptor salt/digest and yielded twin wrapper `70fd77f7…fc72b1` over unchanged raw recovery `2f460aa0…628a01`; atomic publication, exact output inventory, normalized descriptor equivalence, prefix/geometry/digest checks, and the complete artifact preflight pass; generation two is unbooted and singly admitted pending reviewed pins and connected preflight; AVB remains algorithm `NONE`, so trust is the pinned host identity plus embedded production Ed25519 root, not the generation nonce; see the [generation-1 result](../test-results/2026-08-02-listener-successor-avb-generation-offline.md), [live result](../test-results/2026-08-02-diagnostic-nfs-handoff-bypass-live.md), and [generation-2 result](../test-results/2026-08-02-nfs-gated-generation-2-avb-offline.md) |
