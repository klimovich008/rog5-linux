...
| early-target diagnostic successor | distinguish post-NCM failure boundaries without adding a target command channel | the exact 67,288-byte write-only AArch64 reporter and 6,010,870-byte diagnostic initramfs reproduce under the qualified rootless ARM64 builder; `headless-netroot-early-diag-v1` binds them to the accepted Linux 7.1.4 Image, corrected DTB, and 37,735-entry sealed Arch lower; two disposable-signed bundles, stable-recovery initramfses, clean ASUS 5.4 wrapper Images, header-v3 raw images, and test-only AVB images are byte-identical, pass native verification, and retain `authority=none`; the disposable private key is destroyed, no phone or credential was used, complete local CI passes, and independent packaging review reports no actionable findings; a separate exact admission/lifecycle mode now starts the receive-only collector before recovery control readiness, refuses COMMIT without one exact readiness line and pre-handoff liveness, independently binds canonical evidence to candidate/anchor/target boot, skips target SSH acceptance, rejects fallback boot-ID reuse, preserves rejected evidence, and resolves only after exact fallback; 16 admission and 34 lifecycle tests pass, complete local CI is green, and independent lifecycle closure review reports no remaining findings; a credential-free no-replace preparer, fixed-ID pre-key signing admission, guarded diagnostic builder, and exact diagnostic recovery-artifact profile now pass focused tests and complete local CI, while direct boot rejects both consumed normal manifests; the complete disposable-key wrapper/twin-build/native-verification/artifact-preflight path passes at clean checkpoint `0375e97a`, independent signing-readiness review reports no findings, and GitHub Actions run `30700630487` passes both required jobs at `6654c0c`; the production candidate is signed and installed, but its first stable-recovery boot rejected before bundle transfer when graphical PolicyKit exceeded the host readiness window; no COMMIT occurred, exact automatic fallback passed, and the still-unexecuted candidate is held behind the prompt-free host-socket remediation; see the [package result](../test-results/2026-08-01-early-target-diagnostic-candidate-offline.md), [lifecycle result](../test-results/2026-08-01-early-target-diagnostic-lifecycle-offline.md), [signing-readiness result](../test-results/2026-08-01-early-target-diagnostic-signing-readiness-offline.md), [GitHub CI result](../test-results/2026-08-01-early-target-diagnostic-github-ci.md), and [first live result](../test-results/2026-08-01-early-target-diagnostic-host-auth-timeout.md) |
