...
| deployment SSH-key admission, profile threading, and export installation | bind one caller key to one non-fixture v3 root/package/candidate/runtime manifest before any privilege or phone discovery, retain those identities through NFS/runtime acceptance, and publish the fixed export without replacement | fixed-descriptor `/usr/bin/ssh-keygen` derivation, canonical key/record metadata, exact corrected Image/DTB/generic-initramfs tuple, package-to-candidate-to-manifest root equality, explicit fixture-identity rejection, public-only canonical output, 14 hostile admission tests, 17 lifecycle tests, 13 recovery-control tests, and 27 runtime-verifier tests pass; the root-owned installer privately snapshots caller-owned archive bytes, rejects unsafe members, verifies and crash-syncs the extracted root, and publishes once with `RENAME_NOREPLACE`, while its launcher keeps the private key/candidate/manifest outside the privileged command; 11 installer and 8 launcher tests cover replacement, link/device/credential, stale-component, and publication-race attacks; one dedicated non-fixture key is now bound to a verified 536,746,495-byte root, candidate, signed bundle, recovery trust root, and byte-identical wrappers, and the exact `headless-ssh-deployment-v3` artifact preflight passes; the reviewed host components and v3 export are not yet installed and no phone boot occurred; see the [admission](../test-results/2026-07-31-headless-ssh-v2-key-admission-offline.md), [profile-threading](../test-results/2026-07-31-headless-ssh-v3-profile-threading-offline.md), [export-installer](../test-results/2026-07-31-headless-ssh-v3-export-installer-offline.md), and [deployment-chain](../test-results/2026-07-31-headless-ssh-deployment-chain-offline.md) results |
