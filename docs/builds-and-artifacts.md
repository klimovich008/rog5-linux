...
| persistent Arch P2 read-only boot | boot the sealed `userdata:/rog5/roots/arch-a` as an OverlayFS lower without authorizing a physical write | the [offline acceptance](../test-results/2026-07-28-persistent-root-p2-offline.md) adds an exact whole-tree AArch64 verifier, 116-node read-only lock, `ro,noload` userdata mount, volatile upper, strict SSH, screen-off attestation, fixed ACM controls, and an independent 600-second reset; five attended packages have safely rejected and returned to exact fallback: the [first target](../test-results/2026-07-28-persistent-root-p2-live-rejected.md), [wrapper contract](../test-results/2026-07-28-persistent-root-p2-wrapper-contract-live-rejected.md), [broad config](../test-results/2026-07-28-persistent-root-p2-config-timing-live-rejected.md), [config identity](../test-results/2026-07-28-persistent-root-p2-config-identity-live-rejected.md), and [kernel release](../test-results/2026-07-28-persistent-root-p2-kernel-release-live-rejected.md); the fifth run executed one target kexec and returned after 36 seconds, left the sealed root exact and `UNBOOTED`, but exposed an unresolved fallback screen-on state; the latest fail-first package reads `/proc/sys/kernel/osrelease` directly, separates read and mismatch timing markers, retains recovery-hashed Image and offline byte-exact config identity, and reproduces at all changed layers; P2/P3 remain HOLD; temporary boot only, never flash |
