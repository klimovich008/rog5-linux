...
| exact-UDC stable-recovery initramfs | remove arbitrary-controller selection before any physical retention experiment | stable recovery now accepts only one stable exact `a600000.dwc3`, revalidates before/after binding, and fail-closes zero-at-deadline, wrong, renamed, multiple, or changing candidates; twin shell-free 7,602,301-byte initramfses reproduce at `afc55f96…d790` with disposable trust input; current `exact-a600000-v1` and hash-pinned historical contracts cannot be interchanged; no wrapper, candidate, policy, credential, phone action, or boot authority; see the [offline result](../test-results/2026-08-09-stable-recovery-exact-udc-offline.md) |
