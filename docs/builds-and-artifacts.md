...
| A660 ucode-allocation tier | isolate SQE/shadow/reglist creation before GPU/GMU runtime power or register access | source audit passes with exactly three new SMMU mappings and identifies mandatory explicit rollback for all objects and retained CPU vmaps; diagnostic patch, duplicate builds, fresh root, and any live gate remain pending |
