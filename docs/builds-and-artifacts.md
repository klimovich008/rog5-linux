...
| stage-75 v2 NFS-return diagnostic successor | separate a returned NFS mount command from a blocked call and correlate it with host transport and current-cycle postmortem activity | the active unissued source makes one diagnostic NFS attempt, emits stage 70/75 around that call, classifies exact UDC/interface/carrier/address/route/NFS state, and retains the published current-cycle postmortem contract; reporter `0b5d318e…7bc1` and 6,011,687-byte initramfs `71537ca0…c58e` reproduce as clean offline twins; the earlier disposable-key wrapper tuple remains superseded composition evidence only; no production credential, policy row, phone execution, or boot authority exists |
