...
| A660 GMU/CX runtime-PM v10 tier | isolate the first normal GMU-device resume and linked CX-supplier transition before GX or later resources | the [offline acceptance](../test-results/2026-07-27-a660-gmu-cx-runtime-pm-v10-offline.md) source-pins the call graph, rejects twelve patch mutations, passes strict patch checks, and reproduces two isolated Linux 7.1.4 builds; config/Image/ABI and every installed module except `msm.ko` remain exactly accepted v8; the default-off atomic diagnostic balances failed gets, synchronously returns GMU/CX to suspended state, and stops before GX, clocks, secure init, MMIO, IRQ, firmware/HFI, hardware, ZAP/SCM, storage, submit, or render; offline-only and HOLD with no runtime oracle, protected root, bounded server case, boot authority, retry, or flash |
...
byte-identical isolated builds. It remains offline-only and unauthorized for
runtime use: no protected root, bounded server case, boot, retry, or flash
exists. The
