...
| key-bound headless SSH v2 root | credential-clean successor with exact SSH access identity | twin 536,750,378-byte source archives from commit `9739abe` have SHA-256 `2abe8c…72e6a`; source and sealed root expose the same byte-pinned 152-package closure `13586291…f8b`, now checked against fresh Pacman state on every stage; the 536,747,283-byte sorted sealed lower has 37,735 entries and a 731-byte `rog5-headless-network-root-package-v3` manifest; one canonical Ed25519 fingerprint is recomputed from the SSH blob and bound across the root build record, `/root/.ssh/authorized_keys`, complete tree seal, and package; v3 fixes effective `AuthorizedKeysFile` to the bound path while historical v1/v2 formats remain exact; the distinct `headless-ssh-network-root-v3` fixture candidate binds that tree/seal to the accepted DTB and passes twin signed-bundle, shell-free recovery, native-verifier, and byte-identical ASUS wrapper gates; public fixture only, disposable signing key destroyed, no phone, unbooted, `authority=none`; see the [package](../test-results/2026-07-30-headless-ssh-v2-key-bound-package.md), [closure](../test-results/2026-08-08-headless-package-closure-offline.md), and [candidate](../test-results/2026-07-30-headless-ssh-v2-candidate-offline.md) results |
| early-target diagnostic successor | distinguish post-NCM failure boundaries without adding a target command channel | the reproducible 67,288-byte write-only AArch64 reporter and 6,010,870-byte diagnostic initramfs bind the accepted Linux 7.1.4 Image, corrected DTB, and 37,735-entry sealed Arch lower. Successive one-shot cycles closed PolicyKit, bundle-inventory, recovery-fetch, and host-listener defects. Generation 1 then fetched and verified the complete signed bundle, returned `PREPARED`, and claimed one commit, proving the recovery path through the non-retryable boundary. A host control omission let diagnostic bypass the exact v3 NFS rendezvous, so NFS never started and no target frame arrived; signed same-port Alpine fallback passed and the intent is resolved `FALLBACK_RETURNED`. All used wrappers are consumed. The fail-closed host fix adds diagnostic to the exact profile/package-bound NFS policy and rejects unknown guarded bundles before device discovery; 20 control and 39 lifecycle tests pass. See the [production build](../test-results/2026-08-01-corrected-diagnostic-recovery-production-build.md), [generation-1 issuer](../test-results/2026-08-02-listener-successor-avb-generation-offline.md), and [live NFS-bypass result](../test-results/2026-08-02-diagnostic-nfs-handoff-bypass-live.md). |
| diagnostic recovery generations | preserve one-shot evidence while correcting the recovery transport | Generations zero `f710bbcd…97b0ef`, one `332889a8…b51830`, two `70fd77f7…fc72b1`, three `eb514a57…d77b6`, four `220e8556…270d`, five `abe4501f…beb1a`, six `6aa47517…d398`, seven `d3d4cdb9…12901`, eight `f102d53c…f2415`, nine `b458e64b…d008`, ten `b983e89b…8b51`, eleven `8472b206…bcf562`, and twelve `615d7498…d72cf6` are consumed, absent from boot policy, and never reusable. `manifests/artifacts.tsv` is the full-identity inventory, not boot authority; its role field's leading `consumed` token is the machine-enforced lifecycle disposition. Only `manifests/temporary-boot-images.tsv` can admit a temporary boot. Generation 10 embedded the five-phase responder in production initramfs `99046d30…6e31`, wrapper Image `bb49b405…5f98`, and raw wrapper `27f4dbcc…73b3`. Its reviewed connected-preflight checkpoint `f4b9e1c` passed exact-head run `30872608193`; the sole RAM-only lifecycle then reached exact recovery ACM/NCM, emitted correlated `REQUEST_ACCEPTED`, and completed the 46,163,787-byte host transfer. ACM then closed before any later progress or `PREPARED` response reached the host; the exact device-side boundary remains unknown. Explicit `prepare-replay` discovery saw 216 stable fallback/product-mismatch samples, the diagnostic collector recorded zero target frames, no COMMIT intent existed, and no target ran. Exact Alpine fallback, strict SSH, profile restoration, and final host cleanup passed. The permanent private boot claim, removed policy row, and consumed inventory role independently prevent reuse or former-exact-basis readmission. Generation 11 adds the receive-only NCM progress responder in production initramfs `3695ded2…6a8c`, clean twin wrapper Image `895272e8…3eae`, raw wrapper `44c43e27…12b2`, and deterministic AVB `8472b206…bcf562`. It exists only in two ignored byte-identical trees with `authority=none`; immutable offline profile `headless-diagnostic-generation11-offline-v1` and the exact inventory row verify both trees against the unchanged signed-target twins. Separate live-capable profile `headless-diagnostic-generation11-live-v1` binds the identical tuple to the one-shot lifecycle, while direct connected actions and malformed policy fixtures reject before host inspection. Its exact key and connected preflight passed after an anchored Alpine-to-fastboot transition. The sole RAM-only lifecycle reached recovery ACM/NCM, then failed closed before the bundle-server ready marker when the privileged host path rejected its newly started TCP 8081 collector as not uniquely confined. Progress remained `PARTIAL/NO_ADMISSION` with zero records and `authority=NONE`; no PREPARE, transfer, COMMIT intent, NFS, or target occurred. Exact Alpine fallback, strict SSH, host cleanup, and Steam socket restoration passed. The policy row is removed, the role is consumed, and the permanent claim prevents reuse. See the Generation-9 [live result](../test-results/2026-08-03-generation-9-prepared-response-gap-live.md), [progress result](../test-results/2026-08-03-prepare-progress-observability-offline.md), [wrapper integration](../test-results/2026-08-03-prepare-progress-wrapper-integration-offline.md), [Generation-10 successor](../test-results/2026-08-03-generation-10-prepare-progress-successor-offline.md), [offline profile](../test-results/2026-08-03-generation-10-offline-profile.md), [live-profile transition](../test-results/2026-08-04-generation-10-live-profile-offline.md), [admission](../test-results/2026-08-04-generation-10-live-admission-offline.md), [connected preflight](../test-results/2026-08-04-generation-10-connected-preflight-live.md), [live result](../test-results/2026-08-04-generation-10-request-accepted-transport-gap-live.md), [Generation-11 offline wrapper](../test-results/2026-08-04-generation-11-ncm-progress-wrapper-offline.md), [Generation-11 offline profile](../test-results/2026-08-04-generation-11-offline-profile.md), [Generation-11 live-profile transition](../test-results/2026-08-04-generation-11-live-profile-offline.md), [Generation-11 admission](../test-results/2026-08-04-generation-11-live-admission-offline.md), [Generation-11 connected preflight](../test-results/2026-08-04-generation-11-connected-preflight-live.md), [Generation-11 live result](../test-results/2026-08-04-generation-11-progress-listener-confinement-live.md), and historical [archive](archive-index.md). |
| Generation-12 recovery successor | isolate the next target failure after correcting Generation-11 host listener confinement | deterministic ignored twins preserve exact raw wrapper `44c43e27…12b2`, kernel `895272e8…3eae`, config, and NCM-capable initramfs `3695ded2…6a8c` while deriving distinct AVB `615d7498…d72cf6`; offline checkpoint `52ce322`, live admission `328b33c`, connected-preflight publication `1ee5508`, and their exact-head CI passed; the sole lifecycle entered its irreversible claim, transferred all 46,163,787 signed-bundle bytes, accepted correlated PREPARE/COMMIT, and captured 40 lossless target frames through stage 70 `nfs-mount-begin`; USB disconnected before stage 80 `nfs-mount-ok` or terminal fault evidence; the outer lifecycle parser then rejected the valid 18-field PREPARE record after commit because it still expected the obsolete seven-field shape, a separate host-classification defect now covered by exact response and native-fingerprint tests; watchdog fallback, strict SSH, profile/host/Steam cleanup, and `FALLBACK_RETURNED` resolution passed; policy removed, artifact and claim consumed, production diagnostics held before credentials or phone inspection, never retry or flash; see the [live result](../test-results/2026-08-04-generation-12-nfs-mount-disconnect-live.md) |
| stage-75 v2 NFS-return diagnostic successor | separate a returned NFS mount command from a blocked call and correlate it with host transport and current-cycle postmortem activity | the active unissued wire identity makes one diagnostic NFS attempt, emits stage 70/75 around that call, classifies exact UDC/interface/carrier/address/route/NFS and host-port state, and retains the published current-cycle postmortem contract; reporter `26249252…bafa` and the 6,013,458-byte bounded-rendezvous v3 initramfs `94edd625…cffc` reproduce as clean offline twins; the v2 payload and earlier disposable-key wrapper tuple remain superseded historical composition evidence only; no production credential, policy row, phone execution, or boot authority exists |
| pstore-aware stable-recovery refreeze | prove the lineage responder is present in a complete reproducible recovery product before any retention experiment | twin shell-free 7,601,886-byte initramfses `c778588a…a380` and two clean source-sealed ASUS 5.4 builds reproduce config `df28224e…d78f`, 50,498,048-byte kernel `4b30cfff…9495`, 58,105,856-byte raw boot-v3 `5141f0d0…deab`, and 100,663,296-byte unsigned AVB `b004e500…c218`; the built config and inspected command line retain pstore/ramoops and the exact 4 MiB reservation; disposable trust input, `Algorithm: NONE`, no candidate/policy/boot authority, and physical retention still unproven; see the [offline result](../test-results/2026-08-09-recovery-postmortem-refreeze-offline.md) |
| exact-UDC stable-recovery initramfs | remove arbitrary-controller selection before any physical retention experiment | stable recovery now accepts only one stable exact `a600000.dwc3`, revalidates before/after binding, and fail-closes zero-at-deadline, wrong, renamed, multiple, or changing candidates; twin shell-free 7,602,307-byte initramfses reproduce at `afc55f96…d790` with disposable trust input; current `exact-a600000-v1` and hash-pinned historical contracts cannot be interchanged; no wrapper, candidate, policy, credential, phone action, or boot authority; see the [offline result](../test-results/2026-08-09-stable-recovery-exact-udc-offline.md) |
| observation-only stable-recovery initramfs | make postmortem inspection a distinct non-execution identity before any retention experiment | explicit sealed `observation-only-v1` mode permits only `HELLO`/`STATUS`, refuses prepare/commit before mutation, and requires pristine state; twin 5,371,780-byte archives reproduce at `613d6e3e…70db` without fetcher, verifier, trust key, kexec binary, or bundle root; full/observer/historical contracts cannot be interchanged; ignored offline evidence only, with no wrapper, candidate, policy, credential, phone action, retention claim, or boot authority; see the [offline result](../test-results/2026-08-09-observation-only-recovery-offline.md) |
| current Haven observation-recovery HOLD gate | bind the final observer clean twins to a fixed hardware-free entry point before sequence design | exact profile pins 5,374,739-byte initramfs `b2440d8c…`, 48,400,896-byte Image `eedb7deb…`, 53,784,576-byte raw boot-v3 `5daf0919…`, 100,663,296-byte unsigned AVB `3c9b2820…`, and all 22 verifier inputs; unsafe links, identity drift, and connected actions fail closed; hard-linked retained outputs require an independent fixed-file copy-on-write snapshot; no claim, policy row, sequence runner, phone action, or boot authority; see the [offline result](../test-results/2026-08-10-current-observation-recovery-live-gate-offline.md) |
| two-claim retention sequence reference | define the exact irreversible transaction before writing a live runner | pure 11,923-byte no-I/O model `97075ed7…` binds cycle `d8a3a085…`, execution/observer AVBs, and distinct 557/555-byte draft records `ef0895b7…`/`f0b68716…`; hostile tests enforce rollback, target boot-ID correlation, exact fallback and ramoops preflight, same-port bootloader/observer handoff, one postmortem read, exact failure dispositions, no retry, and inconclusive missing pstore; drafts are unregistered, policy is empty, both connected gates remain closed, and no claim, runner, phone action, or boot authority exists; see the [offline result](../test-results/2026-08-10-retention-sequence-reference-offline.md) |
| retention-cycle transaction journal | preserve exact dynamic lineage and at-most-once intent across future host-process handoffs | offline-only 39,553-byte source `a7018537…` writes private append-only canonical events chained by SHA-256 with exclusive creation, file/directory `fsync`, no-follow metadata/path/inventory revalidation, and nonblocking writer exclusion; it binds host boot, USB location, target/fallback boot IDs, both claim dispositions, exact fastboot serial, rollback intents, and one preclaimed observer read; 9 hostile groups and 22 joint-admission groups pass; no CLI, helper invocation, claim registration, policy row, device action, or boot authority; see the [offline result](../test-results/2026-08-10-retention-cycle-transaction-offline.md) |
| retention-cycle callback adapter | prove fixed helper ordering behind the durable transaction without adding an executor | corrected 10,260-byte callback-only source `c36b4bfa…` binds journal `a7018537…`, cycle `d8a3a085…`, and six exact repository helper descriptors; fallback transition uses the accepted nonce-framed ACM helper instead of the legacy SSH-key path; injected callbacks see their fsynced intent before execution, while all callback failures, hostile/type-aliased results, weak lineage, and state mutation fail without advancing or retrying; 7 adapter and 25 joint-admission hostile groups pass; no CLI, built-in executor, claim registration, policy row, credential, device action, or boot authority; see the [offline result](../test-results/2026-08-10-retention-cycle-adapter-offline.md) |
| retention-cycle executor contract | pin the future process boundary independently of callback ordering | pure 14,560-byte source `8705c7fd…` binds six exact helper identities/modes, fixed interpreters and arguments, closed per-action environments, devnull stdin, separate bounded stdout/stderr, 15/300/240/15/300/90-second deadlines, process-group cleanup, and zero-only exits; all boot actions request one canonical result protocol and carry exact fastboot/USB lineage; a canonical unopened known-hosts path is the sole fallback pin input and no SSH private key is accepted; 8 contract and 25 joint-admission hostile groups pass; no subprocess, CLI, connected executor, claim, policy row, credential access, phone action, or boot authority; see the [offline result](../test-results/2026-08-10-retention-cycle-executor-contract-offline.md) |
| retention-cycle executor boundary | prove descriptor and output interpretation before any launcher exists | pure 24,548-byte source `76cd7367…` pins descriptor/path/symlink-target evidence for all six programs and Python/Bash interpreters, plus a caller-owned public Ed25519 host-pin snapshot; exact claim, boot-result, and postmortem records decode only with matching descriptor attestation and lineage; fallback has one guarded producer grounded in same-port fastboot and public-pin evidence, while execution/observer producers remain HOLD; 11 boundary and 26 joint-admission hostile groups pass; admitted host-pin digest and production execution remain undefined, with no I/O, CLI, credential use, connected admission, claim, policy row, phone action, or boot authority; see the [boot-output result](../test-results/2026-08-10-retention-cycle-boot-output-contract-offline.md) |
| retention-cycle runtime-closure fixture | prove fresh intent/pipe/process binding without wiring a live launcher | mode-`0644` source `2cce1d45…` holds and revalidates one fsynced intent, exact program/interpreter descriptors, and optional public host pin before creating distinct empty CLOEXEC pipes and a 256-bit nonce; a fixed forked writer uses devnull stdin, closed unrelated FDs, bounded streams/deadline, and TERM/KILL group cleanup; 13 hostile groups include all six actions in one journal, exact decoded-result/durable-event equality, result-event pathname replacement, stale cross-cycle proof, reopened/duplicate preparation, path/journal races, preloaded pipes, timeout/descendant/overflow, and pin symlink refusal; results are authority-free and adapter-ineligible, production descriptor execution remains unproven, and no live entry point, claim, credential, gate, phone action, or boot authority exists; see the [offline result](../test-results/2026-08-10-retention-cycle-runtime-closure-offline.md) |
| retention-cycle held-descriptor execution fixture | prove the exact exec mechanism without running a phone-capable helper | mode-`0644` runner `7e82e52e…` and harmless probe `afba8ae9…` hold/revalidate repository, probe, and Python descriptors; `fchdir()` enters the held cwd, interpreter FD 199 executes probe FD 198 under exact kernel-visible argv/closed environment, `0077`, devnull, bounded pipes, `/proc/self/fd`-verified inherited-FD closure, a new session/process group, and `ECHILD`-aware TERM/KILL cleanup plus bounded post-stop EOF drain; 11 hostile groups cover process-context/path mutation before and after exec, high FDs above a lowered soft limit, reaped-PID signal safety, timeout/descendant PID and pipe-EOF cleanup, overflow/nonzero exit, terminal proof consumption, proof substitution, and malformed output; 27 joint-admission tests pin both sources; fixture execution is proven but all six production helper executions remain unproven, with no adapter, claim, credential, policy row, recovery gate, phone action, or authority; see the [offline result](../test-results/2026-08-10-retention-cycle-descriptor-execution-offline.md) |
| deployment SSH-key admission, profile threading, and export installation | bind one caller key to one non-fixture v3 root/package/candidate/runtime manifest before any privilege or phone discovery, retain those identities through NFS/runtime acceptance, and publish the fixed export without replacement | fixed-descriptor `/usr/bin/ssh-keygen` derivation, canonical key/record metadata, exact corrected Image/DTB/generic-initramfs tuple, package-to-candidate-to-manifest root equality, explicit fixture-identity rejection, public-only canonical output, 14 hostile admission tests, 17 lifecycle tests, 13 recovery-control tests, and 27 runtime-verifier tests pass; the root-owned installer privately snapshots caller-owned archive bytes, rejects unsafe members, verifies and crash-syncs the extracted root, and publishes once with `RENAME_NOREPLACE`, while its launcher keeps the private key/candidate/manifest outside the privileged command; 13 installer and 8 launcher tests cover replacement, link/device/credential, unsafe destination ancestry, host-version refusal, stale-component, and publication-race attacks; one dedicated non-fixture key is now bound to a verified 536,746,495-byte root, candidate, signed bundle, recovery trust root, and byte-identical wrappers, and the exact `headless-ssh-deployment-v3` artifact preflight passes; the fixed host components and signed bundle are installed, but export publication stopped safely because SteamOS's 230 MiB `/var` cannot hold the 1.53 GiB lower; the reviewed replacement is the root-owned `/home/rog5-linux/exports` store, and no phone boot occurred; the consumed bundle is now replaced by `headless-ssh-network-root-v3-r2`, whose production twin build is byte-identical, destroys its private key snapshot, separately enforces candidate and transfer-bundle identities, and passes the same exact artifact gate without phone access; see the [admission](../test-results/2026-07-31-headless-ssh-v2-key-admission-offline.md), [profile-threading](../test-results/2026-07-31-headless-ssh-v3-profile-threading-offline.md), [export-installer](../test-results/2026-07-31-headless-ssh-v3-export-installer-offline.md), [deployment-chain](../test-results/2026-07-31-headless-ssh-deployment-chain-offline.md), [SteamOS storage remediation](../test-results/2026-07-31-steamos-export-storage-remediation.md), and [signed r2 build](../test-results/2026-07-31-headless-ssh-successor-r2-signed-build.md) results |
| headless battery-series oracle | define the sustained read-only evidence needed before H3 charger/battery hardware acceptance | fixed `headless-ssh-network-root-v3` collector records 21 samples over 10 minutes for one explicit unplugged, USB-online, or wireless-online phase; it requires exact SM8350 battery/USB/wireless supplies, read-only telemetry and input-current-limit files, zero charge thresholds, and zero Type-C control devices; the host verifier binds source/candidate/boot identity, canonical record metadata and schedule, then compares unplugged/USB status and median current while deriving either sign convention; 11 hostile groups plus the current 39 compatibility-oracle and 74 source/DT tests (1 optional skip) pass hardware-free; no phone, control write, dual-cell interpretation, or charging-safety claim; see the [contract](battery-telemetry-series.md) and [offline result](../test-results/2026-07-31-headless-battery-series-offline.md) |
| dual-cell read-only network-root candidate | prove the bounded ASUS PMIC GLINK read as a reproducible linked kernel feature before any phone observation | deterministic patched source commit `7ee91d34…ea5` and tree `ef7703ec…eefe` produce release `7.1.4-00001-g7ee91d34b545`; two clean complete uncached builds reproduce `.config`, Image, Image.gz, `Module.symvers`, module archive, metadata, and 351,824-byte `qcom_battmgr.ko` (`5a338fb5…42ca`); twin construction and exact-delta verification reproduce the 102,977-byte candidate DTB (`ffd8204c…90a7`); the local manifest records `authority=none`, `boot_authority=none`, `hardware_acceptance=unproven`, and `execution_state=unbooted`; see the [contract](dual-cell-readonly-telemetry.md) and [clean-twin result](../test-results/2026-08-09-dual-cell-readonly-clean-twin-offline.md) |
| dependency-free physical-key gate | make H4 input acceptance executable on the actual no-Python minimal root | exact guarded POSIX gate covers power, resin/volume-down, and GPIO volume-up identity, one press/release pair each, 2–16 named-IRQ deltas, power/volume-up wake enabled, resin wake absent, and unchanged kernel/systemd/NFS/storage/rollback/USB state; the source oracle now pins both drivers' suspend IRQ arm/disarm callbacks; dynamic hostile fixtures and retained-source/module integration pass offline; no phone, physical-key, indicator, suspend, credential, or boot claim; see the [contract](buttons-indicator.md) and [offline result](../test-results/2026-08-09-physical-keys-runtime-gate-offline.md) |
| VCNL36866 ALS/proximity port oracle | establish the exact first-sensor boundary before writing a 7.1.4 driver | 16 byte-pinned ASUS 5.4 files prove EVB-to-MP5 inheritance, QUPv3 SE0 `0x980000`, address `0x60`, GPIO89 low IRQ, PM8350C L7 3.3 V, little-endian 16-bit values, ID `0xf6`/`0x62`, and raw registers `0xf1`/`0xf4`; accepted Linux 7.1.4 is `port-required`, while synthetic source/DT/runtime hostile cases pin a read-only IIO future candidate; no driver, kernel artifact, phone action, or hardware acceptance; see the [contract](vcnl36866-als-proximity.md) and [offline result](../test-results/2026-08-09-vcnl36866-source-port-contract-offline.md) |
| static thermal-policy oracle | prevent source/config/DTB changes from silently weakening the accepted thermal topology | 43 source checks plus an exact cross-node DT policy pin registered TSENS/PMIC OF drivers, TSENS v2 critical IRQ registration and PDC/GIC routes, step-wise CPU cooling prerequisites, 12 CPU zones with exact trips/maps, five PMIC alarms/zones, and the default orderly/forced shutdown call chain; 39 compatibility and 74 source/DT tests pass hardware-free, including every required thermal source literal and hostile disabled/rewired/duplicate/out-of-range/trip/cooling/PMIC mutations; the accepted PMIC driver remains modular and emergency delay zero, so built-in PMIC enforcement and a profiled 10–30 second forced fallback remain future-only; no phone or credential use and no runtime safety claim; see the [contract](thermal-policy-static-oracle.md) and [offline result](../test-results/2026-07-31-thermal-policy-static-oracle-offline.md) |
...
| observation-only recovery wrapper | provide a machine-enforced read-only pstore observer for a separately reviewed retention experiment | twin 5,371,780-byte initramfses `613d6e3e…70db` contain no fetcher, verifier, trust key, kexec binary, or bundle root; two clean network-disabled ASUS 5.4 builds reproduce 48,400,896-byte Image `efcc4db8…a6ab`, 53,780,480-byte raw boot-v3 `fdcf9b85…a163`, and 100,663,296-byte unsigned AVB `63fc0a1a…43b1`; accepted config `df28224e…578f`, exact 4 MiB ramoops reservation, and sealed source `4c495838…3444` pass; ignored offline evidence only, `authority=none`, `candidate=none`, `boot_authority=none`, retention unproven, never flash |
| target/observer retention admission review | prevent individually valid execution and observer artifacts from being combined under an ambiguous lifecycle | repository-owned profile and read-only verifier jointly bind exact execution candidate `41c23330…b9cf`, runtime manifest `54f53420…6efc`, full wrapper AVB `142f44f4…6ba`, observation-only wrapper AVB `63fc0a1a…43b1`, fixed target/fallback/bootloader/observer sequence, distinct non-nested ignored evidence roots, exact generic claim-consumer state, and zero temporary-boot `allow` rows; hostile fixtures reject role, schema, signature, archive, derivation, boot-v3, AVB, path-race, alias, inventory, and claim mutations; both roles remain unissued, physical retention unproven, missing pstore inconclusive, and recommendation `HOLD`; see the [offline review](../test-results/2026-08-09-retention-cycle-two-identity-review-offline.md) |
...
`rebuild-headless-network-root-initramfs.sh` defaults to `normal` mode and
reconstructs the frozen reporter-free archive from the five required files at
pinned commit `27a270f2955c57f61e2cb8aeae0be23b31223499`, so later shared-init
changes cannot rewrite historical identity. Pass `diagnostic` as its third
argument to twin-build the current verifier, sealed early-target reporter, and
...
The complete disposable clean-twin wrapper and bundle proof is recorded in the
