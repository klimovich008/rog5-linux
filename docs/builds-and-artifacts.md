...
| headless battery-series oracle | define the sustained read-only evidence needed before H3 charger/battery hardware acceptance | fixed `headless-ssh-network-root-v3` collector records 21 samples over 10 minutes for one explicit unplugged, USB-online, or wireless-online phase; it requires exact SM8350 battery/USB/wireless supplies, read-only telemetry and input-current-limit files, zero charge thresholds, and zero Type-C control devices; the host verifier binds source/candidate/boot identity, canonical record metadata and schedule, then compares unplugged/USB status and median current while deriving either sign convention; 11 hostile groups plus 34 compatibility-oracle tests and 53 source/DT tests (1 optional skip) pass hardware-free; no phone, control write, dual-cell interpretation, or charging-safety claim; see the [contract](battery-telemetry-series.md) and [offline result](../test-results/2026-07-31-headless-battery-series-offline.md) |
