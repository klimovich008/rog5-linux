...
| GPUCC plus Adreno SMMU network-root candidate | register only the idle SMMU before any GPU/GMU consumer | v18 stopped before probe on a verifier false positive; v19 passed corrected baseline and GPUCC but safely rejected a no-bind SMMU result, then watchdog fallback/cleanup passed; v20 exact-device deferred/supplier and platform-reprobe work is offline-only; never flash |
