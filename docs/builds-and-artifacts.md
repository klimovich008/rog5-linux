...
| diagnostic AVB generation issuer | avoid recompiling a byte-identical stable recovery after a consumed host-only lifecycle | generation zero exactly reproduces consumed AVB `f710bbcd…97b0ef`; generation one changed only the hash descriptor salt/digest and yielded twin wrapper `332889a8…b51830` over unchanged raw recovery `2f460aa0…628a01`; its one live boot reached signed-bundle `PREPARED` and a commit claim, then exposed a host-only NFS-policy omission before target evidence; exact fallback passed and generation one is consumed; the issuer's atomic publication, exact output inventory, normalized descriptor equivalence, prefix/geometry/digest checks, and distinct-generation tests remain the path to a future generation two after the host fix is reviewed and green; AVB remains algorithm `NONE`, so trust is the pinned host identity plus embedded production Ed25519 root, not the generation nonce; see the [offline result](../test-results/2026-08-02-listener-successor-avb-generation-offline.md) and [live result](../test-results/2026-08-02-diagnostic-nfs-handoff-bypass-live.md) |
