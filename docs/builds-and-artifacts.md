...
| diagnostic recovery generations | preserve one-shot evidence while correcting the recovery transport | generation zero `f710bbcd…97b0ef`, generation one `332889a8…b51830`, and generation two `70fd77f7…fc72b1` are consumed; generation two returned correlated `PREPARED` without a completed host transfer, the NFS gate prevented COMMIT, no target ran, and exact fallback passed. The fresh generation-three production twins enforce fatal `/run` tmpfs validation, fresh-fetch-only PREPARE, corrected lifecycle causality, and continuously stabilized cleanup/deferred-profile proof. AVB `eb514a57…d77b6`, raw wrapper `f1a7c5ad…6a4ce`, recovery initramfs `144f1cfd…e4ec`, and fresh fetcher `77eff28d…e800` pass exact phone-free artifact preflight after 41 lifecycle tests, local CI, Claude review, and GitHub run `30750260056` at `1af3275`. The immutable offline profile remains non-live; a distinct exact live profile now binds the lifecycle, and generation three is the sole one-cycle RAM-only boot admission pending connected preflight. AVB remains algorithm `NONE`, so trust is the pinned host identity plus embedded production Ed25519 root; see the [generation-1 result](../test-results/2026-08-02-listener-successor-avb-generation-offline.md), [NFS-bypass result](../test-results/2026-08-02-diagnostic-nfs-handoff-bypass-live.md), [generation-2 live result](../test-results/2026-08-02-generation-2-fresh-fetch-gap-live.md), [generation-3 production build](../test-results/2026-08-02-generation-3-fresh-fetch-production-build.md), and [generation-3 admission](../test-results/2026-08-02-generation-3-live-admission-offline.md) |
