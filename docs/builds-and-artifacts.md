...
| GPUCC/CCF network-root diagnostic | trace the SM8350 GPU clock-controller with every consumer disabled | v12 live completes the GPUCC orphan and stops inside `__clk_init_parent()` for a DISPCC orphan; v13 passes duplicate source/kernel/wrapper/package gates and separately brackets the callback/cache lookup but remains unbooted; normal coldplug remains rejected and this is never a flash target |
