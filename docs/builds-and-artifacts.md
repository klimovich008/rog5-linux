...
| early-target diagnostic successor | distinguish post-NCM failure boundaries without adding a target command channel | the exact 67,288-byte write-only AArch64 reporter and 6,010,870-byte diagnostic initramfs reproduce under the qualified rootless ARM64 builder; `headless-netroot-early-diag-v1` binds them to the accepted Linux 7.1.4 Image, corrected DTB, and 37,735-entry sealed Arch lower; two disposable-signed bundles, stable-recovery initramfses, clean ASUS 5.4 wrapper Images, header-v3 raw images, and test-only AVB images are byte-identical, pass native verification, and retain `authority=none`; the disposable private key is destroyed, no phone or credential was used, complete local CI passes, and independent packaging review reports no actionable findings; a separate exact admission/lifecycle mode now starts the receive-only collector before recovery control readiness, refuses before COMMIT without one exact readiness line and pre-handoff liveness, independently binds canonical evidence to candidate/anchor/target boot, skips target SSH acceptance, rejects fallback boot-ID reuse, preserves rejected evidence, and resolves only after exact fallback; 16 admission and 34 lifecycle tests pass, complete local CI is green, and independent lifecycle closure review reports no remaining findings; a credential-free no-replace preparer, fixed-ID pre-key signing admission, guarded diagnostic builder, and exact diagnostic recovery-artifact profile now pass focused tests and complete local CI, while direct boot rejects both consumed normal manifests; the production candidate is signed and installed, but its first stable-recovery boot rejected before bundle transfer when graphical PolicyKit exceeded the host readiness window; no COMMIT occurred and exact automatic fallback passed. The replacement fixed host socket passes hostile tests, local/GitHub CI, real SteamOS installation, exact hash/read-only checks, and prompt-free deployment-root preflight at commit `aa39503`; the candidate remains unexecuted pending one new admitted lifecycle. See the [package result](../test-results/2026-08-01-early-target-diagnostic-candidate-offline.md), [lifecycle result](../test-results/2026-08-01-early-target-diagnostic-lifecycle-offline.md), [signing-readiness result](../test-results/2026-08-01-early-target-diagnostic-signing-readiness-offline.md), [GitHub CI result](../test-results/2026-08-01-early-target-diagnostic-github-ci.md), [first live result](../test-results/2026-08-01-early-target-diagnostic-host-auth-timeout.md), and [host-control acceptance](../test-results/2026-08-01-steamos-prompt-free-host-control-live.md) |
