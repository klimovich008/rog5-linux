...
| diagnostic recovery generations | preserve one-shot evidence while correcting the recovery transport | generations zero `f710bbcd…97b0ef`, one `332889a8…b51830`, two `70fd77f7…fc72b1`, and three `eb514a57…d77b6` are consumed. Generation 3 enforced fatal `/run` tmpfs validation and fresh-fetch-only PREPARE; its sole cycle reached verified `PREPARED` after fresh fetch, verification, and kexec load, but the host's former 70-second transfer service never emitted its independent completion receipt. NFS did not start, control failed before COMMIT, no target ran, and exact strict-SSH fallback plus clean host state passed. Source now nests the 180-second fetch worker, 190-second recovery supervisor, 195-second host transfer, 205-second privileged watchdog, 220-second lifecycle receipt, 260-second PREPARE, and 320-second control limits. A hardware-free lattice test and a hostile PREPARED/forged-receipt/nonzero-exit case pass; the latter still blocks NFS and COMMIT. Commit `4c2da4b` passed local and GitHub CI, and its exact controller/server sources are installed with matching hashes. Generation 4 `220e8556…270d` was independently reproduced twice over unchanged raw recovery `f1a7c5ad…6a4ce`; its exact generation record, salt, digest, twin identity, semantic delta, and full artifact chain pass offline. Immutable offline and lifecycle-only live profiles pin the same chain; direct boot is refused and all 42 lifecycle tests select generation 4. One exact central-policy row now admits only one connected-preflight-gated RAM-only lifecycle and must be removed after any result; the image must never be flashed. AVB remains algorithm `NONE`, so trust is the pinned host identity plus embedded production Ed25519 root; see the [generation-1 result](../test-results/2026-08-02-listener-successor-avb-generation-offline.md), [NFS-bypass result](../test-results/2026-08-02-diagnostic-nfs-handoff-bypass-live.md), [generation-2 live result](../test-results/2026-08-02-generation-2-fresh-fetch-gap-live.md), [generation-3 production build](../test-results/2026-08-02-generation-3-fresh-fetch-production-build.md), [generation-3 live result](../test-results/2026-08-03-generation-3-transfer-timeout-live.md), [generation-4 offline issuance](../test-results/2026-08-03-generation-4-timeout-lattice-offline.md), [generation-4 live-profile transition](../test-results/2026-08-03-generation-4-live-profile-offline.md), and [generation-4 admission](../test-results/2026-08-03-generation-4-live-admission-offline.md) |
