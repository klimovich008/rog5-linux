...
| locked Arch Plasma rootfs | headless-first target with Plasma/KRDP and browser/network tools | manifest-pinned 2,007,186,653-byte network-root archive passes live headless gates; the [packaging audit](../test-results/2026-07-27-arch-userspace-readiness-offline.md) adds the isolated, resource-bounded `rog5-agent` and fixes a hotspot/dnsmasq systemd cycle; the [2,006,999,039-byte successor](../test-results/2026-07-27-arch-successor-rootfs-offline.md) clean-round-trips 655 current packages and both corrected units but remains outside the manifest/export and unbooted |
