...
| diagnostic AVB generation issuer | avoid recompiling a byte-identical stable recovery after a consumed host-only lifecycle | generation zero exactly reproduces consumed AVB `f710bbcd…97b0ef`; generation one `332889a8…b51830` reached signed-bundle `PREPARED`, exposed the host NFS-policy omission, returned to exact fallback, and is consumed; generation two `70fd77f7…fc72b1` preserved raw recovery `2f460aa0…628a01`, passed artifact and connected preflight, then booted once and returned `PREPARED` without a completed host transfer; the NFS gate prevented COMMIT, no target ran, exact fallback passed, and generation two is consumed; the offline generation-three prerequisite now enforces fatal `/run` tmpfs validation, fresh-fetch-only PREPARE, corrected lifecycle causality, and continuously stabilized cleanup/deferred-profile proof; 41 lifecycle tests and complete local CI pass, while generation three remains blocked on reviewed publication and GitHub CI; AVB remains algorithm `NONE`, so trust is the pinned host identity plus embedded production Ed25519 root, not the generation nonce; see the [generation-1 result](../test-results/2026-08-02-listener-successor-avb-generation-offline.md), [NFS-bypass result](../test-results/2026-08-02-diagnostic-nfs-handoff-bypass-live.md), and [generation-2 live result](../test-results/2026-08-02-generation-2-fresh-fetch-gap-live.md) |
