...
| key-bound headless SSH v2 root | credential-clean successor with exact SSH access identity | twin 536,750,378-byte source archives from commit `9739abe` have SHA-256 `2abe8c…72e6a`; the 536,747,283-byte sorted sealed lower has 37,735 entries and a 731-byte `rog5-headless-network-root-package-v3` manifest; one canonical Ed25519 fingerprint is recomputed from the SSH blob and bound across the root build record, `/root/.ssh/authorized_keys`, complete tree seal, and package; v3 fixes effective `AuthorizedKeysFile` to the bound path while historical v1/v2 formats remain exact; public fixture only, no private/signing key, no phone, unbooted, `authority=none`; see the [offline result](../test-results/2026-07-30-headless-ssh-v2-key-bound-package.md) |
