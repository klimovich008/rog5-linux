...
| A660/GMU registration tier | separate registration from first DRM open before building a live candidate | full dependency graph passes source audit; guarded modular Linux `7.1.4-rog5-a660reg1` kernel and all nine acceptance outputs reproduce byte-for-byte with zero UFS/firmware; four-node DT/runtime bundle and live gates pending |
