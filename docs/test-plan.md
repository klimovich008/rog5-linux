...
- The active hardware-free successor is the
  ...
  write-only ACM service, credential-checked local updates, malformed and
  oversized datagrams, blocked output, fixed cadence, and automatic watchdog
...
- The receive-only recovery NCM progress gate uses four focused suites plus the
  ...
  direct connected action is admitted. After the sole live result, central
  policy contains zero `allow` rows and Generation 11 is exact `consumed`;
  hostile missing-file, missing-row, malformed-header,
  missing/duplicate-artifact-row, duplicate-policy-row, wrong-basis, denied
  status, identity mutation, exact-basis readmission, and trailing-field
  fixtures reject both preflight and boot before host inspection.
  Generation-11 `boot`
  ...
  inspection at its admitted checkpoint; seven hardware-free claim-consumer
