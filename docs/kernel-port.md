...
Recovery v13 implements this reversible two-stage route:
...
4. Both the staging and target initramfs arm a 180-second forced-reboot timer,
   ...
   block device, and expose USB only after that gate passes. USB ACM is the
...
lacked the pre-USB block-device lock. Recovery v13 adds that fail-closed gate,
rebuilds the complete credential-free dependency chain twice, and passes the
expanded offline verifier. It has not been booted, so live ACM,
RAM-backed-root, block read-only state, kexec, and rollback behavior remain
unproven.
