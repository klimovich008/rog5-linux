...
| Next recovery increment | The receive-only [NCM progress channel](docs/recovery-ncm-progress.md) now spans the exact device namespace, privileged broker/controller, root-to-user collector, and post-COMMIT lifecycle assessment. Generation 11 AVB `8472b206…bcf562` passed every offline gate, but its [sole live cycle](test-results/2026-08-04-generation-11-progress-listener-confinement-live.md) failed closed before bundle-server readiness when the host rejected its TCP 8081 collector. A [production-faithful host-only reproduction](test-results/2026-08-04-generation-11-progress-listener-scope-reproduction-offline.md) showed that `SO_BINDTODEVICE` makes `ss` render the listener as `169.254.77.1%interface:8081`; the controller had searched for an unscoped endpoint. It now parses one exact record and requires the scoped endpoint, sole launched `python3` PID/fd owner, a live process, and no IPv6 conflict. All 38 controller cases, complete local CI, installed-host verification, publication, and exact-head CI pass, including Steam-shaped races and hostile lookalikes. Generation 11 remains consumed and never reusable. The next increment is offline issuance and preflight of a distinct diagnostic successor; no successor boot is authorized by this checkpoint. |
