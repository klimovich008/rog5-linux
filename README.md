...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but the latest hardened r2 target lost USB 23 seconds after NCM enumeration and returned safely to fallback. The distinct `headless-netroot-early-diag-v1` successor now passes authority-free twin packaging: its reporter/initramfs, disposable-signed bundle, stable-recovery wrapper, raw image, and test-only AVB image reproduce byte-for-byte, and the disposable private key is destroyed. A sealed 9.6 MB ARM64 runtime executes its exact stage 130/140 units under real Arch systemd in QEMU; the test SSH service proves ordering, not OpenSSH. The receive-only exact-port ACM collector is integrated into the one-shot lifecycle before the non-retryable commit boundary; its focused admission/lifecycle tests and complete local CI pass while the normal r2 path remains unchanged. Independent closure review reports no remaining findings. A separate credential-free preparer and guarded builder now pin the diagnostic candidate before key access, while the live gate selects an exact diagnostic recovery profile and rejects both consumed normal manifests. Its complete disposable-key wrapper/twin-build/native-verification/artifact-preflight path passes at clean checkpoint `0375e97a`, independent signing-readiness review is closed, and GitHub Actions run `30700630487` passes both required jobs at `6654c0c`. Production signing and one phone acceptance cycle remain pending. |
