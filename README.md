...
| Next recovery increment | The receive-only [NCM progress channel](docs/recovery-ncm-progress.md) now spans the exact device namespace, privileged broker/controller, root-to-user collector, and post-COMMIT lifecycle assessment. Missing or invalid evidence remains `authority=NONE` and cannot gate COMMIT; listener conflicts fail closed. Focused suites and the complete local `ci` and provisioned `quick` tiers pass. Publication, installed-host verification, final AArch64/twin-build review, and connected preflight remain; no Generation-11 image exists. |
