...
| Mainline userspace | SSH-only Arch lower exposed target NCM but reset before SSH with historical DTB v1. The credential-clean `headless-ssh-v2` deployment chain now passes exact artifact and connected-fastboot gates. Its 37,735-entry v3 lower is published under the fixed root-owned `/home/rog5-linux` store and passes the sealed-root and NFS host preflights. A sealed 9.6 MB ARM64 runtime now executes the production-generated stage 130/140 units under real Arch systemd in QEMU; its SSH service is an ordering stub, not OpenSSH evidence. A receive-only exact-port ACM collector now preserves bounded validated early-target stages and matching USB events in private evidence, with an exact journal-live handshake for a future diagnostic lifecycle supervisor. Hardware-free tests pass; live use remains pending separate authority and phone acceptance. |
