...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4. Diagnostic generations 0–7 are consumed and never reusable. Generation 7 passed connected preflight and booted once in RAM; recovery received the complete signed bundle, but no `PREPARED` record or target ACM appeared, no commit intent existed, and no target ran. Exact Alpine profile restoration and strict SSH fallback passed. The final cleanup deadline defect is reproduced and fixed offline by reducing one strict host observation from about 5.72 to 1.11 seconds without extending the deadline; the transitional NetworkManager association remains fail-closed and now reports a non-sensitive shape. Central temporary-boot policy has zero active `allow` rows. |
