...
| Storage migration | Phase-1 read-only inventory found 7 UFS LUNs and 109 CRC-valid GPT entries; private source/host hash-matched backups cover 14 GPT ranges and all 107 non-`super`, non-`userdata` partitions, and all seven GPT layouts pass offline restoration rehearsal. Generation 22 was consumed: recovery committed the target, no target USB appeared, exact Alpine returned after a 25.255-second USB blackout, and mounted pstore was empty but inconclusive. Generation 23 moves exact NCM setup before UFS discovery so the next RAM-only cycle can separate pre-initramfs failure from a bounded UFS gate. Secondary subsystems remain frozen |
