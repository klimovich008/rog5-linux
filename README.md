...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but hardened r2 lost USB 23 seconds after NCM enumeration and returned safely to fallback. The diagnostic chain then exposed pre-target recovery gaps: PolicyKit timing, stale bundle inventory, a fetcher profile mismatch, and—after those fixes—a duplicated root-controller check that rejected Steam's unrelated loopback TCP 8080 listener before transfer. No bundle response, `PREPARE`, intent, NFS, `COMMIT_EXEC`, or target boot occurred; watchdog fallback and strict pinned Alpine SSH passed. Wrapper `f710bbcd…97b0ef` is consumed and denied. The scoped listener fix passes review/CI and is installed byte-exact; generation-1 AVB successor `332889a8…b51830` passes the complete artifact gate over byte-identical recovery payload bytes. The target remains unexecuted; publication CI and connected preflight remain before one RAM-only lifecycle. |
