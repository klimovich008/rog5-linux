...
| Next recovery increment | The [Generation-12 live result](test-results/2026-08-04-generation-12-nfs-mount-disconnect-live.md) isolates failure to the first NFS mount or a lower kernel/USB/network boundary without claiming an unproven panic. Generation 12 is consumed and production diagnostic actions are held before credentials or phone inspection. The published host-only successor contract adds pre/post-syscall stages, exact host NFS RPC plus target-specific TCP flow/queue/current-unrecovered-RTO evidence, same-port USB/NCM continuity, and current-cycle postmortem lineage; implementation commit `eeb157b` passed exact-head GitHub Actions run `30988099391`. No successor candidate or boot authority exists, and issuance remains HOLD. |
