...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but the latest hardened r2 target lost USB 23 seconds after NCM enumeration and returned safely to fallback. The distinct production-signed `headless-netroot-early-diag-v1` successor has a complete twin build and passes QEMU plus native artifact/lifecycle gates. Its first stable-recovery boot rejected before transfer when graphical PolicyKit exceeded the host-ready window; the prompt-free replacement then exposed a second pre-transfer gap because the active bundle root still contained consumed r2. Both attempts created no `COMMIT_EXEC`, automatic fallback and strict signed proof passed, and the diagnostic candidate remains unexecuted. The consumed bundle is recoverably archived; the same descriptor validator now checks sole-root inventory during preflight without a listener, and the published/reinstalled remediation passed real bundle and 37,735-entry NFS preflights with no residue. One separate new lifecycle is admitted. |
