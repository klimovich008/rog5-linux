...
| Next recovery increment | A separate receive-only [NCM progress channel](docs/recovery-ncm-progress.md) now passes hostile host and native-responder tests, including the Generation-10 ACM-loss shape and every wire truncation. It is advisory and cannot authorize COMMIT. Privileged broker/lifecycle integration and exact AArch64/CI/review gates remain; no Generation-11 image exists. |
