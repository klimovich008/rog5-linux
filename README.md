...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but hardened r2 lost USB after NCM and returned safely to fallback. The diagnostic chain eliminated recovery fetch and host-listener defects, then generation-1 recovery `332889a8…b51830` fetched/verified the exact bundle and claimed one commit. A host policy omission let the diagnostic commit precede NFS startup, so no target frame arrived; same-port signed Alpine fallback passed and the intent is resolved `FALLBACK_RETURNED`. The wrapper is consumed. The fail-closed diagnostic NFS-gate regression now passes 20 control and 39 lifecycle tests; reviewed publication, complete CI, and a distinct generation-2 AVB identity are required before another RAM-only cycle. |
...
evidence. No temporary recovery is currently admitted; every diagnostic image
used so far is consumed. No image may be flashed.
