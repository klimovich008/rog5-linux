...
| Storage migration | Phase-1 read-only inventory found 7 UFS LUNs and 109 CRC-valid GPT entries; private source/host hash-matched backups cover 14 GPT ranges and all 107 non-`super`, non-`userdata` partitions, and all seven GPT layouts pass offline restoration rehearsal. Generations 22–25 are consumed after no target USB and exact fallback. Generation 26 is the RAM-only UFS-specific memory-ownership discriminator: it restores the historical RMTFS reservation and omits ramoops only from persistent-root boots while retaining read-only storage and recovery. Secondary subsystems remain frozen |
