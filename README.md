...
| Storage migration | Phase-1 read-only inventory found 7 UFS LUNs and 109 CRC-valid GPT entries; private source/host hash-matched backups cover 14 GPT ranges and all 107 non-`super`, non-`userdata` partitions, and all seven GPT layouts pass offline restoration rehearsal. Generations 22–26 are consumed after no target USB and exact fallback. Generation 27 then proved the persistent initramfs reaches stable target NCM with Generation 20's exact Image/DTB and no storage access. Generation 28 cross-pairs that Image with the UFS-enabled persistent DTB before UFS. Secondary subsystems remain frozen |
