...
| Next recovery increment | The receive-only [NCM progress channel](docs/recovery-ncm-progress.md) now spans the exact device namespace, privileged broker/controller, root-to-user collector, and post-COMMIT lifecycle assessment. Generation 11 remains consumed after its sole live cycle exposed and motivated the corrected host TCP-8081 confinement check. A [production-faithful host-only reproduction](test-results/2026-08-04-generation-11-progress-listener-scope-reproduction-offline.md) and all local/installed/GitHub gates pass. The distinct [Generation-12 offline successor](test-results/2026-08-04-generation-12-host-confinement-successor-offline.md) is AVB `615d7498…d72cf6` over the byte-identical accepted raw recovery, kernel, and NCM-capable initramfs. Its retained local deterministic twins and immutable offline profile pass, while central policy stays at zero `allow` rows and no live profile or lifecycle selector exists. Independent review and complete local CI pass; publication and clean-checkout exact-head CI come before a separate live-profile/connected-preflight increment. No Generation-12 phone boot is authorized. |
