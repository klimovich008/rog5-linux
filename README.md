...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but hardened r2 lost USB after NCM and returned safely to fallback. Diagnostic generations 0–4 are consumed. Generation 4 reached verified recovery ACM/NCM with rollback armed; its collector and one-transfer bundle service became ready, but the service never emitted its completion marker before the 45-second NFS-readiness deadline. NFS never started, COMMIT was never sent, and no target ran. The exact stall and early-failure fallback cleanup now pass hardware-free regression; corrected host components and real bundle/NFS preflights pass. Distinct Generation-5 AVB `abe4501f…beb1a` is twin-reproducible and passes the complete offline artifact gate over the unchanged raw recovery. The lifecycle selects its exact live-capable profile, and central policy now admits at most one connected-preflight-gated RAM-only cycle. It has not been connected, booted, or consumed. |
