...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but hardened r2 lost USB after NCM and returned safely to fallback. Diagnostic generations 0–4 are consumed and no image is admitted. Generation 4 reached verified recovery ACM/NCM with rollback armed; its collector and one-transfer bundle service became ready, but the service never emitted its completion marker before the 45-second NFS-readiness deadline. NFS never started, COMMIT was never sent, and no target ran. The phone returned automatically to Alpine; after the 205-second host watchdog, fixed anchored profile restoration and strict fallback preflight passed with no project server/export residue. The exact stall and early-failure fallback cleanup now pass hardware-free regression, PREPARED and artifact progress are observable, and the real server/fetcher pair transfers the Generation-4-sized artifacts. Complete local CI and GitHub Actions run `30793088424` pass at implementation commit `38b6019`; no successor is admitted. |
