...
| Storage migration | Phase-1 read-only inventory found 7 UFS LUNs and 109 CRC-valid GPT entries; private source/host hash-matched backups cover 14 GPT ranges and all 107 non-`super`, non-`userdata` partitions, and all seven GPT layouts pass offline restoration rehearsal. Generations 22–24 are consumed after no target USB and exact fallback. Generation 25 is the unbooted RAM-only discriminator: it keeps the Generation-24 DTB/initramfs and changes the target to a clean-twin kernel with the exact historically accepted UFS source/config/release and read-only guards. Secondary subsystems remain frozen |
