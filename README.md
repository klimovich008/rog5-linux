...
| Storage migration | Phase-1 read-only inventory found 7 UFS LUNs and 109 CRC-valid GPT entries; private source/host hash-matched backups cover 14 GPT ranges and all 107 non-`super`, non-`userdata` partitions, and all seven GPT layouts pass offline restoration rehearsal. Generations 22 and 23 are consumed after no target USB and exact fallback; Generation 23's 25.330-second blackout minus an 18.162-second Alpine reboot baseline points strongly, but not conclusively, to its five-second pre-USB command-line gate. Generation 24 keeps the exact Image/DTB and moves NCM before command-line/release validation and all userspace UFS work. Secondary subsystems remain frozen |
