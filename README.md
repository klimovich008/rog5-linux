...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4. Diagnostic generations 0–6 are consumed and never reusable. Generation 6 passed connected preflight and booted once in RAM; recovery transferred the complete signed bundle, but control produced no output or `PREPARED` record. Independently, the collector expired with zero target frames. No COMMIT intent existed and no target ran. Alpine restoration and strict SSH passed. Two host-verifier defects are now reproduced and corrected offline: exact Alpine udev-model classification, and NetworkManager's retained exact UUID on an address-free unmanaged deferred interface. Private timestamps and the lifecycle order prove the second false rejection made the host terminate control before it waited for `PREPARED`; Generation 6 therefore did not establish recovery-side silence. No live `PREPARED` or successor acceptance is claimed yet. |
...
evidence. Diagnostic generations 0–6 are consumed and there is no `allow`
row. No image may be booted or flashed.
