...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but the latest hardened r2 target lost USB 23 seconds after NCM enumeration and returned safely to fallback. The distinct production-signed `headless-netroot-early-diag-v1` successor has a complete twin build and passes QEMU plus native artifact/lifecycle gates. Its first stable-recovery boot rejected before bundle transfer when graphical PolicyKit exceeded the host-ready window; no `COMMIT_EXEC` occurred, the diagnostic candidate remains unexecuted, and the independent 180-second watchdog plus strict signed fallback proof passed. The replacement mode-`0600` operator socket and fixed root broker now pass hostile tests, complete local/GitHub CI, real SteamOS installation, and a prompt-free 37,735-entry deployment-root preflight. One new admitted diagnostic lifecycle remains. |
