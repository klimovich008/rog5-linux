...
| Storage migration | Phase-1 read-only inventory found 7 UFS LUNs and 109 CRC-valid GPT entries; private source/host hash-matched backups cover 14 GPT ranges and all 107 non-`super`, non-`userdata` partitions, and all seven GPT layouts pass offline restoration rehearsal. Generations 27–29 prove the persistent initramfs, UFS-enabled DTB, and rebuilt Image each reach stable target NCM when active UFS binding is absent. Reconstructed builds do not reproduce the original accepted Image, so Generation 30 uses the retained hash-verified clean-twin persistent-root Image with the current DTB and USB-first read-only initramfs. Secondary subsystems remain frozen |
...
| Persistent Arch root | WW33 restoration replaced the former Linux ext4 `userdata` with metadata-encrypted Android F2FS. The owner confirmed a no-GPT ext4 reset of only partition 23; the formatter is being sealed before one-use admission. Next: recreate and RAM-boot one bounded 16-GiB Arch image |
