...
| Storage migration | Phase-1 read-only inventory found 7 UFS LUNs and 109 CRC-valid GPT entries; private source/host hash-matched backups cover 14 GPT ranges and all 107 non-`super`, non-`userdata` partitions, and all seven GPT layouts pass offline restoration rehearsal. Generations 27 and 28 proved the persistent initramfs and UFS-enabled DTB reach stable target NCM with the live-proven Image and no storage access, isolating the remaining early regression to the UFS Image/config lineage. Generation 29 tests that Image with UFS disabled in DT. Secondary subsystems remain frozen |
