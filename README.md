...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4, but hardened r2 lost USB 23 seconds after NCM enumeration and returned safely to fallback. The distinct production-signed `headless-netroot-early-diag-v1` successor then exposed three recovery-path gaps before target execution: graphical PolicyKit timing, stale bundle-root inventory, and finally a fetcher-only diagnostic-profile policy mismatch after manifest transfer. Every attempt created no `COMMIT_EXEC`; exact watchdog fallback passed, and the candidate remains unexecuted. The fetch-policy correction passes native/root-isolated/AArch64 tests, a disposable full-path replay, and a fresh production-signed twin build. Exact AVB wrapper `f710bbcd…97b0ef` embeds fetcher `f410ca87…b5d13d`, passes the real artifact gate, and is admitted for one RAM-only boot. The bounded fallback-profile correction now passes exact-port, duplicate, detach, timeout, partial-rollback, socket, and lifecycle fault suites plus full local CI. The old `9c060a27…204ef` wrapper remains consumed and denied; reviewed host installation and connected preflight are next. |
...
evidence and admits one exact single-use diagnostic recovery, which remains
gated by installation and connected preflight. Neither image may be flashed.
