...
| Mainline userspace | Native Arch/systemd/NFS/OverlayFS/NCM/SSH passed twice on Linux 7.1.4. Diagnostic generations 0–5 are consumed and never reusable. Generation 5 transferred the complete signed bundle but inherited a blocked TERM mask, so its completion receipt could not gate NFS before COMMIT; no target ran and fallback passed. The corrected broker is installed and its hardware-free choreography tests pass. Distinct Generation-6 AVB `6aa47517…d398` is twin-reproduced over the unchanged accepted recovery, the lifecycle selects its exact live-capable profile, and central policy admits exactly one connected-preflight-gated RAM-only cycle. Generation 6 remains unbooted and unconsumed. |
...
evidence. Generations 0–5 are consumed. Exactly one Generation-6 image is
admitted for one connected-preflight-gated RAM-only lifecycle; it may never be
flashed, and its row must be removed after any result.
