...
awk -F '\t' \
	...
	$1 == "build/persistent-root-power-usb-v9-generation102-20260823-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact first read-only local-image Arch boot after verified userdata-only sparse staging; unchanged live-proven V8 charging/UFS/local-root target under fresh signed bundle; RAM-only kernel/recovery; externally consumed exact claim required; never flash or retry after entry" &&
