...
awk -F '\t' \
	...
	$1 == "build/persistent-root-power-usb-v10-generation103-20260823-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact bounded local-image probe creation and first Arch boot after verified userdata sparse staging; unchanged live-proven charging/UFS/local-root target with local-write/current probe mode only; RAM-only kernel/recovery; externally consumed exact claim required; never flash or retry after entry" &&
