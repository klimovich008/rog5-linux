...
awk -F '\t' \
	...
	$1 == "build/persistent-root-local-v51-generation160-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 160 read-only boot of the staged-seal 16 GiB local Arch image with exact ae717 reboot-mode modules; userdata and image ro-noload, OverlayFS/systemd, power/USB, key-only SSH, systemd timing, controlled reboot, exact fastboot fallback; RAM-only; never flash or retry after entry" &&
