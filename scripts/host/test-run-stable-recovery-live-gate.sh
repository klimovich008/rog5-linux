...
awk -F '\t' '
	...
	$1 == "build/mainline-udc-nfs-xattr-generation10-wrapper-20260811-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact NFS-xattr-projected retention execution recovery; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { execution++ ; next }
...
grep -Fq '"retention-host-rendezvous-v12-nfs-xattr-execution-v1"' "$lifecycle" ||
