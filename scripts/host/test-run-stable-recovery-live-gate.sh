...
awk -F '\t' '
	...
	$1 == "build/persistent-root-local-image-post-write-v43-generation65-20260814-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact read-only post-write local-image Arch cycle using the previously accepted read-only UFS kernel; both ext4 layers remain ro,noload, no write window is entered, and the persisted Generation 64 marker must match its exact pinned writer boot while differing from the current boot; RAM-only kernel/recovery; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { generation65++ ; next }
...
[[ $(awk -F '\t' '$2 == "allow" { count++ } END { print count + 0 }' \
	"$boot_policy") == 3 ]] ||
	{ echo 'FAIL temporary-boot policy does not contain exactly observer, core, and Generation 65' >&2; exit 1; }
