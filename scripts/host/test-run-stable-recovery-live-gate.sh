...
grep -Fq "expected_boot_role='unbooted Generation 122 explicit-sysfs full staging successor; no second USB mdev scan, unchanged Image/DTB/power-USB/UFS/one-file installer and fastboot rollback; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-explicit-v13-generation122-20260824-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 122 full staging cycle using only initial devtmpfs mdev plus explicit UDC/usb0 polling; unchanged Image/DTB/address/power-USB/UFS/installer, one exact userdata image path, key-only SSH, RAM-only; never flash or retry after entry" &&
