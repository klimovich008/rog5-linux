...
grep -Fq "expected_boot_role='unbooted Generation 147 target-only installer glob fix; proven kernel, UFS, NCM, key-only SSH, exact image transfer, bounded userdata image, relock, fastboot; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-globfix-v38-generation147-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 147 target-only staging cycle removing installer set -f and emitting bounded failure output before reboot; proven kernel, DTB, modules, recovery raw bytes, one image-file scope, relock, and slot-A fallback unchanged; RAM-only; never flash or retry after entry" &&
