...
grep -Fq "expected_boot_role='unbooted Generation 139 power/USB-before-UFS baseline; unchanged ae717 Image/DT/modules, minimal reporter, built-in reboot mode; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/ufs-power-reboot-baseline-v30-generation139-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 139 UFS baseline restoring the Generation-109-live-proven power/USB loader before the unchanged ae717 UFS module chain; validates charging, temperature, UCSI sink/device role, NCM, UFS, and built-in fastboot return; no SSH, mount, installer, or storage-write path; RAM-only; never flash or retry after entry" &&
