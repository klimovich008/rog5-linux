...
awk -F '\t' \
	...
	$1 == "build/persistent-root-power-usb-v1-generation77-20260821-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact RAM-only combined-stack cycle proving current stable recovery, side-port NCM and charging, deferred read-only UFS, local Arch image, key-only SSH, and exact stock slot-A fallback; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { generation77++ ; next }
