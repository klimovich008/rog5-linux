...
grep -Fq "expected_boot_role='unbooted Generation 151 crash-bounded partial classifier plus direct-versus-buffered 32 MiB UFS benchmark; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-write-benchmark-v42-generation151-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 151 cycle accepting only absent or one root-owned mode-0600/0644 partial no larger than 825884672 bytes, then comparing one 32 MiB aligned direct write with one 32 MiB buffered write; disposable files removed on success; sync-independent fallback; RAM-only; never flash or retry after entry" &&
