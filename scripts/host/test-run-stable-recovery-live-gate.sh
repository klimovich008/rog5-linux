...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-native-verify-v1-generation221-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 221 read-only grown-p24 geometry, UUID, clean-state, seal and boot-critical verification cycle; no storage writes, RAM-only, never flash or retry after COMMIT" &&
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-verify-v1-generation221-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 221 read-only native-root verifier; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
