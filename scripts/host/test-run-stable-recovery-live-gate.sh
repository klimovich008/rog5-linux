...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-mainline-clone-v1-generation194-20260826-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 194 allocated-block clone from the verified p23 Arch image into p24; verify source hash, power, thermal, exact geometry, UUID, grown tree seal, read-only relock, and exact slot-A fastboot return; RAM-only wrapper, never flash or retry after COMMIT" &&
...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-mainline-clone-v1-generation194-20260826-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 194 allocated-block clone from the verified p23 Arch image into p24; verify source hash, power, thermal, exact geometry, UUID, grown tree seal, read-only relock, and exact slot-A fastboot return; RAM-only wrapper, never flash or retry after COMMIT" && NF == 3 {
		stage2_clone++
	}
	END { exit allowed == 2 + power_usb && power_usb <= 1 && stage2_clone == 1 ? 0 : 1 }
' "$boot_policy" ||
...
grep -Fq "expected_boot_basis='one exact Generation 194 allocated-block clone from the verified p23 Arch image into p24; verify source hash, power, thermal, exact geometry, UUID, grown tree seal, read-only relock, and exact slot-A fastboot return; RAM-only wrapper, never flash or retry after COMMIT'" "$gate" ||
