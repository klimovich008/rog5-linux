...
grep -Fq "expected_boot_role='unbooted Generation 144 writer; immediate target activation after canonical bundle cleanup, proven UFS/SSH, bounded installer, relock, fastboot; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-fast-v35-generation144-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 144 bounded staging cycle activating target networking immediately after the bundle server has completed canonical cleanup, without a redundant post-COMMIT cleanup wait; final cleanup remains mandatory; otherwise identical proven writer, one image file, relock, fastboot; RAM-only; never flash or retry after entry" &&
