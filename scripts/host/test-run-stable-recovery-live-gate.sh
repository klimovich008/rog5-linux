...
awk -F '\t' \
	...
	END { exit allowed == 2 + power_usb && power_usb <= 1 && stage2_clone_v2 == 1 ? 0 : 1 }
' "$boot_policy" ||
...
grep -Fq "expected_boot_basis='one exact Generation 196 p24 clone from the verified sealed p23 source into Generation-195-proven zero/non-ext4 p24; read-only loop/tree source validation, 30-second APSS watchdog, allocated-block restore, UUID/grow/seal/relock, and exact slot-A fastboot return; RAM-only, never flash or retry after COMMIT'" "$gate" ||
