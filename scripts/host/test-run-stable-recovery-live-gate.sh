...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-softdog-probe-v1-generation207-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed successful Generation 207 softdog proof; exact armed stage then automatic slot-A fastboot in 23.6 seconds, TARGET_ACCEPTED, no UFS or storage access; never retry or flash" &&
		NF == 3 { stage2_softdog++ ; next }
	$1 == "build/storage-layout-stage2-softdog-clone-v1-generation208-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
	...
	$1 == "build/storage-layout-stage2-native-postmortem-v2-generation210-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 210 read-only p24 postmortem; ext4 magic present but dumpe2fs failed, proving partial/corrupt clone; exact fastboot and FALLBACK_RETURNED passed; never retry or flash" &&
		NF == 3 { stage2_postmortem_v2++ ; next }
	$1 == "build/storage-layout-stage2-softdog-direct-clone-v1-generation211-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 211 direct p24 clone timeout; source admission and softdog passed, no post-arm stage before 840-second fallback, p24 partial/unknown; never retry or flash" &&
		NF == 3 { stage2_direct_clone++ ; next }
	$1 == "build/storage-layout-stage2-direct-chunk1-v1-generation212-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 212 chunk timeout; extents 1-17 passed, extent 18 began without PASS before the 840-second softdog fallback; p24 remains partial; never retry or flash" &&
		NF == 3 { stage2_chunk1++ ; next }
	$1 == "build/storage-layout-stage2-direct-extent18-v1-generation213-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
	...
	$1 == "build/storage-layout-stage2-direct-extent20-seg1-v1-generation215-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed successful Generation 215 extent-20 segment-1 overwrite; exact PASS, sync, relock, softdog disarm, TARGET_ACCEPTED and slot-A fastboot in 480.461 seconds; never retry or flash" &&
		NF == 3 { stage2_extent20_seg1++ ; next }
	$1 == "build/storage-layout-stage2-direct-extent20-seg2-v1-generation216-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 216 extent-20 segment-2 stall; BEGIN emitted without PASS, NCM failed, exact gadget remained enumerated, softdog and fallback proof failed; p24 subrange partial/unknown; never retry or flash" &&
		NF == 3 { stage2_extent20_seg2++ ; next }
	$1 == "build/storage-layout-stage2-direct-extent20-seg2a-v1-generation217-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed successful Generation 217 extent-20 segment-2A overwrite; exact PASS, sync, relock, softdog disarm, TARGET_ACCEPTED and slot-A fastboot in 296.065 seconds; never retry or flash" &&
		NF == 3 { stage2_extent20_seg2a++ ; next }
	$1 == "build/storage-layout-stage2-direct-extent20-seg2b-v1-generation218-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed successful Generation 218 extent-20 segment-2B overwrite; exact PASS, sync, relock, softdog disarm, TARGET_ACCEPTED and slot-A fastboot in 294.734 seconds; never retry or flash" &&
		NF == 3 { stage2_extent20_seg2b++ ; next }
	$1 == "build/storage-layout-stage2-direct-extent20-seg3a-v1-generation219-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 219 extent-20 segment-3A hard stall; BEGIN without PASS, NCM unreachable with configured gadget and TX watchdogs, softdog/emergency/fallback failed, p24 partial/unknown; never retry or flash" &&
		NF == 3 { stage2_extent20_seg3a++ ; next }
	$1 == "build/storage-layout-stage2-native-progress-v1-generation220-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed successful Generation 220 read-only segment-3A progress cycle; exact source SHA, 16384-block p24 prefix, zero mismatch at 1480465, empty pstore inconclusive, TARGET_ACCEPTED and slot-A fastboot in 439.436 seconds; never retry or flash" &&
		NF == 3 { stage2_native_progress++ ; next }
	$1 == "build/storage-layout-stage2-native-verify-v1-generation221-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 221 read-only grown-p24 verifier; geometry, UUID, label and clean state passed, generic target-tree failure returned exact slot-A fastboot; never retry or flash" &&
		NF == 3 { stage2_native_verify++ ; next }
	$1 == "build/storage-layout-stage2-native-tree-detail-v1-generation222-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 222 read-only per-item classifier; only sshd and ssh-keygen content hashes differed, metadata and all other boot-critical files passed, TARGET_ACCEPTED and exact slot-A fastboot returned; never retry or flash" &&
		NF == 3 { stage2_native_tree_detail++ ; next }
	$1 == "build/storage-layout-stage2-native-ssh-repair-v1-generation223-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 223 exact-two-file repair; old hashes, write and in-mount new hashes passed, then post-unmount filesystem-clean assertion failed before terminal success; exact slot-A fallback returned, final state requires read-only proof; never retry or flash" &&
		NF == 3 { stage2_native_ssh_repair++ ; next }
	$1 == "build/storage-layout-stage2-native-postrepair-verify-v1-generation224-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
	...
	$1 == "build/persistent-native-root-v1-generation226-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 226 RAM-only native-p24 cycle; root verification, OverlayFS, runtime and switch-root passed, final sshd host key appeared, but every authenticated SSH attempt returned 255 until watchdog fallback; exact slot-A recovery and cleanup passed, no storage write; never retry or flash" &&
		NF == 3 { persistent_native_root++ ; next }
	$1 == "build/persistent-native-root-v2-generation227-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 227 native-p24 cycle; switch-root and host-key pin passed, 127 SSH attempts returned 255, exact offline AArch64 controls disproved the locked-root diagnosis, and slot-A recovery fallback passed; never retry or flash" &&
		NF == 3 { persistent_native_root_v2++ ; next }
	$1 == "build/persistent-native-root-v3-generation228-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
	...
	$1 == "build/persistent-native-root-v5-generation230-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
		$3 == "consumed Generation 230 postrepair native-p24 cycle; p24/root/OverlayFS/switch-root, NCM, first key auth and UFS passed; runtime raced the not-yet-present ready marker, later snapshot proved systemd running with zero failed units; exact slot-A fastboot and FALLBACK_RETURNED passed; never retry or flash" &&
		NF == 3 { persistent_native_root_v5++ ; next }
	$1 == "build/persistent-native-root-v6-generation231-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "revoked" &&
	...
	$1 == "build/persistent-native-root-v8-generation233-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 233 RAM-only repeat of the successful native-p24/systemd/key-SSH/UFS/NCM/full-readiness/direct-fastboot baseline; unchanged target and raw recovery; never flash or retry after COMMIT" &&
		NF == 3 { persistent_native_root_v8++ ; next }
...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v8-generation233-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 233 RAM-only repeat of the successful native-p24/systemd/key-SSH/UFS/NCM/full-readiness/direct-fastboot baseline; unchanged target and raw recovery; never flash or retry after COMMIT" && NF == 3 {
		native_root++
	}
...
grep -Fq "expected_boot_basis='consumed successful Generation 207 softdog proof; exact armed stage then automatic slot-A fastboot in 23.6 seconds, TARGET_ACCEPTED, no UFS or storage access; never retry or flash'" "$gate" ||
...
grep -Fq "expected_boot_basis='consumed Generation 210 read-only p24 postmortem; ext4 magic present but dumpe2fs failed, proving partial/corrupt clone; exact fastboot and FALLBACK_RETURNED passed; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 210 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='consumed Generation 211 direct p24 clone timeout; source admission and softdog passed, no post-arm stage before 840-second fallback, p24 partial/unknown; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 211 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='consumed Generation 212 chunk timeout; extents 1-17 passed, extent 18 began without PASS before the 840-second softdog fallback; p24 remains partial; never retry or flash'" "$gate" ||
...
grep -Fq "expected_boot_basis='consumed successful Generation 215 extent-20 segment-1 overwrite; exact PASS, sync, relock, softdog disarm, TARGET_ACCEPTED and slot-A fastboot in 480.461 seconds; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 215 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='consumed Generation 216 extent-20 segment-2 stall; BEGIN emitted without PASS, NCM failed, exact gadget remained enumerated, softdog and fallback proof failed; p24 subrange partial/unknown; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 216 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='consumed successful Generation 217 extent-20 segment-2A overwrite; exact PASS, sync, relock, softdog disarm, TARGET_ACCEPTED and slot-A fastboot in 296.065 seconds; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 217 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='consumed successful Generation 218 extent-20 segment-2B overwrite; exact PASS, sync, relock, softdog disarm, TARGET_ACCEPTED and slot-A fastboot in 294.734 seconds; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 218 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='consumed Generation 219 extent-20 segment-3A hard stall; BEGIN without PASS, NCM unreachable with configured gadget and TX watchdogs, softdog/emergency/fallback failed, p24 partial/unknown; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 219 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='consumed successful Generation 220 read-only segment-3A progress cycle; exact source SHA, 16384-block p24 prefix, zero mismatch at 1480465, empty pstore inconclusive, TARGET_ACCEPTED and slot-A fastboot in 439.436 seconds; never retry or flash'" "$gate" ||
	{ echo 'FAIL Generation 220 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_basis='one exact Generation 221 read-only grown-p24 geometry, UUID, clean-state, seal and boot-critical verification cycle; no storage writes, RAM-only, never flash or retry after COMMIT'" "$gate" ||
	{ echo 'FAIL Generation 221 gate basis differs from policy' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-watchdog-mmio-v1-generation204-20260827-r1/repack/stable-recovery-a.avb.img" &&
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-softdog-probe-v1-generation207-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed successful Generation 207 softdog fastboot-expiry proof; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 207 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-softdog-clone-v1-generation208-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 208 prewrite source-verifier timeout; exact fallback, no writes; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-postmortem-v2-generation210-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 210 partial-ext4 postmortem; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 210 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-softdog-direct-clone-v1-generation211-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 211 direct p24 clone timeout; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 211 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-chunk1-v1-generation212-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 212 direct chunk timeout; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 212 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent18-v1-generation213-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed successful Generation 213 extent-18 overwrite; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent20-seg1-v1-generation215-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed successful Generation 215 extent-20 segment-1 overwrite; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 215 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent20-seg2-v1-generation216-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 216 segment-2 stall without autonomous fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 216 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent20-seg2a-v1-generation217-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed successful Generation 217 extent-20 segment-2A overwrite; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 217 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent20-seg2b-v1-generation218-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed successful Generation 218 extent-20 segment-2B overwrite; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 218 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent20-seg3a-v1-generation219-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 219 segment-3A hard stall without autonomous fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 219 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-progress-v1-generation220-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed successful Generation 220 read-only progress cycle; exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 220 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-verify-v1-generation221-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 221 grown-p24 verifier; exact geometry passed, boot-critical tree failed generically, exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 221 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-tree-detail-v1-generation222-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 222 classifier; exact sshd and ssh-keygen content-only mismatches, TARGET_ACCEPTED and exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 222 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-ssh-repair-v1-generation223-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 223 two-file repair; in-mount hashes passed, post-unmount clean-state assertion failed, exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 223 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-postrepair-verify-v1-generation224-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 224 read-only postrepair verifier; ext4 clean-with-errors, tree skipped, exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v1-generation226-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 226 native-p24 cycle; switch-root and host-key pin passed, authenticated SSH returned 255 until exact fallback; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 226 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/persistent-native-root-v2-generation227-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 227 native-p24 cycle; switch-root and host-key pin passed, 127 SSH attempts returned 255, exact offline AArch64 controls disproved the locked-root diagnosis, and slot-A recovery fallback passed; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 227 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/persistent-native-root-v3-generation228-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed successful Generation 228 first-SSH-attempt discriminator; exact root/key/config state, client timeout while target blocked at PAM initialization, TARGET_ACCEPTED and direct fastboot return; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v5-generation230-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 230; repaired p24, switch-root, first key auth and UFS passed; runtime raced the ready marker, later systemd snapshot passed, exact fastboot returned; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 230 artifact role differs from live gate' >&2; exit 1; }
awk -F '\t' '
	$1 == "build/persistent-native-root-v6-generation231-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "consumed Generation 231; marker/attestation passed before systemd final state, later full snapshot passed, exact fastboot returned; never retry or flash" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v8-generation233-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 233 exact repeat of successful native-root baseline; unchanged target/raw recovery, fresh signed v8 intent and AVB generation; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 233 artifact role differs from live gate' >&2; exit 1; }
grep -Fq "expected_boot_basis='one exact Generation 233 RAM-only repeat of the successful native-p24/systemd/key-SSH/UFS/NCM/full-readiness/direct-fastboot baseline; unchanged target and raw recovery; never flash or retry after COMMIT'" "$gate" ||
	{ echo 'FAIL Generation 233 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_role='unbooted Generation 233 exact repeat of successful native-root baseline; unchanged target/raw recovery, fresh signed v8 intent and AVB generation; never flash or retry after COMMIT'" "$gate" ||
	{ echo 'FAIL Generation 233 gate role differs from artifact manifest' >&2; exit 1; }
