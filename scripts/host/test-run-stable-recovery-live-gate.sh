...
grep -Fq "expected_boot_role='unbooted Generation 150 direct-versus-buffered 32 MiB UFS benchmark with sync-independent fastboot fallback; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-write-benchmark-v41-generation150-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 150 cycle comparing one 32 MiB aligned direct write with one 32 MiB buffered write inside the reviewed userdata image directory; existing partial preserved; disposable files removed on success; sync-independent restart2-plus-SysRq fallback; RAM-only; never flash or retry after entry" &&
