...
awk -F '\t' '
	...
	$1 == "build/ssh-acceptance-v7-production-20260811-r1/wrapper/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact stage-150 SSH-acceptance diagnostic recovery; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { execution++ ; next }
...
grep -Fq '"headless-diagnostic-ssh-acceptance-v13-live-v1"' "$lifecycle" ||
