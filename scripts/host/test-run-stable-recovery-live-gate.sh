...
awk -F '\t' '
	...
	$1 == "build/persistent-root-deferred-ufs-v10-generation31-20260812-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact deferred Qualcomm UFS-probe discriminator after stable USB NCM; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { persistent++ ; next }
	...
	{ exit 1 }
	END { if (NR != 11 || observer != 1 || core != 1 || generation25 != 1 || generation26 != 1 || generation27 != 1 || generation28 != 1 || generation29 != 1 || generation30 != 1 || persistent != 1 || revoked != 1) exit 1 }
	' "$boot_policy" ||
