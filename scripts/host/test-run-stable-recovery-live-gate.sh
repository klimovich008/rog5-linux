...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-watchdog-observer-v2-generation203-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 203 read-only early inherited-watchdog register snapshot published through the repeated stage channel immediately after NCM carrier and before power/UFS/SSH; no watchdog registration, MMIO write or storage path; RAM-only, never flash or retry after COMMIT" &&
...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-watchdog-observer-v2-generation203-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 203 read-only early inherited-watchdog register snapshot published through the repeated stage channel immediately after NCM carrier and before power/UFS/SSH; no watchdog registration, MMIO write or storage path; RAM-only, never flash or retry after COMMIT" && NF == 3 {
		stage2_watchdog_observer_v2++
	}
	END { exit allowed == 2 + power_usb && power_usb <= 1 && stage2_watchdog_observer_v2 == 1 ? 0 : 1 }
' "$boot_policy" ||
...
grep -Fq "expected_boot_basis='one exact Generation 203 read-only early inherited-watchdog register snapshot published through the repeated stage channel immediately after NCM carrier and before power/UFS/SSH; no watchdog registration, MMIO write or storage path; RAM-only, never flash or retry after COMMIT'" "$gate" ||
