...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-native-tree-detail-v1-generation222-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 222 read-only per-item p24 metadata and hash classification cycle; no storage writes, RAM-only, never flash or retry after COMMIT" &&
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-tree-detail-v1-generation222-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 222 read-only per-item native-root classifier; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
