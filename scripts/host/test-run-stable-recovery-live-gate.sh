...
awk -F '\t' \
	...
	$1 == "build/persistent-root-local-image-probe-writer-v11-generation104-20260823-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact local-image probe write using the live-proven Generation-64 bounded-write kernel, DTB, UFS modules, and relock path; no charging or unrelated subsystem change; RAM-only kernel/recovery; externally consumed exact claim required; never flash or retry after entry" &&
