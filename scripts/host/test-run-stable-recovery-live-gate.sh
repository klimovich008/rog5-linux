...
grep -Fq "expected_boot_role='unbooted Generation 137 live-proven UFS baseline; exact g359 Image/DT/module pair, UFS-only init, NCM reporter, slot-A fallback; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/ufs-baseline-proven-v28-generation137-20260824-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 137 minimal UFS-only cycle using the live-proven g359 Image, DTB, and four UFS modules; NCM stage reporting and bounded fallback only; no power loader, SSH, userdata, installer, mount, or storage-write path; RAM-only; never flash or retry after entry" &&
