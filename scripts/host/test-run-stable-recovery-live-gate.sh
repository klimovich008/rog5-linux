...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v2-generation227-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 227 RAM-only native-p24 systemd/SSH cycle with volatile-only root account unlock, immutable lower-root proof, password and keyboard auth disabled, charging/UFS evidence and restart2 fallback; no phone storage write, never flash or retry after COMMIT" &&
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v2-generation227-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 227 native-p24 systemd/key-only-SSH successor; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
