...
awk -F '\t' '
	...
	$1 == "build/power-usb-observer-v1-offline-r3/wrapper/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact side-port NCM SSH and charging observer; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { power_usb++ ; next }
...
[[ $(awk -F '\t' '$2 == "allow" { count++ } END { print count + 0 }' \
	"$boot_policy") == 3 ]] ||
	{ echo 'FAIL temporary-boot policy does not contain exactly retention observer, core, and power/USB observer' >&2; exit 1; }
