...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v3-generation228-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 228 RAM-only native-p24 SSH diagnostic: one client attempt, bounded target OpenSSH/PAM/config/namespace transcript over NCM, direct restart2 fallback, no phone storage write, never flash or retry after COMMIT" &&
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v3-generation228-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 228 first-SSH-attempt discriminator with direct restart2-to-fastboot proof; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
