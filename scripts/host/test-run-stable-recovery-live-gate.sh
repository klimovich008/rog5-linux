...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-direct-extent20-seg3a-v1-generation219-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 219 direct p24 overwrite of extent-20 segment 3A at block 1464081 for 27204 blocks, with exact evidence, sync, relock and softdog disarm; RAM-only, never flash or retry after COMMIT" &&
...
grep -Fq "expected_boot_basis='one exact Generation 219 direct p24 overwrite of extent-20 segment 3A at block 1464081 for 27204 blocks, with exact evidence, sync, relock and softdog disarm; RAM-only, never flash or retry after COMMIT'" "$gate" ||
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent20-seg3a-v1-generation219-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 219 direct extent-20 segment 3A; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
