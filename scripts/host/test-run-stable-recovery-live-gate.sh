...
grep -Fq "expected_boot_role='unbooted Generation 141 writer; proven power/UFS topology, key-only SSH, one bounded Arch image installer, relock, built-in fastboot return; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-glob-v32-generation141-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 141 write-capable staging cycle using the Generation-140-proven power/USB, glob-enabled 116-node UFS topology, exact userdata geometry, key-only SSH, one verified 649960943-byte compressed Arch image, sealed installer, complete relock, and built-in fastboot return; writes only the bounded image file in userdata; RAM-only; never flash or retry after entry" &&
