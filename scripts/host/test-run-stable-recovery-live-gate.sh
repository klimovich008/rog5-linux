...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-native-postmortem-v1-generation195-20260826-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 195 read-only p24 disposition cycle after ambiguous Generation 194; bounded 4 MiB prefix, superblock, and known-tree inspection with every UFS node read-only, no filesystem repair or write, and exact slot-A fastboot return; RAM-only, never flash or retry after COMMIT" &&
...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-native-postmortem-v1-generation195-20260826-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 195 read-only p24 disposition cycle after ambiguous Generation 194; bounded 4 MiB prefix, superblock, and known-tree inspection with every UFS node read-only, no filesystem repair or write, and exact slot-A fastboot return; RAM-only, never flash or retry after COMMIT" && NF == 3 {
		stage2_postmortem++
	}
	END { exit allowed == 2 + power_usb && power_usb <= 1 && stage2_postmortem == 1 ? 0 : 1 }
' "$boot_policy" ||
...
grep -Fq "expected_boot_basis='one exact Generation 195 read-only p24 disposition cycle after ambiguous Generation 194; bounded 4 MiB prefix, superblock, and known-tree inspection with every UFS node read-only, no filesystem repair or write, and exact slot-A fastboot return; RAM-only, never flash or retry after COMMIT'" "$gate" ||
