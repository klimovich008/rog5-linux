...
awk -F '\t' \
	...
	END { exit allowed == 2 + power_usb && power_usb <= 1 && stage2_clone_v3 == 1 ? 0 : 1 }
' "$boot_policy" ||
...
grep -Fq "expected_boot_basis='one exact Generation 197 p24 clone from the verified sealed p23 source into zero/non-ext4 p24; qcom-wdt driver/device/process liveness without optional sysfs, 30-second APSS watchdog, read-only source validation, allocated restore, UUID/grow/seal/relock, and exact slot-A fastboot return; RAM-only, never flash or retry after COMMIT'" "$gate" ||
