...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v6-generation231-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 231 RAM-only native-p24 readiness-wait acceptance: unchanged target bytes, bounded 90-second ready-marker wait, systemd/runtime/UFS acceptance and direct restart2 fastboot; never flash or retry after COMMIT" &&
...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v6-generation231-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 231 RAM-only native-p24 readiness-wait acceptance: unchanged target bytes, bounded 90-second ready-marker wait, systemd/runtime/UFS acceptance and direct restart2 fastboot; never flash or retry after COMMIT" && NF == 3 {
		native_root++
	}
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v6-generation231-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 231 readiness-wait native-p24 acceptance; unchanged target/raw recovery, fresh signed v6 intent and AVB generation; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
	{ echo 'FAIL Generation 231 artifact role differs from live gate' >&2; exit 1; }
grep -Fq "expected_boot_basis='one exact Generation 231 RAM-only native-p24 readiness-wait acceptance: unchanged target bytes, bounded 90-second ready-marker wait, systemd/runtime/UFS acceptance and direct restart2 fastboot; never flash or retry after COMMIT'" "$gate" ||
	{ echo 'FAIL Generation 231 gate basis differs from policy' >&2; exit 1; }
grep -Fq "expected_boot_role='unbooted Generation 231 readiness-wait native-p24 acceptance; unchanged target/raw recovery, fresh signed v6 intent and AVB generation; never flash or retry after COMMIT'" "$gate" ||
	{ echo 'FAIL Generation 231 gate role differs from artifact manifest' >&2; exit 1; }
