...
awk -F '\t' '
	...
	$1 == "build/ssh-acceptance-v19-iproute-trailing-space-fix-20260812-r1/wrapper/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact iproute-whitespace-aware SSH recovery; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { execution++ ; next }
...
grep -Fq '"headless-diagnostic-ssh-iproute-whitespace-v19-live-v1"' "$lifecycle" ||
