...
awk -F '\t' '
	...
	$1 == "build/host-rendezvous-v6-post-claim-status-production-20260810-r1/wrapper/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one post-claim-status diagnostic execution recovery; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { execution++ ; next }
...
grep -Fq '"headless-diagnostic-host-rendezvous-v3-live-v6"' "$lifecycle" ||
