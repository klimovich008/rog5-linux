...
awk -F '\t' \
	...
	$1 == "build/persistent-root-power-usb-v5-generation81-20260822-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact RAM-only userdata-mount discriminator after proven side-port power, NCM, deferred UFS, storage lock, and exact userdata resolution; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { generation81++ ; next }
