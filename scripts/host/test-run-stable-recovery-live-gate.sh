...
grep -Fq "expected_boot_role='unbooted Generation 148 target-only parent-before-child read-write correction; proven kernel, UFS, SSH, transfer, bounded image, relock, fastboot; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-rworder-v39-generation148-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 148 target-only staging cycle clearing and verifying the parent disk before userdata; proven kernel, DTB, modules, recovery raw bytes, one image-file scope, relock, and slot-A fallback unchanged; RAM-only; never flash or retry after entry" &&
