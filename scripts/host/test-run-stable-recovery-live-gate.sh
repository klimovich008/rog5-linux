...
grep -Fq "expected_boot_role='unbooted Generation 136 exact-OF-node classifier; exact g359 closure and read-only platform identity, unchanged kernel/DT/stable recovery, slot-A fallback; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-of-node-v27-generation136-20260824-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 136 full staging cycle matching the UFS platform device by exact runtime of_node symlink identity, independent of basename; unchanged g359 modules, Image, DTB, reporter/listener, installer, storage scope, RAM-only; never flash or retry after entry" &&
