...
grep -Fq "expected_boot_role='unbooted Generation 125 immediate exact-UDC bind full staging successor; unchanged Image/DTB/power-USB/UFS/one-file installer and fastboot rollback; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-bind-v16-generation125-20260824-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 125 full staging cycle with immediate exact a600000.usb bind, absent-only retry, wrong/multiple refusal, and post-bind verification; unchanged Image/DTB/power-USB/UFS/installer, one exact userdata image path, key-only SSH, RAM-only; never flash or retry after entry" &&
