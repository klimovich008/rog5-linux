...
grep -Fq "expected_boot_role='unbooted Generation 156 direct-image successor; preserves failure output and exact residual variants; RAM-only; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-direct-v47-generation156-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 156 cycle preserving bounded target failures and accepting only empty or one exact bounded buffered-file residual before staging the reviewed 16 GiB Arch image through 37 ordered direct extents; RAM-only; never flash or retry after entry" &&
