...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v1-generation226-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 226 RAM-only direct-p24 boot to tmpfs OverlayFS, systemd, strict key-only SSH, charging/UFS evidence and restart2 slot-A fastboot fallback; no phone storage write, never flash or retry after COMMIT" &&
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v1-generation226-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 226 RAM-only native-p24 systemd/SSH cycle; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
