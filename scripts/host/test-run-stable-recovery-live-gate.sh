...
awk -F '\t' '
	...
	$1 == "build/persistent-root-storage-read-v1-generation22-20260812-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact read-only UFS persistent Arch recovery; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { persistent++ ; next }
