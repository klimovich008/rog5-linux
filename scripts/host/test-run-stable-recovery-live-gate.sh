...
awk -F '\t' '
	...
	$1 == "build/ssh-acceptance-v14-host-key-product-fix-20260811-r1/wrapper/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact host-key-product-corrected SSH diagnostic recovery; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { execution++ ; next }
...
grep -Fq '"headless-diagnostic-ssh-bootstrap-v14-live-v1"' "$lifecycle" ||
