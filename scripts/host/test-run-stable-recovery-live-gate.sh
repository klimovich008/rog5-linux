...
awk -F '\t' '
	...
	$1 == "build/persistent-root-local-image-early-ssh-v45-generation69-20260814-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact read-only local-image Arch repeat proving a bounded authenticated-SSH rendezvous before one runtime evidence command; same accepted v45 target bundle, four-module UFS, two ro,noload ext4 layers, persisted Generation 64 marker, early strict Ed25519 SSH, storage attestation, tmpfs OverlayFS, and rollback; RAM-only kernel/recovery; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { generation69++ ; next }
...
[[ $(awk -F '\t' '$2 == "allow" { count++ } END { print count + 0 }' \
	"$boot_policy") == 3 ]] ||
	{ echo 'FAIL temporary-boot policy does not contain exactly observer, core, and Generation 69' >&2; exit 1; }
