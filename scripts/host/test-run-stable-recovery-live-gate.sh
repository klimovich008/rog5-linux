...
grep -Fq "expected_boot_role='unbooted Generation 145 writer discriminator; exact post-UFS pre-SSH stages, bounded installer, relock, fastboot; never flash'" "$gate"
...
awk -F '\t' \
	...
	$1 == "build/local-image-stage-stages-v36-generation145-20260825-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 145 pre-write discriminator adding existing userdata-resolved, storage-locked, and runtime stages around exact identity, lock, nologin removal, host-key generation, and sshd; otherwise identical proven writer, one image file, relock, fastboot; RAM-only; never flash or retry after entry" &&
