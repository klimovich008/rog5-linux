...
awk -F '\t' \
	...
	$1 == "build/persistent-root-local-image-any-prior-v13-generation106-20260823-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact continuous-lifecycle read-only Arch boot using byte-identical V12 target bytes, pre-committed host choreography, canonical prior-writer probe validation, charging/UFS/NCM, pinned SSH, and stock slot-A fallback; never flash or retry after entry" &&
