...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-softdog-clone-v1-generation208-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 208 softdog-bounded clone from verified userdata image to p24, followed by grow, seal, read-only verification and relock; RAM-only kernel, never flash or retry after COMMIT" &&
...
grep -Fq "expected_boot_basis='one exact Generation 208 softdog-bounded clone from verified userdata image to p24, followed by grow, seal, read-only verification and relock; RAM-only kernel, never flash or retry after COMMIT'" "$gate" ||
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-softdog-clone-v1-generation208-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 208 softdog-bounded native p24 clone; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
