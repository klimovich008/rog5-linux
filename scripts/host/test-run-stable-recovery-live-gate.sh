...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v5-generation230-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 230 RAM-only native-p24 postrepair acceptance: read-only root verification, volatile PAM-free key-only early SSH, systemd/runtime/UFS acceptance and direct restart2 fastboot; never flash or retry after COMMIT" &&
...
awk -F '\t' \
	...
	$1 == "build/persistent-native-root-v5-generation230-20260828-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 230 RAM-only native-p24 postrepair acceptance: read-only root verification, volatile PAM-free key-only early SSH, systemd/runtime/UFS acceptance and direct restart2 fastboot; never flash or retry after COMMIT" && NF == 3 {
		native_root++
	}
...
awk -F '\t' '
	$1 == "build/persistent-native-root-v5-generation230-20260828-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 230 postrepair native-p24 acceptance; unchanged proven target bytes under fresh signed v5 intent and fresh AVB generation; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
...
grep -Fq "expected_boot_role='unbooted Generation 230 postrepair native-p24 acceptance; unchanged proven target bytes under fresh signed v5 intent and fresh AVB generation; never flash or retry after COMMIT'" "$gate" ||
	{ echo 'FAIL Generation 230 gate role differs from artifact manifest' >&2; exit 1; }
