...
awk -F '\t' \
	...
	$1 == "build/persistent-root-power-usb-v7-generation83-20260822-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact RAM-only direct-magic filesystem classifier after blkid reported no recognized type and ext4 mount returned status 255 EINVAL; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { generation83++ ; next }
