...
awk -F '\t' \
	...
	$1 == "build/persistent-root-local-image-any-prior-v12-generation105-20260823-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact read-only local-image Arch boot accepting only a canonical non-current probe UUID after the freshly staged image was proven marker-free and one writer was consumed; current charging/UFS/NCM baseline; externally consumed exact claim required; never flash or retry after entry" &&
