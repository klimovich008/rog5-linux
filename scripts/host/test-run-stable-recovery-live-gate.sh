...
awk -F '\t' '
	...
	$1 == "build/ssh-acceptance-v20-fatal-token-boundary-fix-20260812-r1/wrapper/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact token-delimited-fatal-filter SSH recovery; RAM-only; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { execution++ ; next }
