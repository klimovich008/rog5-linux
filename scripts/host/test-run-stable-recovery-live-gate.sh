...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-native-ssh-repair-v1-generation223-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 223 p24-only repair of sshd and ssh-keygen after exact old-hash precondition, power/thermal gate, 120-second softdog, post-write verification and relock; RAM-only, never flash or retry after COMMIT" &&
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-ssh-repair-v1-generation223-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 223 exact-two-file native-root repair; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
