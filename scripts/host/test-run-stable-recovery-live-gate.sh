...
awk -F '\t' '
	...
	$1 == "build/persistent-root-local-image-write-mountpoint-v42-generation64-20260814-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact bounded SM8350 UFS local-image contained-write attempt with the missing fixed /mnt/probe-root created and verified before opening the userdata write window; all 116 physical nodes are locked first, only the exact userdata partition and parent open for one fixed marker inside the existing 16 GiB image, and every node is relocked before read-only Arch SSH; RAM-only kernel/recovery; externally consumed exact claim required; never flash or retry after entry" && NF == 3 { generation64++ ; next }
...
[[ $(awk -F '\t' '$2 == "allow" { count++ } END { print count + 0 }' \
	"$boot_policy") == 3 ]] ||
	{ echo 'FAIL temporary-boot policy does not contain exactly observer, core, and Generation 64' >&2; exit 1; }
