...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-native-postrepair-verify-v1-generation224-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 224 read-only postrepair ext4 state and seven-item boot-critical verification cycle; no storage writes, RAM-only, never flash or retry after COMMIT" &&
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-native-postrepair-verify-v1-generation224-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 224 read-only postrepair verifier; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
