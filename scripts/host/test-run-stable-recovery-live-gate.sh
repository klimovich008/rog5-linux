...
awk -F '\t' \
	...
	$1 == "build/storage-layout-stage2-direct-extent18-v1-generation213-20260827-r1/repack/stable-recovery-a.avb.img" &&
		$2 == "allow" &&
		$3 == "one exact Generation 213 direct p24 overwrite of extent 18 with BEGIN/PASS evidence, sync, relock and softdog disarm; no fsck, grow or seal; RAM-only, never flash or retry after COMMIT" &&
...
grep -Fq "expected_boot_basis='one exact Generation 213 direct p24 overwrite of extent 18 with BEGIN/PASS evidence, sync, relock and softdog disarm; no fsck, grow or seal; RAM-only, never flash or retry after COMMIT'" "$gate" ||
...
awk -F '\t' '
	$1 == "build/storage-layout-stage2-direct-extent18-v1-generation213-20260827-r1/repack/stable-recovery-a.avb.img" &&
	$4 == "unbooted Generation 213 direct extent 18; never flash or retry after COMMIT" && NF == 5 { found++ }
	END { exit found == 1 ? 0 : 1 }
' "$artifact_manifest" ||
