...
runner=$repo/scripts/host/run-a660-ucode-allocation-v6-live-gate.sh

[[ -x $runner ]] || {
	echo 'FAIL missing host A660 ucode-allocation v6 live-gate runner' >&2
	exit 1
}
bash -n "$runner"
...
for contract in \
	...
	'run-network-root-a660-ucode-allocation-v6-gate.sh' \
...
do
	grep -Fq "$contract" "$runner" || {
		echo "FAIL host ucode-allocation v6 runner omits: $contract" >&2
		exit 1
	}
done

[[ $(grep -Fc \
	'exec env ALLOW_MAINLINE_A660_UCODE_ALLOCATION_V6_GATE=1' "$runner") == 1 ]]
...
if grep -Eq \
	...
	"$runner"
then
	echo 'FAIL host ucode-allocation v6 runner bypasses identity or storage safety' >&2
	exit 1
fi
...
set -eu
case $* in
	*"exec env ALLOW_MAINLINE_A660_UCODE_ALLOCATION_V6_GATE=1"*)
		printf '%s\n' gate >>"$MOCK_CALLS"
		echo 'PASS A660-ucode-allocation-v6 baseline storage=0 mounts=0 firmware_files=2 firmware_requests=0 firmware_releases=0 zap=absent render=0 drm_fds=0 maps=0 unmaps=0 closes=0 gem_frees=0 failed_units=0 thermal_zones=29 thermal_max_mC=38500 module_files=7 helper=exact compiler=relocations watchdog=armed'
		echo 'PASS A660 ucode-allocation-v6 open_invocations=1 open_errno=117 firmware_requests=2 firmware_releases=2 success_markers=1 maps=3 unmaps=3 closes=3 gem_frees=3 kernel_news=3 kernel_puts=2 wrapper_gets=1 wrapper_puts=2 logical_gets=4 logical_puts=4 gem_snapshot=equal zap=absent power=0 hfi=0 scm=0 drm_fds=0 storage=0 mounts=0 failed_units=0 iommu=2 render=1 thermal_zones=29 thermal_max_mC=38500 exact_reprobe=1 driver_override=unset-null-representation watchdog=disarmed'
		echo 'PASS compound A660 ucode-allocation v6 gate open_errno=117 maps=3 unmaps=3 closes=3 gem_frees=3 kernel_news=3 kernel_puts=2 wrapper_gets=1 wrapper_puts=2 logical_gets=4 logical_puts=4 gem_snapshot=equal transition_watchdog=armed reboot=requested'
		exit 255
		;;
	*"file_count="*)
		printf '%s\n' verify >>"$MOCK_CALLS"
...
[[ $(stat -c %a \
	"$stage/evidence/a660-ucode-allocation-v6-live-gate.log") == 600 ]]

echo 'PASS host A660 ucode-allocation v6 gate stages two exact tmpfs inputs, invokes once, logs privately, and never retries'
