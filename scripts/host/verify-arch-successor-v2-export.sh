...
root=${1:?usage: verify-arch-successor-v2-export.sh ROOT}
...
project_commit=ed7fa5e12e888c90edfe6e89a45beb30a7b222f6
...
report_hash=6e2af69d729c4d4f8d22e7ba2688561218e870bc8de14debf0fe435019c08a67
staged_verifier_hash=5137868d14400815e99ee642d78ccd125196ce811238120836c59cce92abe44e
...
[[ -d $root && ! -L $root ]] ||
	fail 'successor v2 export root must be a real directory'
root=$(realpath -e "$root")
[[ $root != / ]] || fail 'refusing the host root filesystem'
[[ $(findmnt -n -o FSTYPE --target "$root") == btrfs ]] ||
	fail 'successor v2 export is not on Btrfs'
btrfs subvolume show "$root" >/dev/null 2>&1 ||
	fail 'successor v2 export is not a Btrfs subvolume'
[[ $(btrfs property get -ts "$root" ro) == ro=true ]] ||
	fail 'successor v2 export Btrfs subvolume is writable'
[[ $(stat -c '%u:%g:%a' "$root") == 0:0:555 ]] ||
	fail 'successor v2 export root is not root-owned mode 0555'
...
[[ -f $archive && ! -L $archive ]] ||
	fail 'manifest-pinned successor v2 archive is absent'
[[ $(stat -c %s "$archive") == "$expected_size" ]] ||
	fail 'successor v2 archive size changed'
check_hash "$archive" "$expected_hash" 'successor v2 archive'
...
check_hash \
	...
	"$report_hash" 'offline successor v2 report'
check_hash "$repo/scripts/device/verify-staged-arch-rootfs-v2.sh" \
	"$staged_verifier_hash" 'successor v2 staged-root verifier'
...
[[ -f $seal && ! -L $seal ]] ||
	fail 'successor v2 export seal is absent or linked'
[[ $(stat -c '%u:%g:%a' "$seal") == 0:0:444 ]] ||
	fail 'successor v2 export seal metadata changed'
...
cmp "$expected_seal" "$seal" ||
	fail 'successor v2 export seal is not byte-exact'
...
echo 'PASS Arch successor v2 export package=655 agent=isolated hotspot=fail-closed-v2 services=exact secrets=absent root-owned read-only Btrfs mode 0555 promotion=UNBOOTED_HOLD'
