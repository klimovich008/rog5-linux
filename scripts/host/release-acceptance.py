...
def run_one(test, output, release=None, capture=None, rescue_inputs=None, activation_fixture_build=None, wifi_restart_inputs=None, standalone_boot_inputs=None, runtime_inputs=None):
    ...
    if any('{' in token or '}' in token for command in commands for token in command):
        ...
        if test['id'] in ('S02','S03','S04','S05'):
            if runtime_inputs is None or test['id'] not in runtime_inputs:
                ...
                return row
            path,pin=runtime_inputs[test['id']]
        ...
        if test['id']=='S01':
            if standalone_boot_inputs is None:
                ...
                return row
            path,pin=standalone_boot_inputs
            if not path.is_absolute() or not re.fullmatch('[0-9a-f]{64}',pin):
    ...
    if row['status']=='PASS' and test['id'] in ('F02','S01','S02','S03','S04','S05'):
        try:
            test_id=test['id']
            ...
            if (proof['status']!='PASS' or proof[test_id.lower()+'_qualified'] is not True or
                ...
                raise ValueError(test_id+' input, source or artifact binding mismatch')
...
def main():
    ...
    parser.add_argument('--runtime-inputs',action='append',default=[],metavar='S02|S03|S04|S05=PATH,SHA256',
                        help='pinned completed transfer/restart/durability/repeated-boot evidence; each outcome once')
    args = parser.parse_args()
    runtime_inputs={}
    for raw in args.runtime_inputs:
        try:
            kind,value=raw.split('=',1);path,pin=value.rsplit(',',1)
            if kind not in ('S02','S03','S04','S05') or kind in runtime_inputs or not Path(path).is_absolute() or not re.fullmatch('[0-9a-f]{64}',pin):
                raise ValueError('invalid or duplicate runtime input')
            runtime_inputs[kind]=(Path(path),pin)
        except ValueError:
            parser.error('runtime evidence requires unique S02|S03|S04|S05=ABSOLUTE_PATH,SHA256')
