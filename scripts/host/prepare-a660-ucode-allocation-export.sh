...
export_root=${5:-/var/lib/rog5-network-root-a660-ucode-allocation-v5}
...
work=$(mktemp -d /var/tmp/rog5-a660-ucode-allocation-export.XXXXXX)
succeeded=0
...
install -Dm0755 "$helper" \
	"$stage/usr/local/libexec/rog5-a660-ucode-allocation-open"
...
install -Dm0755 "$baseline" \
	"$stage/usr/local/sbin/rog5-a660-ucode-allocation-baseline"
install -Dm0755 "$probe" \
	"$stage/usr/local/sbin/rog5-a660-ucode-allocation-probe"
install -Dm0444 "$acceptance" \
	"$stage/etc/rog5/a660-registration-v3-live.accepted"

seal=$stage/etc/rog5/a660-ucode-allocation-export
{
	printf 'diagnostic_generation=v5\n'
	...
	printf 'trace_policy=PID_FILTERED_EXACT_BALANCE\n'
	printf 'state_policy=PRE_POST_GEM_SNAPSHOT_EQUAL\n'
	printf 'v4_reuse=FORBIDDEN\n'
} >"$seal"
...
succeeded=1
echo "PASS prepared A660 ucode-allocation v5 export at $export_root"
