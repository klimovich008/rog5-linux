...
[[ ${ALLOW_ARCH_SUCCESSOR_V1_LIVE_GATE:-} == 1 ]] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V1_LIVE_GATE=1 for the one-shot gate'
[[ ${ALLOW_ARCH_SUCCESSOR_V1_REBOOT:-} == 1 ]] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V1_REBOOT=1 for immediate fallback'
...
export_verifier=$repo/scripts/host/verify-arch-successor-export.sh
...
verify_input "$gate" \
	50803d4c42619a0118d1458c694296d7151031befcf52110c23de80034bcd3c6
verify_input "$export_verifier" \
	6176b4172c7ad3a0338686eb4c3fd30a6cbcb32c6237c190317da4a4b197a983
...
ssh_options=(
	...
	-o HostKeyAlias=rog5-arch-successor-v1
	-o ConnectTimeout=8
...
remote_prepare='
set -eu
directory=/run/rog5-arch-successor-v1-control
[ ! -e "$directory" ] || {
...
scp -q "${ssh_options[@]}" "$disarm" "$gate" \
	"$target:/run/rog5-arch-successor-v1-control/"
...
chmod 0500 \
	...
	"$directory/run-network-root-arch-successor-v1-gate.sh"
...
check "$directory/disarm-network-root-watchdog.sh" \
	b126182b615831e6f39784e4a2657cc60096ff906c26f1458be7d9a0d3ea065a
check "$directory/run-network-root-arch-successor-v1-gate.sh" \
	50803d4c42619a0118d1458c694296d7151031befcf52110c23de80034bcd3c6
seal=/.rog5/root-ro/etc/rog5/arch-successor-v1-export
[ "$(stat -c "%u:%g:%a" "$seal")" = 0:0:444 ]
[ "$(sha256sum "$seal" | cut -d " " -f 1)" = \
	6b5fa1b8e93b7e9f1ad41788ca524d5be6b4195c28ce85f70a28143360109eb4 ]
grep -qx "export_generation=arch-successor-v1" "$seal"
grep -qx "archive_sha256=88c2d671a26f577aef963212cda17bc61baa888d77d0c1aaf1ca25c6fb3ad62a" \
	"$seal"
grep -qx "project_commit=22f5429fd5497ce1a37addb4ff9ab3cb9027af78" "$seal"
...
umask 077
log=$evidence_dir/arch-successor-v1-live-gate.log
[[ ! -e $log ]] || fail 'private live-gate log already exists'
remote_gate='exec env ALLOW_ARCH_SUCCESSOR_V1_GATE=1 ALLOW_ARCH_SUCCESSOR_V1_REBOOT=1 /run/rog5-arch-successor-v1-control/run-network-root-arch-successor-v1-gate.sh'
set +e
...
grep -Eq \
	'^PASS Arch successor v1 headless gate kernel=7[.]1[.]4-g7a5cef0db479 packages=655 systemd=running coldplug=success tmpfiles=success sysusers=success agent=isolated headless=1 screen=(off|absent) machine_id=volatile lower=sealed storage=0 mounts=0 failed_units=0 transition_watchdog=armed reboot=requested$' \
	"$log" || fail 'target Arch successor v1 acceptance did not pass'
[[ $ssh_status -ne 0 ]] ||
	fail 'target gate returned without expected reboot disconnect'

echo 'PASS one-shot Arch successor v1 gate returned exact headless first-boot evidence and requested fallback reboot; verify fallback and host cleanup separately'
