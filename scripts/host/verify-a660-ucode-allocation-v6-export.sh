...
root=${1:?usage: verify-a660-ucode-allocation-v6-export.sh ROOT BASE_ROOT}
...
seal=$root/etc/rog5/a660-ucode-allocation-v6-export
[[ -f $seal && ! -L $seal ]]
...
for identity in \
	...
	"predecessor_report_sha256=$rejection_sha" \
	'predecessor_consumption_commit=86b6663' \
	'compiler_policy=PINNED_MSM_RELOCATIONS' \
	"compiler_verifier_sha256=$relocation_verifier_sha" \
...
do
	grep -Fqx "$identity" "$seal" ||
		fail "ucode-allocation v6 seal omits: $identity"
done
...
baseline=$root/usr/local/sbin/rog5-a660-ucode-allocation-v6-baseline
probe=$root/usr/local/sbin/rog5-a660-ucode-allocation-v6-probe
helper=$root/usr/local/libexec/rog5-a660-ucode-allocation-v6-open
...
for removed in \
	...
	usr/local/sbin/rog5-a660-ucode-allocation-baseline \
	usr/local/sbin/rog5-a660-ucode-allocation-probe \
	usr/local/libexec/rog5-a660-firmware-request-only-open \
	usr/local/libexec/rog5-a660-ucode-allocation-open \
	etc/rog5/a660-registration-export \
	etc/rog5/a660-firmware-request-only-export \
	etc/rog5/a660-ucode-allocation-export
do
	[[ ! -e $root/$removed ]] ||
...
echo 'PASS A660 ucode-allocation v6 export modules=7 firmware=2 zap=absent helper=exact compiler=relocations logical_vmap=4/4 gem_snapshot=equal credentials=preserved base=registration-v3 root-owned mode 0555'
