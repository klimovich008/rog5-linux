...
root=${1:?usage: verify-arch-successor-v3-export.sh ROOT}
...
hotspot_control_hash=5e2b4af39227f3afd37a494474faf982f1a87f3e8807406e47196d92b3bb079d
hotspot_service_hash=8ea3d2509bb220d200816571f379c2992c5281771be22d1b84d49d4a716cd814
power_handler_hash=66b3a8bfc32434e450d10ea707e21481b991e6fc728cd7afa618664331b4298a
power_service_hash=c617188753e17482328f69abc55c3d2b6da62dd543ecb3a14f551c4f17fb72c7
report_hash=8185962d42cdf4f77f052a39f7b8eeb2e4d107d62c184f05a84dcb9c0450bd80
staged_verifier_hash=734283f0a465011682f8cca625614f4e67ec4554d3b3163e9bad326d78f3551d
...
seal_relative=etc/rog5/arch-successor-v3-export
...
[[ -d $root && ! -L $root ]] ||
	fail 'successor v3 export root must be a real directory'
root=$(realpath -e "$root")
[[ $root != / ]] || fail 'refusing the host root filesystem'
[[ $(findmnt -n -o FSTYPE --target "$root") == btrfs ]] ||
	fail 'successor v3 export is not on Btrfs'
btrfs subvolume show "$root" >/dev/null 2>&1 ||
	fail 'successor v3 export is not a Btrfs subvolume'
[[ $(btrfs property get -ts "$root" ro) == ro=true ]] ||
	fail 'successor v3 export Btrfs subvolume is writable'
[[ $(stat -c '%u:%g:%a' "$root") == 0:0:555 ]] ||
	fail 'successor v3 export root is not root-owned mode 0555'
...
[[ -f $archive && ! -L $archive ]] ||
	fail 'manifest-pinned successor v3 archive is absent'
[[ $(stat -c %s "$archive") == "$expected_size" ]] ||
	fail 'successor v3 archive size changed'
check_hash "$archive" "$expected_hash" 'successor v3 archive'
...
check_hash "$repo/scripts/device/vpn-hotspot-v2.sh" \
	"$hotspot_control_hash" 'VPN hotspot v2 control'
check_hash "$repo/packaging/arch/rog5-vpn-hotspot-v2.service" \
	"$hotspot_service_hash" 'VPN hotspot v2 service'
check_hash "$repo/scripts/device/power-buttond.py" \
	"$power_handler_hash" 'power-button handler'
check_hash "$repo/packaging/arch/rog5-power-button.service" \
	"$power_service_hash" 'power-button service'
check_hash \
	"$repo/test-results/2026-07-27-arch-successor-v3-power-button-offline.md" \
	"$report_hash" 'offline successor v3 report'
check_hash "$repo/scripts/device/verify-staged-arch-rootfs-v3.sh" \
	"$staged_verifier_hash" 'successor v3 staged-root verifier'
...
[[ -f $seal && ! -L $seal ]] ||
	fail 'successor v3 export seal is absent or linked'
[[ $(stat -c '%u:%g:%a' "$seal") == 0:0:444 ]] ||
	fail 'successor v3 export seal metadata changed'
...
{
	printf 'export_generation=arch-successor-v3\n'
	...
	printf 'hotspot_control_sha256=%s\n' "$hotspot_control_hash"
	printf 'hotspot_service_sha256=%s\n' "$hotspot_service_hash"
	printf 'power_handler_sha256=%s\n' "$power_handler_hash"
	printf 'power_service_sha256=%s\n' "$power_service_hash"
...
cmp "$expected_seal" "$seal" ||
	fail 'successor v3 export seal is not byte-exact'
...
cmp "$root/usr/local/sbin/rog5-vpn-hotspot.sh" \
	"$repo/scripts/device/vpn-hotspot-v2.sh"
cmp "$root/etc/systemd/system/rog5-vpn-hotspot.service" \
	"$repo/packaging/arch/rog5-vpn-hotspot-v2.service"
cmp "$root/usr/local/libexec/rog5-power-buttond" \
	"$repo/scripts/device/power-buttond.py"
cmp "$root/etc/systemd/system/rog5-power-button.service" \
	"$repo/packaging/arch/rog5-power-button.service"
[[ $(stat -c '%u:%g:%a' \
	"$root/usr/local/libexec/rog5-power-buttond") == 0:0:755 ]]
[[ $(stat -c '%u:%g:%a' \
	"$root/etc/systemd/system/rog5-power-button.service") == 0:0:644 ]]
[[ $(readlink \
	"$root/etc/systemd/system/multi-user.target.wants/rog5-power-button.service") == \
...
for unit in NetworkManager.service rog5-power-button.service \
	rog5-server-inhibit.service sshd.service; do
...
echo 'PASS Arch successor v3 export package=655 agent=isolated hotspot=fail-closed-v2 power-button=press-only-screen-toggle services=exact secrets=absent root-owned read-only Btrfs mode 0555 promotion=UNBOOTED_HOLD'
