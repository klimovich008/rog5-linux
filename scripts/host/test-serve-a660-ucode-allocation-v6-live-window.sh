...
do
	grep -Fq "$contract" "$serve" || {
		echo "FAIL bounded NFS server omits v6 live-window contract: $contract" >&2
		exit 1
	}
done

case_line=$(grep -n \
	'/var/lib/rog5-network-root-a660-ucode-allocation-v6)' "$serve" |
	head -n 1 | cut -d: -f1)
guard_line=$(grep -n \
	'ALLOW_MAINLINE_A660_UCODE_ALLOCATION_V6_NFS:-' "$serve" |
	head -n 1 | cut -d: -f1)
verify_line=$(grep -n \
	'verify-a660-ucode-allocation-v6-export.sh' "$serve" |
	head -n 1 | cut -d: -f1)
...
echo 'PASS A660 ucode-allocation v6 NFS window is exact-root, opt-in, verifier-first, bounded, and non-flashing'
