...
root=${1:-/var/lib/rog5-network-root-arch-successor-v2}
...
[[ -d $root && ! -L $root ]] || fail 'missing prepared successor v2 root'
root=$(realpath -e "$root")
[[ $root == /var/lib/rog5-network-root-arch-successor-v2 ]] ||
	fail 'unexpected successor v2 export root'
[[ ${ALLOW_ARCH_SUCCESSOR_V2_NFS:-} == 1 ]] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V2_NFS=1 for the attended Arch successor v2 window'
"$repo/scripts/host/verify-arch-successor-v2-export.sh" "$root"
