...
TARGET_LOCK = REPO / "initramfs/generated-power-usb-active.sh"
...
def validate_capability(source: dict[str, object]) -> dict[str, object]:
    ...
    if not isinstance(integration, dict):
        raise GenerationError("candidate integration is absent")
    target_commands = (
        ["ip", "modprobe", "rog5-early-power-observer", "watchdog"]
        if integration.get("probe_phase") == "early-initramfs"
        else ["ip", "mount", "sshd", "systemd", "watchdog"]
    )
    expected = {
...
def python_payload(
    ...
    for name, value in (
        ...
        ("PROBE_PHASE", values["probe_phase"]),
...
def shell_payload(
    ...
    pairs = (
        ...
        ("POWER_USB_PROBE_PHASE", values["probe_phase"]),
...
def target_payload(record: dict[str, object], values: dict[str, str]) -> bytes:
    return (
        "#!/bin/sh\n"
        "# Generated by generate-power-usb-active.py; do not edit.\n"
        "power_usb_bundle_prefix=headless-power-usb-observer-v\n"
        f"power_usb_probe_phase={values['probe_phase']}\n"
    ).encode("ascii")
...
def outputs() -> dict[Path, bytes]:
    ...
    target_lock = target_payload(record, values)
    ...
    lock = {
        ...
        "target_lock_sha256": digest(target_lock),
        "expected_manifest_sha256": values["expected_manifest_sha256"],
        "output_root": values["output_root"],
        "probe_phase": values["probe_phase"],
        "boot_policy_basis": values["boot_policy_basis"],
...
def verify_consumer_closure(source: dict[str, object]) -> None:
    ...
    allowed = {
        ...
        TARGET_LOCK.relative_to(REPO).as_posix(),
