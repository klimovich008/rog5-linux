...
def shell_payload(
    ...
    pairs = (
        ...
        ("POWER_USB_BOOT_POLICY_STATUS", values["boot_policy_status"]),
...
def target_payload(record: dict[str, object]) -> bytes:
    return (
        "#!/bin/sh\n"
        "# Generated by generate-power-usb-active.py; do not edit.\n"
        f"power_usb_candidate={shlex.quote(str(record['candidate']))}\n"
    ).encode("ascii")
...
def outputs() -> dict[Path, bytes]:
    ...
    target_lock = target_payload(record)
