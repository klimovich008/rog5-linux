...
root=${1:?usage: verify-a660-ucode-allocation-export.sh ROOT BASE_ROOT}
...
"$repo/scripts/device/verify-a660-ucode-allocation-runtime-sources.sh" \
	"$repo/scripts/device/check-network-root-a660-ucode-allocation-baseline.sh" \
	"$repo/scripts/device/probe-network-root-a660-ucode-allocation.sh" \
	>/dev/null

seal=$root/etc/rog5/a660-ucode-allocation-export
[[ -f $seal && ! -L $seal ]]
...
do
	grep -Fqx "$identity" "$seal" ||
		fail "ucode-allocation seal omits: $identity"
done

baseline=$root/usr/local/sbin/rog5-a660-ucode-allocation-baseline
probe=$root/usr/local/sbin/rog5-a660-ucode-allocation-probe
helper=$root/usr/local/libexec/rog5-a660-ucode-allocation-open
acceptance=$root/etc/rog5/a660-registration-v3-live.accepted
for source_target in \
	"$repo/scripts/device/check-network-root-a660-ucode-allocation-baseline.sh:$baseline:755" \
	"$repo/scripts/device/probe-network-root-a660-ucode-allocation.sh:$probe:755" \
...
for symbol in adreno_load_ucode_only msm_gem_vma_map msm_gem_vma_unmap \
	...
	a6xx_ucode_unload msm_gpu_pm_resume adreno_runtime_resume \
	a6xx_pm_resume a6xx_gmu_resume adreno_hw_init a6xx_hw_init \
...
for removed in \
	...
	usr/local/sbin/rog5-a660-firmware-request-only-baseline \
	usr/local/sbin/rog5-a660-firmware-request-only-probe \
	usr/local/libexec/rog5-a660-firmware-request-only-open \
	etc/rog5/a660-registration-export \
	etc/rog5/a660-firmware-request-only-export
do
	[[ ! -e $root/$removed ]] ||
		fail "consumed diagnostic control remains: $removed"
done
for relative in \
...
unchanged_manifest() {
	...
	find "$tree" -xdev \
		...
		! -path "$tree/usr/local/sbin/rog5-a660-ucode-allocation-baseline" \
		! -path "$tree/usr/local/sbin/rog5-a660-ucode-allocation-probe" \
		! -path "$tree/etc/rog5/a660-registration-export" \
		! -path "$tree/etc/rog5/a660-ucode-allocation-export" \
...
unchanged_hashes() {
	...
	(
		...
		find . -xdev -type f \
			...
			! -path './usr/local/sbin/rog5-a660-ucode-allocation-baseline' \
			! -path './usr/local/sbin/rog5-a660-ucode-allocation-probe' \
			! -path './etc/rog5/a660-registration-export' \
			! -path './etc/rog5/a660-ucode-allocation-export' \
...
echo 'PASS A660 ucode-allocation v5 export modules=7 firmware=2 zap=absent helper=exact trace=balanced gem_snapshot=equal credentials=preserved base=registration-v3 root-owned mode 0555'
