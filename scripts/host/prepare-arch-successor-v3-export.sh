...
archive=${1:-$repo/artifacts/arch/rog5-arch-plasma-network-root-7.1.4-successor-v3.tar.gz}
export_root=${2:-/var/lib/rog5-network-root-arch-successor-v3}
...
verifier=$repo/scripts/host/verify-arch-successor-v3-export.sh
...
[[ -x $verifier ]] || fail 'missing successor v3 export verifier'
[[ $export_root == /var/lib/rog5-network-root-arch-successor-v3 ]] ||
	fail 'successor v3 export path is not exact'
[[ -f $archive && ! -L $archive ]] ||
	fail "missing regular successor v3 archive: $archive"
archive=$(realpath -e "$archive")
case $archive in
	"$repo"/*) relative=${archive#"$repo"/} ;;
	*) fail 'successor v3 archive must be inside the repository artifact store' ;;
esac
[[ $relative == "$archive_name" ]] || fail 'unexpected successor v3 archive'
...
[[ $(stat -c %s "$archive") == "$expected_size" ]] ||
	fail 'successor v3 archive size does not match the manifest'
[[ $(sha256sum "$archive" | cut -d ' ' -f 1) == "$expected_hash" ]] ||
	fail 'successor v3 archive hash does not match the manifest'
...
check_hash "$repo/scripts/device/verify-staged-arch-rootfs-v3.sh" \
	...
	'successor v3 staged-root verifier'
...
cleanup() {
	if [[ $succeeded != 1 && -e $stage ]]; then
		case $stage in
			/var/lib/rog5-network-root-arch-successor-v3.partial.*)
...
[[ -z $(find "$stage/etc/ssh" -maxdepth 1 -type f \
	...
	fail 'successor v3 archive unexpectedly contains an SSH host key'
host_key=$stage/etc/ssh/ssh_host_ed25519_key
ssh-keygen -q -t ed25519 -N '' -C rog5-arch-successor-v3 -f "$host_key"
...
chmod 0644 "$seal"
{
	...
	printf 'project_commit=b8b80013d0acd912530ce42af7bc0adf7f9fd6ea\n'
	...
	printf 'power_handler_sha256=66b3a8bfc32434e450d10ea707e21481b991e6fc728cd7afa618664331b4298a\n'
	printf 'power_service_sha256=c617188753e17482328f69abc55c3d2b6da62dd543ecb3a14f551c4f17fb72c7\n'
	printf 'offline_report_sha256=8185962d42cdf4f77f052a39f7b8eeb2e4d107d62c184f05a84dcb9c0450bd80\n'
	printf 'staged_verifier_sha256=734283f0a465011682f8cca625614f4e67ec4554d3b3163e9bad326d78f3551d\n'
...
succeeded=1
echo "PASS prepared Arch successor v3 protected export at $export_root"
