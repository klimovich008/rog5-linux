...
class RetentionCycleAdmissionTest(unittest.TestCase):
    ...
    def build_fixture(self) -> None:
        ...
        self.policy_path.write_text(
            ...
            "consumed by the sole Generation 70 RAM-only cycle; exact UFS, "
            "local image, tmpfs OverlayFS, P2 storage attestation, bounded "
            "authenticated SSH marker acceptance, strict key-only runtime, "
            "normal reboot, exact Alpine fallback, PS_HOLD lineage, and host "
            "restoration passed in 326.300 seconds; never retry or flash\n"
            "build/persistent-root-local-image-early-ssh-v45-generation67-"
            "20260814-r1/repack/stable-recovery-a.avb.img\trevoked\t"
            "consumed by the sole Generation 67 RAM-only cycle; exact UFS, two "
            "ro,noload ext4 layers, persisted marker, early strict key-only SSH, "
            "and storage attestation passed, but recovery ACM closed after the "
            "COMMIT claim before the post-claim response; host salvage proved "
            "early SSH active at about 94.147 seconds and full attestation at "
            "130.057 seconds; normal reboot, exact Alpine fallback, and host "
            "restoration passed; never retry or flash\n"
    ...
    def test_embedded_ram_record_must_match_canonical_checkout(self) -> None:
        records = [record for record in ADMISSION.canonical_native_ram_claims().values()
                   if b"\nexecution=fastboot-boot-ram-bundle\n" in record]
        self.assertTrue(records, "embedded RAM family omitted from closure")
        field = next(line for line in records[0].decode().splitlines()
                     if line.startswith("boot_image_sha256="))
