...
class RetentionCycleAdmissionTest(unittest.TestCase):
    ...
    def build_fixture(self) -> None:
        ...
        self.policy_path.write_text(
			...
			"repack/stable-recovery-a.avb.img\tallow\t"
			"one exact read-only SM8350 UFS local-root boot with bounded boot-critical "
			"identity admission, runtime block locks, exact userdata ro,noload mount, "
			"tmpfs OverlayFS, systemd, key-only SSH, receive-only stage heartbeats, "
			"and bounded rollback; RAM-only; externally consumed exact claim required; "
			"never flash or retry after entry\n"
