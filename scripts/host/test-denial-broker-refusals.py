...
def main():
    ...
    result = {'scope': 'actual broker functions with data adapters; no VM or phone proof', 'physical': 'NOT RUN', 'source_commit': selection.BASE, 'patch_sha256': hashlib.sha256(patch.read_bytes()).hexdigest(), 'runs': {}}
    ...
    for label, text in [('before', originals[path]), ('after', (source / path).read_text())]:
        ...
        methods = '\n'.join(selection.extract(text, name) for name in ('acquire', 'expire_authorizations'))
        ...
        expected = '3 passed; 4 failed' if label == 'before' else '7 passed; 0 failed'
