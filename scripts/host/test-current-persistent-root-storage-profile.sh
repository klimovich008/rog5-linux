...
profile=persistent-root-image-control-v8-live-v1
image_name=build/persistent-root-image-control-v8-generation29-20260812-r1/repack/stable-recovery-a.avb.img
basis='one exact UFS Image plus UFS-disabled DTB control with no storage probe; RAM-only; externally consumed exact claim required; never flash or retry after entry'
role='unbooted Generation 29 UFS Image plus live-proven UFS-disabled DTB control; UFS cannot probe or expose storage; twin signed bundle and byte-distinct AVB wrapper; one RAM-only use only; never flash'
tmp=$(mktemp -d)
...
for assignment in \
	...
	expected_target_id=persistent-root-image-control-v8 \
	expected_bundle=persistent-root-image-control-v8 \
	expected_bundle_profile=persistent-root-ro-v1 \
	expected_target_release=7.1.4-gcfd385a1c754 \
	expected_avb_salt=c7a39cc0963be73a261d349533e22d6c30b25a71be9565d67aa1faaa3f8b672a \
	expected_avb_digest=ecdc51ae970c2fd92ccbd702aaa00069c6eb0faf0e5ccb9fc8e9105d1b4cecff \
	expected_generation_record=5d3cc8063b69b7de16633ab025208cf1b34bdee8a4533f0ddf344edcee5c4a46 \
...
exact=(
	...
	persistent-root-image-control-v8
)
run_policy() {
...
grep -Fxq 'target_id=persistent-root-image-control-v8' <<<"$policy"
...
errors=(
	...
	'profile requires bundle=persistent-root-image-control-v8'
)
...
awk -F '\t' -v name="$image_name" -v role="$role" '
	$1 == name && $2 == "100663296" &&
	$3 == "0fa7d7511b0acce2427dccbc1c02dfdda6ee95b48040c6e66acb6ff77396f2ec" &&
...
production_root=$repo/build/persistent-root-image-control-v8-production-20260812-r1
generation_root=$repo/build/persistent-root-image-control-v8-generation29-20260812-r1
...
echo 'PASS Generation 29 UFS Image plus UFS-disabled DTB control profile, exact claim, and admission are pinned'
