...
PREFLIGHT_SIGNATURE = REPO / "manifests/storage-layout-stage2-preflight-generation186.manifest"
PREFLIGHT_SIGNATURE_WIDE = REPO / "manifests/storage-layout-stage2-preflight-generation187.manifest"
PREFLIGHT_SIGNATURE_PREFIX = REPO / "manifests/storage-layout-stage2-preflight-generation188.manifest"
PREFLIGHT_SIGNATURE_OUTPUT = REPO / "manifests/storage-layout-stage2-preflight-generation189.manifest"
PREFLIGHT_TEMPERATURE = REPO / "manifests/storage-layout-stage2-preflight-generation190.manifest"
...
class StorageLayoutStage2ContractTest(unittest.TestCase):
    ...
    def test_read_only_preflight_exits_before_watchdog_or_write_window(self) -> None:
        ...
        self.assertIn("Stage-2 signature classification changed", collector)
        self.assertIn("head -c 4097", self.executable_source(EXECUTOR))
    ...
    def test_generation186_reports_residual_signature(self) -> None:
        fields = dict(
            line.split("=", 1)
            for line in PREFLIGHT_SIGNATURE.read_text(encoding="ascii").splitlines()
        )
        digest = hashlib.sha256(PREFLIGHT_SIGNATURE.read_bytes()).hexdigest()
        self.assertEqual(digest, "faecb6ded2dea36099195dbb0e611e39a47dfef733ba3f94d9f17bd127a2e345")
        self.assertEqual(fields["signature_record"], "type,uuid,label,bytes,sha256")
        self.assertEqual(fields["storage_write"], "forbidden")
        rows = [line.split("\t") for line in BOOT_POLICY.read_text(encoding="ascii").splitlines()[1:]]
        self.assertEqual(sum(row[1] == "allow" for row in rows), 1)
        admitted = next(row for row in rows if row[0] == fields["profile"])
    ...
    def test_generation187_widens_only_signature_evidence(self) -> None:
        fields = dict(
            line.split("=", 1)
            for line in PREFLIGHT_SIGNATURE_WIDE.read_text(encoding="ascii").splitlines()
        )
        digest = hashlib.sha256(PREFLIGHT_SIGNATURE_WIDE.read_bytes()).hexdigest()
        self.assertEqual(digest, "b9f01bbc792cc41c054fb3c00e39bf8703ab122f74262a3efae83c444e9b987b")
        self.assertEqual(fields["signature_output_max_bytes"], "65536")
        self.assertEqual(fields["storage_write"], "forbidden")
        rows = [line.split("\t") for line in BOOT_POLICY.read_text(encoding="ascii").splitlines()[1:]]
        self.assertEqual(sum(row[1] == "allow" for row in rows), 1)
        admitted = next(row for row in rows if row[0] == fields["profile"])
    ...
    def test_generation188_caps_signature_evidence(self) -> None:
        fields = dict(
            line.split("=", 1)
            for line in PREFLIGHT_SIGNATURE_PREFIX.read_text(encoding="ascii").splitlines()
        )
        digest = hashlib.sha256(PREFLIGHT_SIGNATURE_PREFIX.read_bytes()).hexdigest()
        self.assertEqual(fields["signature_output_max_bytes"], "4096")
        self.assertEqual(fields["signature_truncation_flag"], "required")
        self.assertEqual(
            fields["signature_record"], "type,uuid,label,bytes,sha256,truncated"
        )
        self.assertEqual(fields["storage_write"], "forbidden")
        rows = [line.split("\t") for line in BOOT_POLICY.read_text(encoding="ascii").splitlines()[1:]]
        self.assertEqual(sum(row[1] == "allow" for row in rows), 1)
    ...
    def test_generation189_uses_nonempty_output_as_signature_presence(self) -> None:
        fields = dict(
            line.split("=", 1)
            for line in PREFLIGHT_SIGNATURE_OUTPUT.read_text(encoding="ascii").splitlines()
        )
        digest = hashlib.sha256(PREFLIGHT_SIGNATURE_OUTPUT.read_bytes()).hexdigest()
        self.assertEqual(fields["signature_presence"], "bounded-nonempty-output")
        self.assertEqual(fields["busybox_blkid_status"], "ignored")
        self.assertEqual(fields["storage_write"], "forbidden")
        rows = [line.split("\t") for line in BOOT_POLICY.read_text(encoding="ascii").splitlines()[1:]]
        self.assertEqual(sum(row[1] == "allow" for row in rows), 1)
        admitted = next(row for row in rows if row[0] == fields["profile"])
    ...
    def test_generation190_discovers_one_battery_and_keeps_temperature_limits(self) -> None:
        fields = dict(
            line.split("=", 1)
            for line in PREFLIGHT_TEMPERATURE.read_text(encoding="ascii").splitlines()
        )
        digest = hashlib.sha256(PREFLIGHT_TEMPERATURE.read_bytes()).hexdigest()
        self.assertEqual(fields["battery_discovery"], "exactly-one-type-Battery")
        self.assertEqual(fields["battery_temperature_decic"], "-200..499")
        self.assertEqual(fields["thermal_temperature_millic"], "-40000..64999")
        self.assertEqual(fields["storage_write"], "forbidden")
        rows = [line.split("\t") for line in BOOT_POLICY.read_text(encoding="ascii").splitlines()[1:]]
        self.assertEqual(sum(row[1] == "allow" for row in rows), 1)
        admitted = next(row for row in rows if row[0] == fields["profile"])
