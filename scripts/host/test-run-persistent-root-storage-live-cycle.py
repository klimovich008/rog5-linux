...
class PersistentRootLiveCycleTest(unittest.TestCase):
    ...
    def test_profile_and_artifact_identities_are_exact(self) -> None:
        ...
        self.assertEqual(
            MODULE.BUNDLE,
            "local-image-stage-writekernel-v40",
        )
        for digest in (
        ...
        self.assertEqual(
            ...
            "consume-local-image-stage-writekernel-v40-claim.py",
        )
    ...
    def test_writer_preflight_requires_the_exact_arch_transfer(self) -> None:
        source = MODULE_PATH.read_text()
        preflight = source[
            source.index("def preflight(") : source.index("def stop_recovery_host(")
        ]
        self.assertIn("exact_arch_image()", preflight)
        self.assertIn(
