...
def write_exclusive(
    ...
    try:
        ...
        if (
            ...
            fail(f"{label} output metadata is unsafe")
    except Exception:
        if descriptor >= 0:
            os.close(descriptor)
            descriptor = -1
...
def validate_candidate(payload: bytes) -> None:
    try:
...
def stage_inputs(
    ...
    validate_candidate(candidate_payload)
    staged: list[Path] = []
    try:
        staged.append(
            write_exclusive(
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("format=rog5-headless-ssh-deployment-signing-inputs-v1")
