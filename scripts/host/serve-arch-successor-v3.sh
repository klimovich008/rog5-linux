...
root=${1:-/var/lib/rog5-network-root-arch-successor-v3}
...
[[ -d $root && ! -L $root ]] || fail 'missing prepared successor v3 root'
root=$(realpath -e "$root")
[[ $root == /var/lib/rog5-network-root-arch-successor-v3 ]] ||
	fail 'unexpected successor v3 export root'
[[ ${ALLOW_ARCH_SUCCESSOR_V3_NFS:-} == 1 ]] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V3_NFS=1 for the attended Arch successor v3 window'
"$repo/scripts/host/verify-arch-successor-v3-export.sh" "$root"
