...
for contract in \
	...
	'ALLOW_MAINLINE_A660_GMU_CX_RUNTIME_PM_V10_NFS' \
	...
	'for the attended v10 window'
do
	grep -Fq "$contract" "$serve" || {
		echo "FAIL bounded NFS server omits v10 live-window contract: $contract" >&2
		exit 1
	}
done

case_line=$(grep -n \
	'/var/lib/rog5-network-root-a660-gmu-cx-runtime-pm-v10)' "$serve" |
	head -n 1 | cut -d: -f1)
guard_line=$(grep -n \
	'ALLOW_MAINLINE_A660_GMU_CX_RUNTIME_PM_V10_NFS:-' "$serve" |
	head -n 1 | cut -d: -f1)
verify_line=$(grep -n \
	'verify-a660-gmu-cx-runtime-pm-v10-export.sh' "$serve" |
	head -n 1 | cut -d: -f1)
...
[ "$(grep -Fc 'ALLOW_MAINLINE_A660_GMU_CX_RUNTIME_PM_V10_NFS:-' \
	"$serve")" -eq 1 ]
...
echo 'PASS A660 GMU/CX runtime-PM v10 NFS window is exact-root, opt-in, verifier-first, bounded, and non-flashing'
