...
class ExactClaimConsumerTest(unittest.TestCase):
    def test_exact_selector_rescue_is_bound_and_permanently_one_use(self):
        ...
        self.assertEqual(fields['execution'], 'fastboot-boot-fallback-only')
        ...
        for name in ('boot_image_sha256', 'selector_sha256', 'manifest_sha256',
                     'recovery_initramfs_sha256'):
            self.assertRegex(fields[name], r'^[0-9a-f]{64}$')
            changed = payload.replace(fields[name].encode(), b'0' * 64)
