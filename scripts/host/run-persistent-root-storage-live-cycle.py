...
PROFILE_ID = "storage-layout-stage2-watchdog-probe-v1-generation198-live-v1"
BUNDLE = "storage-layout-stage2-watchdog-probe-v1"
MANIFEST_SHA256 = (
...
LIVE_ROOT = (
    REPO
    / "build/storage-layout-stage2-watchdog-probe-v1-generation198-20260826-r1"
)
COMPONENT_ROOT = (
    REPO
...
PROFILE = CYCLE.CycleProfile(
    ...
    target_id=BUNDLE,
    admission_profile="storage-layout-stage2-watchdog-probe-v1",
    recovery_profile=PROFILE_ID,
    runtime_profile="storage-layout-stage2-watchdog-probe-v1",
    build_profile="storage-layout-stage2-watchdog-probe-v1",
    diagnostic=False,
)
...
def parse_postmortem_evidence(path: Path) -> str:
    ...
    known = {
        "source-clone": ("598a876b-a8db-4859-a01a-1b864b0a87f4", "4194304"),
        "grown-target": ("8b03827a-cc2d-4408-8558-e9b61195f96b", "8388603"),
    }
    if disposition in known:
        if (uuid, blocks, state, label, tree) != (
            *known[disposition], "clean", "ROG5_ARCH_A", "PASS"
        ):
...
def run(
    ...
    try:
        ...
        resolved = True
        print(
            "PASS one RAM-only cycle proved the watchdog path and classified "
            f"native p24 as {disposition} in {elapsed:.3f}s"
        )
...
def main(arguments: list[str]) -> int:
    ...
    if arguments == ["run"] and os.environ.get(
        ...
        fail("set ALLOW_STAGE2_P24_POSTMORTEM=1 for the read-only watchdog probe")
