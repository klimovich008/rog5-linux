...
output_dir=${OUTPUT_DIR:-/root/build/rog5-linux-7.1.4-discovery}
...
[ -d "$source_dir/.git" ] || { echo "FAIL missing source tree $source_dir" >&2; exit 1; }
...
export KBUILD_BUILD_TIMESTAMP="$(git -C "$source_dir" show -s --format=%cD "$expected_base")"
...

{
	...
	sha256sum "$base_fragment" "$discovery_fragment" \
		"$output_dir/.config" \
...
echo 'PASS dedicated Linux 7.1.4 UFS discovery kernel build'
