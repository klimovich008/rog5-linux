...
command_line='console=ttyMSM0,115200n8 rdinit=/init panic=10 oops=panic loglevel=8 ignore_loglevel printk.always_kmsg_dump=Y rog5.ufs_discovery=1 rog5.persistent_ro=1 ramoops.mem_address=0x9b800000 ramoops.mem_size=0x400000 ramoops.record_size=0x100000 ramoops.console_size=0x300000 ramoops.pmsg_size=0 ramoops.ftrace_size=0 ramoops.dump_oops=1'
command_line="$command_line rog5.recovery_timeout=$recovery_timeout"

for required in rog5.ufs_discovery=1 rog5.persistent_ro=1; do
	[ "$(printf '%s\n' "$command_line" | tr ' ' '\n' |
...
echo 'PASS mainline read-only persistent-root payload loaded; run kexec -e only during an attended test'
