...
clock_patch=${CLOCK_PATCH:-$repo/patches/linux-7.1.4/0017-drm-msm-add-a660-gmu-clock-preparation-diagnostic.patch}
...
clock_verifier=$repo/scripts/device/verify-a660-gmu-clock-preparation-patch.sh
...
for input in "$base_fragment" "$network_fragment" \
	...
	"$ucode_patch" "$resume_patch" "$cx_patch" "$clock_patch" "$gmu_verifier" \
	"$firmware_verifier" "$ucode_verifier" "$resume_verifier" \
	"$cx_verifier" "$clock_verifier"
do
...
SKIP_V9_UMBRELLA_RUN=1 \
	"$clock_verifier" "$clock_patch" "$source_dir" >/dev/null
...

{
	...
	printf 'gmu_clock_preparation_patch_sha256=%s\n' \
		"$(sha256sum "$clock_patch" | cut -d ' ' -f 1)"
...
echo 'PASS Linux 7.1.4 storage-disabled A660 GMU clock-preparation build'
