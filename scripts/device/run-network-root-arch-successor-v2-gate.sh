...
[ "${ALLOW_ARCH_SUCCESSOR_V2_GATE:-}" = 1 ] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V2_GATE=1 for the one-shot gate'
[ "${ALLOW_ARCH_SUCCESSOR_V2_REBOOT:-}" = 1 ] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V2_REBOOT=1 for immediate fallback'
[ "$(id -u)" -eq 0 ] || fail 'Arch successor v2 gate requires root'
...
[ -f "$seal" ] && [ ! -L "$seal" ] ||
	fail 'successor v2 seal is absent or linked'
[ "$(stat -c '%u:%g:%a' "$seal")" = 0:0:444 ] ||
	fail 'successor v2 seal metadata changed'
[ "$(sha256sum "$seal" | cut -d ' ' -f 1)" = \
	f7c39890f2777d9d95f963bf802a09fe3cbfdb863ac9f80392a61d01867796c4 ] ||
	fail 'successor v2 seal hash changed'
...
disarm=/run/rog5-arch-successor-v2-control/disarm-network-root-watchdog.sh
...
state_dir=$(mktemp -d /run/rog5-arch-successor-v2-transition.XXXXXX)
...
setsid sh -c '
	...
	printf "armed\n" >"$2"
	sleep "$1"
	echo "rog5-arch-successor-v2-transition: watchdog expired" >&8
	echo b >&9
