...
builder=$repo/scripts/device/build-a660-registration-asus-kexec-stage.sh
compare=$repo/scripts/device/compare-asus-kexec-stage-builds.sh

[ -x "$builder" ] || {
	echo 'FAIL missing executable A660 registration ASUS wrapper builder' >&2
	exit 1
}
[ -x "$compare" ]
...
do
	grep -Fq "$contract" "$builder" || {
		echo "FAIL A660 registration wrapper builder omits: $contract" >&2
		exit 1
	}
done
...
if grep -Eq 'curl|wget|git[[:space:]]+(clone|fetch|pull)|fastboot|adb|/dev/(block|disk)|[[:space:]]mount[[:space:]]|[[:space:]]dd[[:space:]]' \
	"$builder"
then
	echo 'FAIL A660 registration wrapper builder has network or device control' >&2
	exit 1
fi

echo 'PASS A660 registration ASUS wrapper build contract is pinned, offline, and reproducible'
