...
artifact_dir=${1:?usage: verify-network-root-gpucc-runtime-pm-candidate-bundle.sh ARTIFACT_DIR MKBOOTIMG_DIR AVBTOOL EXPECTED_SHA256 EXPECTED_MANIFEST_SHA256}
...
runtime_pm_patch=$repo/patches/linux-7.1.4/0011-clk-guard-orphan-reparent-with-runtime-PM.patch
expected_image=d30df38804750ded48607135a7d23d4f95e0947c49b68395a8f6818c4a27c54b
...
expected_patch=a309fe55dc6221f4475c22beb43018dde0f2eb107fa60e84f8e43f28e17a4a25
...
check_exact_hash() {
	...
	[ "$actual" = "$expected" ] || {
		echo "FAIL $label is not the reviewed v15 runtime-PM candidate" >&2
		exit 1
	}
}
...
for identity in \
	...
	"orphan_runtime_pm_patch_sha256=$expected_patch" \
...
echo 'PASS exact v15 runtime-PM candidate bundle; tested CCF ordering, default-off traces, disabled consumers, and zero-storage rollback remain bounded'
