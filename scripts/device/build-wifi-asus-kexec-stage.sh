...
accepted_config=${ACCEPTED_CONFIG:?missing accepted wrapper config}
initramfs_source=${INITRAMFS_SOURCE:-/root/build/rog5-kexec-stage-initramfs.cpio.gz}
initramfs_sha=${INITRAMFS_SHA256:?missing exact Wi-Fi staging initramfs SHA-256}
jobs=${JOBS:-1}
...
[ ! -d "$output_dir" ] ||
	...
	echo 'FAIL refusing nonempty wrapper output directory' >&2
	exit 1
}
mkdir -p "$output_dir"
...
[ -s "$image" ] || {
	echo 'FAIL missing ASUS kexec-stage Image' >&2
	exit 1
}
strings "$image" |
...
echo 'PASS ASUS 5.4.210 wrapper from accepted config; built-in WCN6855 staging initramfs; compile-only'
