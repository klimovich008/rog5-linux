...
patch=$repo/patches/linux-7.1.4/0028-phy-qcom-qmp-ufs-stop-sm8350-after-second-clock-with-runtime-pm.patch
source_root=${ROG5_LINUX_SOURCE:-/home/deck/.local/state/rog5-qmp-ufs-first-clock-runtime-pm-stage-20260813-r2/linux-source}
expected_source=ad56d4021003b1f1c65ee92f583fda232013e301
expected_parent=cdf38b1ddebb802d0659666a351f845e5897f557
...
[[ -f $patch && ! -L $patch ]] || fail 'missing QMP-UFS second-clock runtime-PM patch'
[[ $(git apply --numstat "$patch") == $'5\t5\tdrivers/phy/qualcomm/phy-qcom-qmp-ufs.c' ]] ||
	fail 'QMP-UFS patch changes anything except the exact second-clock boundary'
[[ $(grep -Fc 'ROG5 QMP-UFS diagnostic: second clock with runtime-PM stage complete' "$patch") == 1 ]]
for forbidden in \
...
if [[ ! -d $source_root ]]; then
	...
	echo 'SKIP retained Generation 43 source integration; committed patch contract passed' >&2
	exit 0
fi

[[ -d $source_root && ! -L $source_root && ! -L $source_root/.git ]] ||
	fail 'retained Generation 43 source is unsafe'
[[ $(git -C "$source_root" rev-parse --is-inside-work-tree) == true ]] ||
	fail 'retained Generation 43 source is not a Git worktree'
git -C "$source_root" cat-file -e "$expected_source^{commit}" 2>/dev/null ||
	fail 'retained Generation 43 source commit is absent'
if [[ -n $explicit_source ]]; then
	[[ $(git -C "$source_root" rev-parse HEAD) == "$expected_source" ]] ||
		fail 'explicit Generation 43 source is not exact'
else
	git -C "$source_root" merge-base --is-ancestor "$expected_source" HEAD ||
		fail 'current retained source does not descend from Generation 43'
fi
[[ $(git -C "$source_root" rev-parse "$expected_source^") == "$expected_parent" ]] ||
	fail 'retained Generation 43 source parent changed'
[[ -z $(git -C "$source_root" status --porcelain) ]] ||
	fail 'retained Generation 43 source is dirty'
...
git -C "$patched" diff --quiet "$expected_source" -- \
	...
	fail 'committed second-clock patch does not reproduce the retained source'
...
for line in "$rx0" "$clock0" "$hws0" "$rx1" "$clock1" "$hws1" \
	"$compatible" "$stop" "$tx0" "$clock2" "$provider"; do
	[[ $line =~ ^[1-9][0-9]*$ && $line -gt $previous ]] ||
		fail 'QMP-UFS second-clock operation order changed'
	previous=$line
done

echo 'PASS QMP-UFS runtime-PM discriminator crosses exactly two fixed-rate clocks before returning'
