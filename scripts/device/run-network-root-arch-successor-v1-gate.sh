...
[ "${ALLOW_ARCH_SUCCESSOR_V1_GATE:-}" = 1 ] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V1_GATE=1 for the one-shot gate'
[ "${ALLOW_ARCH_SUCCESSOR_V1_REBOOT:-}" = 1 ] ||
	fail 'set ALLOW_ARCH_SUCCESSOR_V1_REBOOT=1 for immediate fallback'
[ "$(id -u)" -eq 0 ] || fail 'Arch successor v1 gate requires root'
...
[ -f "$seal" ] && [ ! -L "$seal" ] || fail 'successor seal is absent or linked'
[ "$(stat -c '%u:%g:%a' "$seal")" = 0:0:444 ] ||
	fail 'successor seal metadata changed'
[ "$(sha256sum "$seal" | cut -d ' ' -f 1)" = \
	6b5fa1b8e93b7e9f1ad41788ca524d5be6b4195c28ce85f70a28143360109eb4 ] ||
	fail 'successor seal hash changed'
...
disarm=/run/rog5-arch-successor-v1-control/disarm-network-root-watchdog.sh
...
state_dir=$(mktemp -d /run/rog5-arch-successor-v1-transition.XXXXXX)
...
setsid sh -c '
	...
	printf "armed\n" >"$2"
	sleep "$1"
	echo "rog5-arch-successor-v1-transition: watchdog expired" >&8
	echo b >&9
...
echo "PASS Arch successor v1 headless gate kernel=7.1.4-g7a5cef0db479 packages=655 systemd=running coldplug=success tmpfiles=success sysusers=success agent=isolated headless=1 screen=$screen machine_id=volatile lower=sealed storage=0 mounts=0 failed_units=0 transition_watchdog=armed reboot=requested"
wait "$transition_pid"
