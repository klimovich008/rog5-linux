...
initramfs_sha=206ea5897fcbc12d8213041c133357a1f0055c3e233ec8d1a75fc6795e95a2a5
...
[ ! -d "$output_dir" ] ||
	...
	-print -quit)" ] || {
	echo 'FAIL refusing nonempty P2 wrapper output directory' >&2
	exit 1
}
mkdir -p "$output_dir"
...
[ -s "$image" ] || {
	echo 'FAIL missing ASUS P2 kexec-stage Image' >&2
	exit 1
}
strings "$image" |
...
if image.count(initramfs) != 1:
    raise SystemExit("embedded P2 initramfs count is not one")
PY
...
echo 'PASS ASUS 5.4.210 wrapper from accepted config with built-in P2 staging initramfs; compile-only'
