...
target=${TARGET:-$repo/scripts/device/load-mainline-ufs-discovery.sh}
stage=$(mktemp -d)
...
rog5.ufs_discovery=0 rog5.recovery_timeout=180 ramoops.mem_address=0x1000000 ramoops.mem_size=0x400000 ramoops.record_size=0x100000 ramoops.console_size=0x100000 ramoops.pmsg_size=0x100000 ramoops.ftrace_size=0 ramoops.dump_oops=1
EOF
...
[ "$(grep -c 'rog5.recovery_timeout=120' "$KEXEC_RECORD")" -eq 1 ]
! grep -q 'rog5.recovery_timeout=180' "$KEXEC_RECORD"

for invalid in 29 901 not-a-number; do
	rm -f "$KEXEC_RECORD"
	set +e
	PATH=$stage/bin:$PATH \
		PAYLOAD=$stage/payload \
...
echo 'PASS dedicated loader pins discovery mode, watchdog allowlist, and bounded timeout'
