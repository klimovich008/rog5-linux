...
rog5.netroot=0 rog5.recovery_timeout=180 rog5_qcom_cc_probe_trace=1 rog5_ccf_register_trace=1 rog5_rcg2_parent_trace=1 ramoops.mem_address=0x1000000 ramoops.mem_size=0x400000 ramoops.record_size=0x100000 ramoops.console_size=0x100000 ramoops.pmsg_size=0x100000 ramoops.ftrace_size=0 ramoops.dump_oops=1
EOF
...
! grep -q 'rog5_rcg2_parent_trace=' "$KEXEC_RECORD"
...
[ "$(grep -o 'rog5_rcg2_parent_trace=1' "$KEXEC_RECORD" | wc -l)" -eq 1 ]
...
for invalid in -1 2 yes; do
	...
	PATH=$stage/bin:$PATH \
		...
		SYS_DEVICES=$stage/sys \
		PROC_CMDLINE=$stage/cmdline \
		ROG5_RCG2_PARENT_TRACE=$invalid \
		"$target" >/dev/null 2>&1
	status=$?
	set -e
	[ "$status" -ne 0 ]
...
echo 'PASS network-root loader pins mode, trace flag, diagnostic masks, watchdog allowlist, and timeout'
