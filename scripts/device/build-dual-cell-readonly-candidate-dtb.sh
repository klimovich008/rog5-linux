...
base=${1:?usage: build-dual-cell-readonly-candidate-dtb.sh TELEMETRY_BASE OVERLAY OUTPUT}
overlay=${2:?missing read-only dual-cell overlay}
output=${3:?missing output}
script_dir=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
verifier=$script_dir/verify-dual-cell-readonly-dtb-delta.py
...
dtc -q -@ -I dts -O dtb -o "$stage/dual-cell-readonly.dtbo" "$overlay"
output_parent=$(dirname "$output")
mkdir -p "$output_parent"
publish_stage=$(mktemp -d "$output_parent/.rog5-dual-cell-readonly-dtb.XXXXXX")
candidate=$publish_stage/candidate.dtb
fdtoverlay -i "$base" -o "$candidate" "$stage/dual-cell-readonly.dtbo"
...
publish_stage=
sha256sum "$output"
echo 'PASS opt-in read-only dual-cell candidate DTB'
