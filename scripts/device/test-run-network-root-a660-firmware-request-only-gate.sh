...
[ -x "$gate" ] || {
	echo 'FAIL missing compound A660 firmware-request-only gate' >&2
	exit 1
}
sh -n "$gate"

for contract in \
	'ALLOW_MAINLINE_A660_FIRMWARE_REQUEST_ONLY_GATE' \
	...
	'/.rog5/root-ro/usr/local/sbin/rog5-a660-firmware-request-only-baseline' \
	'/.rog5/root-ro/usr/local/sbin/rog5-a660-firmware-request-only-probe' \
	'/run/rog5-a660-firmware-request-only-control/disarm-network-root-a660-watchdog.sh' \
	88db9503be4c4ee58639bc1afdfdf2958a419c4c2a7d5d6db966a255264026ff \
	17a8d45e6dec02f1977800eab8562c12f7ef3841a92d7797b0ffc4d86313c25e \
	733a2ba85e192e982883de1afee04e9fee0d137d737a611ad0914f185885fbbc \
	'transition_timeout=180' \
	'/run/rog5-a660-firmware-request-transition.' \
	'exec 9>/proc/sysrq-trigger' \
	'echo b >&9' \
	'setsid sh -c' \
	'expected=OPEN_ERRNO=117' \
	'ALLOW_A660_NETWORK_ROOT_WATCHDOG_DISARM=1 "$disarm"' \
	'ALLOW_MAINLINE_A660_FIRMWARE_REQUEST_ONLY=1 "$probe"' \
	'systemctl reboot --no-block' \
	'open_errno=117 transition_watchdog=armed reboot=requested'
do
	grep -Fq "$contract" "$gate" || {
		echo "FAIL compound request-only gate omits: $contract" >&2
		exit 1
	}
done
...
disarm_line=$(grep -n \
	...
	cut -d: -f1)
probe_line=$(grep -n \
	'^ALLOW_MAINLINE_A660_FIRMWARE_REQUEST_ONLY=1 "\$probe"$' "$gate" |
	cut -d: -f1)
...
[ "$(grep -Fc 'ALLOW_A660_NETWORK_ROOT_WATCHDOG_DISARM=1 "$disarm"' \
	"$gate")" -eq 1 ]
[ "$(grep -Fc 'ALLOW_MAINLINE_A660_FIRMWARE_REQUEST_ONLY=1 "$probe"' \
	"$gate")" -eq 1 ]
...
if grep -Eq \
	...
	"$gate"
then
	echo 'FAIL compound request-only gate controls host or writes storage' >&2
	exit 1
fi
...
echo 'PASS compound A660 request-only gate overlaps watchdogs, invokes one probe, and immediately reboots'
