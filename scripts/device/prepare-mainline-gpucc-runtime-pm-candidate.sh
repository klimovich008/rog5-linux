...
base_source=${1:?usage: prepare-mainline-gpucc-runtime-pm-candidate.sh BASE_SOURCE TARGET_SOURCE}
target_source=${2:?missing target source}
repo=$(CDPATH= cd -- "$(dirname "$0")/../.." && pwd)
prepare_v14=$repo/scripts/device/prepare-mainline-gpucc-rcg2-diagnostic.sh
...
[ -x "$prepare_v14" ] && [ -x "$verifier" ] && [ -r "$patch" ]
"$prepare_v14" "$base_source" "$target_source" >/dev/null
...
[ "$(git -C "$target_source" rev-parse HEAD^)" = "$expected_rcg2" ]
[ "$(git -C "$target_source" rev-parse HEAD)" = "$expected_runtime_pm" ]
...
printf 'runtime_pm_commit=%s\n' "$expected_runtime_pm"
printf 'runtime_pm_tree=%s\n' "$expected_tree"
sha256sum "$patch"
echo 'PASS deterministic Linux 7.1.4 GPUCC orphan runtime-PM candidate source'
