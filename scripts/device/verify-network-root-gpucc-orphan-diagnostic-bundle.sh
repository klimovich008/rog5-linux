...
artifact_dir=${1:?usage: verify-network-root-gpucc-orphan-diagnostic-bundle.sh ARTIFACT_DIR MKBOOTIMG_DIR AVBTOOL EXPECTED_SHA256}
...
expected_image=49318395c5ed4850d492e4f29ea841885692bd96b6a5b0982925769282b687d9
expected_modules=2c246d8ceed3c37cc2afefa56710ac5bbca2bc1bce0ca0409a361f8f5923a2e8
expected_module=79a7d3b7d81c28821dd5199cdbcfe9b2cea5b8bc59b6d6e983a61a15f05424ba
expected_manifest=234f8ab909fd8804cf400a3aa1fb8a88e6633047b7680ba331ac308019a3ec04
...
"$repo/scripts/device/verify-ccf-orphan-reparent-trace-patch.sh" \
	...
	>/dev/null
"$repo/scripts/device/test-ccf-orphan-reparent-trace-budget.sh" >/dev/null
...
check_exact_hash() {
	...
	[ "$actual" = "$expected" ] || {
		echo "FAIL $label is not the reviewed orphan diagnostic build" >&2
		exit 1
	}
}
...
grep -qx \
	...
	"$meta"
grep -qx \
	'patched_tree=040d5f9b7be022489079b2ea9cab20a04934d85f' "$meta"
grep -qx \
...
echo 'PASS exact bounded orphan diagnostic bundle; four-entry trace remains opt-in, external, consumer-free, and rollback-bounded'
