...
for symbol in ATH11K ATH11K_PCI CFG80211 MAC80211 RFKILL MHI_BUS QRTR_MHI \
...
for module in phy-qcom-qmp-pcie pci-pwrctrl-pwrseq pwrseq-qcom-wcn qrtr-mhi ath11k_pci cmac; do
	modprobe -d "$root" -S "$release" --show-depends "$module"
done | sort -u >"$output/load-closure.txt"
