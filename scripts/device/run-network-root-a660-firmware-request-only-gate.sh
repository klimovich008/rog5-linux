...
[ "${ALLOW_MAINLINE_A660_FIRMWARE_REQUEST_ONLY_GATE:-}" = 1 ] ||
	fail 'set ALLOW_MAINLINE_A660_FIRMWARE_REQUEST_ONLY_GATE=1 for the one-shot gate'
...
[ "$(id -u)" -eq 0 ] ||
	fail 'compound A660 firmware-request-only gate requires root'
...
baseline=/.rog5/root-ro/usr/local/sbin/rog5-a660-firmware-request-only-baseline
probe=/.rog5/root-ro/usr/local/sbin/rog5-a660-firmware-request-only-probe
disarm=/run/rog5-a660-firmware-request-only-control/disarm-network-root-a660-watchdog.sh
...
state_dir=$(mktemp -d /run/rog5-a660-firmware-request-transition.XXXXXX)
...
setsid sh -c '
	...
	printf "armed\n" >"$2"
	sleep "$1"
	echo "rog5-a660-firmware-request-transition: watchdog expired" >&8
	echo b >&9
...
echo "BEGIN A660 firmware-request-only gate transition_watchdog=${transition_timeout}s expected=OPEN_ERRNO=117"
