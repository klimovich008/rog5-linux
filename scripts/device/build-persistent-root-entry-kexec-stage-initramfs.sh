...
base=${1:?usage: build-persistent-root-entry-kexec-stage-initramfs.sh BASE_STAGE IMAGE DTB INITRAMFS LOADER OUTPUT}
image_input=${2:?missing P2 entry Image}
dtb_input=${3:?missing P2 entry DTB}
initramfs_input=${4:?missing P2 entry target initramfs}
loader_input=${5:?missing P2 entry loader}
...
initramfs_sha=09f7e69daf270c584b1947f41872a9af512c47e26fb2e8a30d3cdfb2fcc5d7a5
loader_sha=a21f2a653c6237253cd0039fec2e15c9348afe714ec3e77b9e2c7b5baf78f8af
epoch=1681862400

check_hash() {
	[ -f "$1" ] && [ ! -L "$1" ] && [ -r "$1" ] || {
		echo "FAIL missing or linked P2 entry stage input: $1" >&2
		exit 1
	}
	[ "$(sha256sum "$1" | cut -d ' ' -f 1)" = "$2" ] || {
		echo "FAIL P2 entry stage input hash changed: $(basename "$1")" >&2
		exit 1
	}
}
...
for input in "$base" "$image_input" "$dtb_input" \
	...
	[ "$output_real" != "$(readlink -f -- "$input")" ] || {
		echo 'FAIL P2 entry stage output aliases an input' >&2
		exit 1
	}
done
[ ! -e "$output_real" ] || {
	echo "FAIL refusing existing P2 entry stage output: $output_real" >&2
	exit 1
}
...
gzip -t "$output_real.tmp"
chmod 0644 "$output_real.tmp"
...
echo 'PASS deterministic credential-free P2 early-entry stage from accepted UFS wrapper'
