...
output_dir=${1:?usage: verify-mainline-a660-gmu-clock-preparation-build.sh BUILD_DIR PINNED_SOURCE FIRMWARE_ROOT ACCEPTED_CONFIG REGISTRATION_BUILD FIRMWARE_ONLY_BUILD UCODE_BUILD RESUME_BUILD V10_BUILD}
...
v10_build=${9:?missing accepted GMU/CX runtime-PM v10 build}
...
predecessor_verifier=$repo/scripts/device/verify-mainline-a660-gmu-cx-runtime-pm-build.sh
patch=$repo/patches/linux-7.1.4/0017-drm-msm-add-a660-gmu-clock-preparation-diagnostic.patch
patch_verifier=$repo/scripts/device/verify-a660-gmu-clock-preparation-patch.sh
accepted_v10_meta_sha256=dbc7270338b3c0589863db84fa9bc2abc63a1dfcfb42f83c1394f48122c298cb
accepted_v10_archive_sha256=87e5c3bae7d5034b64aea7212be8372506bf8b28cbdca7fb1b79bb20db50b9d0
accepted_v10_msm_sha256=c36fd352c48d624eff9f17fb8200c8f151209eae066d1f276ab3bff67c5d465d
accepted_clock_meta_sha256=5e929fef4ec6422c8aae7f7e97d98a852769c8643aadd6069655f0471056338a
accepted_clock_archive_sha256=86a163a4d065ffb66cf5befc10fdb045b955d8e227e576169a40c2ac129ad94b
accepted_clock_msm_sha256=3ca8b7996a94d9a57b3c17f26bd5be4df948ee176c6faa58b76900dbb3f47251
...
SKIP_V7_UMBRELLA_RUN=1 SKIP_V9_UMBRELLA_RUN=1 \
	"$predecessor_verifier" "$v10_build" "$source_dir" \
		"$firmware_root" "$accepted_config" "$registration_build" \
		"$firmware_build" "$ucode_build" "$resume_build" >/dev/null
...
for identity in \
	...
	'gmu_clock_preparation_patch_sha256=e7512f8e0589187bddb93f53d83a31b415ce779b3093623fad5515210cf1258b' \
	'patched_a6xx_gmu_sha256=176391492beacf6b08a0e5d9f45bec7147809779da3a1a2f511cccaebf548c17' \
	'patched_msm_drv_sha256=44b9d1281819a3812711786d488fac8ac727dc24f079c6d0e886ee2cb5a60c14' \
	'patched_msm_gpu_sha256=9065053f0ed68a0a200270aa42548cb021e6e26c035dcb0c4ce53341d3c0bfca' \
...
check_meta_hash gmu_cx_runtime_pm_patch_sha256 \
	"$repo/patches/linux-7.1.4/0016-drm-msm-add-a660-gmu-cx-runtime-pm-diagnostic.patch"
check_meta_hash gmu_clock_preparation_patch_sha256 "$patch"
...
check_hash "$config" \
	...
	'accepted clock-preparation config'
check_hash "$image" \
	52624d3fc8d51234ff4f4d4c31f302d2e031c736997dd9645e96e89b4fcd00db \
	'accepted clock-preparation Image'
check_hash "$image_gz" \
	9d735cac93cf5720e885a4151e3eb8c1257aa52167dd40b439252adb31fbe307 \
	'accepted clock-preparation Image.gz'
check_hash "$symvers" \
	a26677921a28fc0744fe8976c3f7c9dae4257617a92a0a2f18c21ad6d03a4477 \
	'accepted clock-preparation Module.symvers'
check_hash "$gpucc_module" \
	c4ec64279bc72ac9d524717512c94a8f5c47eba6a4cf7cadbfbc620012dac563 \
	'accepted clock-preparation GPUCC module'
check_hash "$mdt_module" \
	001c1526eef5526b35663e9d3d16621e07949e3adc0ca4aa4ddd5f1f7a4e4be3 \
	'accepted clock-preparation MDT module'

if [ "${ALLOW_UNPINNED_BUILD:-0}" != 1 ]; then
	case "$accepted_clock_meta_sha256:$accepted_clock_archive_sha256:$accepted_clock_msm_sha256" in
		*PENDING_V11*)
			fail 'accepted clock-preparation build hashes are not pinned'
			;;
	esac
	check_hash "$meta" "$accepted_clock_meta_sha256" \
		'accepted clock-preparation build metadata'
	check_hash "$archive" "$accepted_clock_archive_sha256" \
		'accepted clock-preparation module archive'
	check_hash "$msm_module" "$accepted_clock_msm_sha256" \
		'accepted clock-preparation MSM module'
fi
...
do
	grep -qx "$symbol" "$config" ||
		fail "clock-preparation config omits: $symbol"
done
[ "$kms_state" = CONFIG_DRM_MSM_KMS=n ] ||
	fail 'internal KMS-disabled contract changed'
if grep -Eq '^CONFIG_DRM_MSM_KMS=(y|m)$' "$config"; then
	fail 'clock-preparation config enables CONFIG_DRM_MSM_KMS'
fi

[ "$(modinfo -F name "$msm_module")" = msm ] ||
	fail 'clock-preparation MSM module name changed'
[ "$(modinfo -F vermagic "$msm_module")" = \
	"$expected_release SMP preempt mod_unload aarch64" ] ||
	fail 'clock-preparation MSM module vermagic changed'
for parameter in \
...
do
	modinfo -p "$msm_module" | grep -Fxq "$parameter" ||
		fail "clock-preparation module parameter is missing: $parameter"
done
for marker in \
...
readelf -S "$msm_module" | grep -Eq '[[:space:]][.]BTF[[:space:]]' ||
	fail 'clock-preparation MSM module has no BTF section'

firmware_pattern='a660_sqe[.]fw|a660_gmu[.]bin|a660_zap[.]mbn'
if tar -tzf "$archive" | grep -Eq "$firmware_pattern"; then
	fail 'A660 firmware exists in the clock-preparation module archive'
fi
if find "$output_dir" -type f -printf '%f\n' |
	grep -Eq "$firmware_pattern"
then
	fail 'A660 firmware exists in the clock-preparation build'
fi

check_hash "$v10_build/build-meta.txt" "$accepted_v10_meta_sha256" \
	'accepted v10 build metadata'
check_hash "$v10_build/modules.tar.gz" "$accepted_v10_archive_sha256" \
	'accepted v10 module archive'
check_hash "$v10_build/drivers/gpu/drm/msm/msm.ko" \
	"$accepted_v10_msm_sha256" 'accepted v10 MSM module'
...
do
	cmp "$output_dir/$rel" "$v10_build/$rel" ||
		fail "accepted v10 artifact changed: $rel"
done
if cmp -s "$msm_module" "$v10_build/drivers/gpu/drm/msm/msm.ko"; then
	fail 'clock-preparation MSM module differs contract was not met'
fi
if cmp -s "$archive" "$v10_build/modules.tar.gz"; then
	fail 'clock-preparation module archive does not differ from accepted v10'
fi

stage=$(mktemp -d)
trap 'rm -rf "$stage"' EXIT INT TERM
mkdir -p "$stage/v10" "$stage/gmu-clock"
tar -xzf "$v10_build/modules.tar.gz" -C "$stage/v10"
tar -xzf "$archive" -C "$stage/gmu-clock"
module_rel=lib/modules/$expected_release/kernel/drivers/gpu/drm/msm/msm.ko
cmp "$stage/v10/$module_rel" \
	"$v10_build/drivers/gpu/drm/msm/msm.ko" ||
	fail 'accepted v10 archive MSM module differs from build output'
cmp "$stage/gmu-clock/$module_rel" "$msm_module" ||
	fail 'clock-preparation archive MSM module differs from build output'

(cd "$stage/v10" && find "lib/modules/$expected_release" -type f \
	-printf '%P\n' | sort) >"$stage/v10-files"
(cd "$stage/gmu-clock" && find "lib/modules/$expected_release" -type f \
	-printf '%P\n' | sort) >"$stage/gmu-clock-files"
cmp "$stage/v10-files" "$stage/gmu-clock-files" ||
	fail 'module archive file lists differ'

different=0
while IFS= read -r rel; do
	v10_file=$stage/v10/lib/modules/$expected_release/$rel
	clock_file=$stage/gmu-clock/lib/modules/$expected_release/$rel
	if cmp -s "$v10_file" "$clock_file"; then
		continue
	fi
	[ "$rel" = kernel/drivers/gpu/drm/msm/msm.ko ] ||
		fail "unexpected module archive difference: $rel"
	different=$((different + 1))
done <"$stage/v10-files"
[ "$different" -eq 1 ] ||
	fail "module archives have $different changed files, expected 1"

(cd "$stage/v10" && find "lib/modules/$expected_release" -type l \
	-printf '%P %l\n' | sort) >"$stage/v10-links"
(cd "$stage/gmu-clock" && find "lib/modules/$expected_release" -type l \
	-printf '%P %l\n' | sort) >"$stage/gmu-clock-links"
cmp "$stage/v10-links" "$stage/gmu-clock-links" ||
	fail 'module archive symlink targets differ'
[ "$(readlink \
	"$stage/gmu-clock/lib/modules/$expected_release/build")" = \
	/root/build/rog5-linux-7.1.4-a660-registration ] ||
	fail 'clock-preparation archive build link is not reproducible'

echo 'PASS accepted v10 Image is unchanged'
echo 'PASS clock-preparation MSM module differs only from its accepted v10 predecessor'
echo 'PASS A660 GMU clock-preparation build is exact-stack, modular, firmware-clean, BTF-bearing, archive-isolated, and offline-only'
