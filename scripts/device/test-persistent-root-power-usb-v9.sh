...
successor=$repo/configs/recovery-candidates/persistent-root-local-v54.json
successor_initramfs=$repo/artifacts/persistent-root-local-v54/initramfs.cpio.gz
successor_manifest=$repo/manifests/persistent-root-local-v54-generation163.manifest
successor_claim=$repo/scripts/host/consume-persistent-root-local-v54-claim.py
...
record = json.loads(Path(sys.argv[1]).read_text(encoding="ascii"))
assert record["candidate"] == "persistent-root-local-v54"
assert record["status"] == "consumed"
assert record["authority"] == "none"
assert record["target_release"] == "7.1.4-gae717d919f87"
artifact = record["artifacts"]["initramfs.cpio.gz"]
assert artifact["size"] == 24007550
...
if path.exists():
    ...
    assert hashlib.file_digest(path.open("rb"), "sha256").hexdigest() == \
        artifact["sha256"]
PY
grep -Fxq 'avb_generation=163' "$successor_manifest"
grep -Fxq 'probe_policy=staged-seal-absent' "$successor_manifest"
grep -Fxq 'shutdown=exitrd-restart2-helper' "$successor_manifest"
...
grep -Fq 'persistent-root-local-v54-generation163-live-v1' "$successor_claim"
