...
for input in "$builder" "$verifier"; do
	[ -x "$input" ] || {
		echo "FAIL missing A660 ucode-allocation v7 runtime tool: $input" >&2
		exit 1
	}
done
...
expect_rejected() {
	...
	if ALLOW_UNPINNED_A660_UCODE_V7_RUNTIME=1 \
		...
		>"$work/$name.log" 2>&1
	then
		echo "FAIL ucode-allocation v7 runtime mutation passed: $name" >&2
		exit 1
	fi
}
...
sed "s/printf '4\\\\n4096\\\\n43288\\\\n'/printf '4096\\\\n4096\\\\n45056\\\\n'/" \
	"$probe" >"$work/rejected-rounded-size-oracle.sh"
expect_rejected rejected-rounded-size-oracle "$baseline" \
	"$work/rejected-rounded-size-oracle.sh"

sed '/p:rog5_ucode_v7\/rog5_ucode_kernel_new msm:msm_gem_kernel_new/d' \
	"$probe" >"$work/no-kernel-new-probe.sh"
expect_rejected no-kernel-new-probe "$baseline" \
	"$work/no-kernel-new-probe.sh"

sed '/p:rog5_ucode_v7\/rog5_ucode_kernel_put msm:msm_gem_kernel_put/d' \
...
sed '/p:rog5_ucode_v7\/rog5_ucode_scm_aperture qcom_scm_set_gpu_smmu_aperture/d' \
...
sed 's/predecessor=v6_live_rejected_consumed/predecessor=v6_live_pending/' \
...
echo 'PASS A660 ucode-allocation v7 rejects unarmed or mixed modes, second open, wrong errno, unfiltered/incomplete traces, rejected wrapper/rounded-size oracles, missing logical/snapshot state, wrong predecessor/firmware, and reboot bypass'
