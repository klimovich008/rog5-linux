...
build_a=${1:?usage: compare-mainline-a660-firmware-request-only-builds.sh BUILD_A BUILD_B}
...
echo 'PASS two clean A660 firmware-request-only builds are byte-identical and the firmware-only MSM module differs only from its accepted registration predecessor'
