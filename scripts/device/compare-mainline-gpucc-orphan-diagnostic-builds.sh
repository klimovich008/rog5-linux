...
build_a=${1:?usage: compare-mainline-gpucc-orphan-diagnostic-builds.sh BUILD_A BUILD_B}
...
do
	...
	cmp -s "$build_a/$file" "$build_b/$file" || {
		echo "FAIL orphan diagnostic clean-build mismatch: $file" >&2
		exit 1
	}
done

echo 'PASS two clean orphan diagnostic builds are byte-identical through BTF, objects, symbols, modules, and metadata'
