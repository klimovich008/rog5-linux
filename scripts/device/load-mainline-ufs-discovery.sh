...
command_line='console=ttyMSM0,115200n8 rdinit=/init panic=10 oops=panic loglevel=8 ignore_loglevel initcall_debug printk.always_kmsg_dump=Y dyndbg="func deferred_probe_work_func +p" rog5.ufs_discovery=1'
...
[ "$(printf '%s\n' "$command_line" | tr ' ' '\n' |
	grep -c '^rog5\.ufs_discovery=1$')" -eq 1 ]
[ "$(printf '%s\n' "$command_line" |
...
echo 'PASS mainline UFS discovery payload loaded; run kexec -e only during an attended test'
