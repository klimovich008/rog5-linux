...
seal=$lower/etc/rog5/a660-firmware-request-only-export
...
for command in awk basename cat cut dmesg find findmnt grep id ip modinfo \
	readlink sed sha256sum sleep stat systemctl tail tr uname wc; do
...
[ -f "$seal" ] && [ ! -L "$seal" ] ||
	fail 'firmware-request-only export seal is absent'
[ "$(stat -c '%u:%g:%a' "$seal")" = 0:0:444 ] ||
	fail 'firmware-request-only export seal metadata is not exact'
...
modinfo -p "$module_root/kernel/drivers/gpu/drm/msm/msm.ko" |
	...
	fail 'MSM module lacks the request-only parameter'
...
printf 'PASS A660-firmware-request-only baseline storage=0 mounts=0 firmware_files=2 firmware_requests=0 zap=absent render=0 drm_fds=0 failed_units=0 thermal_zones=%s thermal_max_mC=%s module_files=7 helper=exact watchdog=armed\n' \
