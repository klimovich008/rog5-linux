...
trace_patch=${TRACE_PATCH:-/workspace/repo/patches/linux-7.1.4/0005-gpucc-sm8350-add-attended-probe-trace.patch}
...
for input in "$base_fragment" "$network_fragment" "$trace_patch"; do
	[ -r "$input" ]
done
[ "$(git -C "$source_dir" rev-parse HEAD^)" = "$expected_base" ]
[ "$(git -C "$source_dir" rev-parse HEAD)" = "$expected_patched" ]
...
[ -s "$module" ]
{
	...
	printf 'trace_patch_sha256=%s\n' \
		"$(sha256sum "$trace_patch" | cut -d ' ' -f 1)"
...
echo 'PASS Linux 7.1.4 network-root build with default-off GPUCC probe tracing'
