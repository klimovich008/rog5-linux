...
[ "${ALLOW_MAINLINE_A660_GMU_RESUME_ENTRY_V9_GATE:-}" = 1 ] ||
	fail 'set ALLOW_MAINLINE_A660_GMU_RESUME_ENTRY_V9_GATE=1 for the one-shot gate'
...
[ "$(id -u)" -eq 0 ] ||
	fail 'compound A660 GMU resume-entry v9 gate requires root'
...
baseline=/.rog5/root-ro/usr/local/sbin/rog5-a660-gmu-resume-entry-v9-baseline
probe=/.rog5/root-ro/usr/local/sbin/rog5-a660-gmu-resume-entry-v9-probe
disarm=/run/rog5-a660-gmu-resume-entry-v9-control/disarm-network-root-a660-watchdog.sh
...
state_dir=$(mktemp -d /run/rog5-a660-gmu-resume-entry-v9-transition.XXXXXX)
...
setsid sh -c '
	...
	printf "armed\n" >"$2"
	sleep "$1"
	echo "rog5-a660-gmu-resume-entry-v9-transition: watchdog expired" >&8
	echo b >&9
...
echo "BEGIN A660 GMU resume-entry v9 gate transition_watchdog=${transition_timeout}s expected=OPEN_ERRNO=117"
