...
[ "${ALLOW_MAINLINE_A660_REGISTRATION_GATE:-}" = 1 ] ||
	fail 'set ALLOW_MAINLINE_A660_REGISTRATION_GATE=1 for the one-shot gate'
[ "${ALLOW_MAINLINE_A660_REGISTRATION_REBOOT:-}" = 1 ] ||
	fail 'set ALLOW_MAINLINE_A660_REGISTRATION_REBOOT=1 for immediate fallback'
[ "$(id -u)" -eq 0 ] || fail 'compound A660 registration gate requires root'
...
baseline=/.rog5/root-ro/usr/local/sbin/rog5-a660-registration-baseline
probe=/.rog5/root-ro/usr/local/sbin/rog5-a660-registration-probe
disarm=/run/rog5-a660-registration-control/disarm-network-root-a660-watchdog.sh
...
state_dir=$(mktemp -d /run/rog5-a660-registration-transition.XXXXXX)
...
setsid sh -c '
	...
	printf "armed\n" >"$2"
	sleep "$1"
	echo "rog5-a660-registration-transition: watchdog expired" >&8
	echo b >&9
...
echo "BEGIN A660 registration gate transition_watchdog=${transition_timeout}s"
