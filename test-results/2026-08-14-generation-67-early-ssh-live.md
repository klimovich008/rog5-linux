...
key-only SSH, and full storage attestation. The recovery ACM closed after the
COMMIT claim but before the post-claim response reached the host. Generation 67
...
the exact 45,810,825-byte bundle and accepted the COMMIT claim. Its ACM endpoint
then closed before the controller received the post-claim response, so the
formal lifecycle correctly refused target acceptance. The same already-running
...
It should make the post-COMMIT acknowledgement robust against the recovery ACM
closing for kexec, then receive its own clean-twin identity, claim, and sole
RAM-only cycle.
